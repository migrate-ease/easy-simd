#define EASYSIMD_TEST_ARM_NEON_INSN cltz

#include "test-neon.h"
#include "../../../easysimd/arm/neon/cltz.h"
#include "../../../easysimd/arm/neon/reinterpret.h"

static int
test_easysimd_vcltz_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    easysimd_float32 a[2];
    uint32_t r[2];
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(  -600.28), EASYSIMD_FLOAT32_C(  -155.01) },
      {           UINT32_MAX,           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(   941.01), EASYSIMD_FLOAT32_C(   717.45) },
      { UINT32_C(         0), UINT32_C(         0) } },
    { { EASYSIMD_FLOAT32_C(    22.41), EASYSIMD_FLOAT32_C(  -542.54) },
      { UINT32_C(         0),           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(  -400.37), EASYSIMD_FLOAT32_C(  -711.08) },
      {           UINT32_MAX,           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(   591.87), EASYSIMD_FLOAT32_C(  -579.22) },
      { UINT32_C(         0),           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(  -322.37), EASYSIMD_FLOAT32_C(    93.44) },
      {           UINT32_MAX, UINT32_C(         0) } },
    { { EASYSIMD_FLOAT32_C(  -455.16), EASYSIMD_FLOAT32_C(   540.07) },
      {           UINT32_MAX, UINT32_C(         0) } },
    { { EASYSIMD_FLOAT32_C(   983.37), EASYSIMD_FLOAT32_C(   113.16) },
      { UINT32_C(         0), UINT32_C(         0) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x2_t a = easysimd_vld1_f32(test_vec[i].a);
    easysimd_uint32x2_t r = easysimd_vcltz_f32(a);
...
static int
test_easysimd_vcltz_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    easysimd_float64 a[1];
    uint64_t r[1];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(  -383.68) },
      {                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(  -437.89) },
      {                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(   762.24) },
      { UINT64_C(                   0) } },
    { { EASYSIMD_FLOAT64_C(  -529.26) },
      {                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(   863.53) },
      { UINT64_C(                   0) } },
    { { EASYSIMD_FLOAT64_C(  -770.66) },
      {                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(  -387.78) },
      {                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(   354.63) },
      { UINT64_C(                   0) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x1_t a = easysimd_vld1_f64(test_vec[i].a);
    easysimd_uint64x1_t r = easysimd_vcltz_f64(a);
...
static int
test_easysimd_vcltz_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int8_t a[8];
    uint8_t r[8];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x8_t r = easysimd_vcltz_s8(a);
...
static int
test_easysimd_vcltz_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int16_t a[4];
    uint16_t r[4];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x4_t r = easysimd_vcltz_s16(a);
...
static int
test_easysimd_vcltz_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int32_t a[2];
    uint32_t r[2];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x2_t r = easysimd_vcltz_s32(a);
...
static int
test_easysimd_vcltz_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int64_t a[1];
    uint64_t r[1];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint64x1_t r = easysimd_vcltz_s64(a);
...
static int
test_easysimd_vcltzq_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    easysimd_float32 a[4];
    uint32_t r[4];
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(  -442.45), EASYSIMD_FLOAT32_C(   552.37), EASYSIMD_FLOAT32_C(  -139.46), EASYSIMD_FLOAT32_C(   880.73) },
      {           UINT32_MAX, UINT32_C(         0),           UINT32_MAX, UINT32_C(         0) } },
    { { EASYSIMD_FLOAT32_C(  -535.61), EASYSIMD_FLOAT32_C(  -314.97), EASYSIMD_FLOAT32_C(  -324.37), EASYSIMD_FLOAT32_C(  -863.63) },
      {           UINT32_MAX,           UINT32_MAX,           UINT32_MAX,           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(  -142.67), EASYSIMD_FLOAT32_C(  -456.23), EASYSIMD_FLOAT32_C(   330.18), EASYSIMD_FLOAT32_C(  -127.74) },
      {           UINT32_MAX,           UINT32_MAX, UINT32_C(         0),           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(   725.38), EASYSIMD_FLOAT32_C(  -382.66), EASYSIMD_FLOAT32_C(   959.27), EASYSIMD_FLOAT32_C(  -336.93) },
      { UINT32_C(         0),           UINT32_MAX, UINT32_C(         0),           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(   249.07), EASYSIMD_FLOAT32_C(   860.93), EASYSIMD_FLOAT32_C(    46.17), EASYSIMD_FLOAT32_C(   812.32) },
      { UINT32_C(         0), UINT32_C(         0), UINT32_C(         0), UINT32_C(         0) } },
    { { EASYSIMD_FLOAT32_C(  -849.19), EASYSIMD_FLOAT32_C(  -336.50), EASYSIMD_FLOAT32_C(   322.45), EASYSIMD_FLOAT32_C(  -511.47) },
      {           UINT32_MAX,           UINT32_MAX, UINT32_C(         0),           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(  -105.46), EASYSIMD_FLOAT32_C(   857.83), EASYSIMD_FLOAT32_C(   267.60), EASYSIMD_FLOAT32_C(  -602.28) },
      {           UINT32_MAX, UINT32_C(         0), UINT32_C(         0),           UINT32_MAX } },
    { { EASYSIMD_FLOAT32_C(   768.23), EASYSIMD_FLOAT32_C(  -752.53), EASYSIMD_FLOAT32_C(  -518.24), EASYSIMD_FLOAT32_C(  -674.22) },
      { UINT32_C(         0),           UINT32_MAX,           UINT32_MAX,           UINT32_MAX } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x4_t a = easysimd_vld1q_f32(test_vec[i].a);
    easysimd_uint32x4_t r = easysimd_vcltzq_f32(a);
...
static int
test_easysimd_vcltzq_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    easysimd_float64 a[2];
    uint64_t r[2];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(   -80.27), EASYSIMD_FLOAT64_C(   326.82) },
      {                    UINT64_MAX, UINT64_C(                   0) } },
    { { EASYSIMD_FLOAT64_C(  -489.46), EASYSIMD_FLOAT64_C(  -457.68) },
      {                    UINT64_MAX,                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(  -441.47), EASYSIMD_FLOAT64_C(     2.14) },
      {                    UINT64_MAX, UINT64_C(                   0) } },
    { { EASYSIMD_FLOAT64_C(  -692.88), EASYSIMD_FLOAT64_C(   270.65) },
      {                    UINT64_MAX, UINT64_C(                   0) } },
    { { EASYSIMD_FLOAT64_C(   806.25), EASYSIMD_FLOAT64_C(   705.39) },
      { UINT64_C(                   0), UINT64_C(                   0) } },
    { { EASYSIMD_FLOAT64_C(  -398.22), EASYSIMD_FLOAT64_C(  -504.15) },
      {                    UINT64_MAX,                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(   952.43), EASYSIMD_FLOAT64_C(  -573.18) },
      { UINT64_C(                   0),                    UINT64_MAX } },
    { { EASYSIMD_FLOAT64_C(  -443.24), EASYSIMD_FLOAT64_C(   861.65) },
      {                    UINT64_MAX, UINT64_C(                   0) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x2_t a = easysimd_vld1q_f64(test_vec[i].a);
    easysimd_uint64x2_t r = easysimd_vcltzq_f64(a);
...
static int
test_easysimd_vcltzq_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int8_t a[16];
    uint8_t r[16];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x16_t r = easysimd_vcltzq_s8(a);
...
static int
test_easysimd_vcltzq_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int16_t a[8];
    uint16_t r[8];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x8_t r = easysimd_vcltzq_s16(a);
...
static int
test_easysimd_vcltzq_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int32_t a[4];
    uint32_t r[4];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x4_t r = easysimd_vcltzq_s32(a);
    easysimd_test_arm_neon_assert_equal_u32x4(r, easysimd_vld1q_u32(test_vec[i].r));
  }
...
static int
test_easysimd_vcltzq_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
  static const struct {
    int64_t a[2];
    uint64_t r[2];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint64x2_t r = easysimd_vcltzq_s64(a);
    easysimd_test_arm_neon_assert_equal_u64x2(r, easysimd_vld1q_u64(test_vec[i].r));
  }
...
static int
test_easysimd_vcltzd_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a;
    uint64_t r;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint64_t r = easysimd_vcltzd_s64(test_vec[i].a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint64_t r = easysimd_vcltzd_s64(a);
...
static int
test_easysimd_vcltzd_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a;
    uint64_t r;
  } test_vec[] = {
    { EASYSIMD_FLOAT64_C(   918.77),
      UINT64_C(                   0) },
    { EASYSIMD_FLOAT64_C(  -706.44),
                          UINT64_MAX },
    { EASYSIMD_FLOAT64_C(  -108.73),
                          UINT64_MAX },
    { EASYSIMD_FLOAT64_C(  -943.89),
                          UINT64_MAX },
    { EASYSIMD_FLOAT64_C(   654.01),
      UINT64_C(                   0) },
    { EASYSIMD_FLOAT64_C(   689.80),
      UINT64_C(                   0) },
    { EASYSIMD_FLOAT64_C(   101.37),
      UINT64_C(                   0) },
    { EASYSIMD_FLOAT64_C(   763.46),
      UINT64_C(                   0) }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint64_t r = easysimd_vcltzd_f64(test_vec[i].a);
    easysimd_assert_equal_u64(r, test_vec[i].r);
  }
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint64_t r = easysimd_vcltzd_f64(a);
...
static int
test_easysimd_vcltzs_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a;
    uint32_t r;
  } test_vec[] = {
    { EASYSIMD_FLOAT32_C(   418.80),
      UINT32_C(         0) },
    { EASYSIMD_FLOAT32_C(   -38.64),
                UINT32_MAX },
    { EASYSIMD_FLOAT32_C(   835.62),
      UINT32_C(         0) },
    { EASYSIMD_FLOAT32_C(  -210.67),
                UINT32_MAX },
    { EASYSIMD_FLOAT32_C(   753.71),
      UINT32_C(         0) },
    { EASYSIMD_FLOAT32_C(   601.27),
      UINT32_C(         0) },
    { EASYSIMD_FLOAT32_C(  -439.56),
                UINT32_MAX },
    { EASYSIMD_FLOAT32_C(   192.08),
      UINT32_C(         0) }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint32_t r = easysimd_vcltzs_f32(test_vec[i].a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint32_t r = easysimd_vcltzs_f32(a);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltz_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltz_f64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltz_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltz_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltz_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltz_s64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzq_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzq_f64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzq_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzq_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzq_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzq_s64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzd_f64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzd_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcltzs_f32)
