#define EASYSIMD_TEST_ARM_NEON_INSN cnt

#include "test-neon.h"
#include "../../../easysimd/arm/neon/cnt.h"

static int
test_easysimd_vcnt_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  struct {
    int8_t a[8];
    int8_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x8_t r = easysimd_vcnt_s8(a);
...
static int
test_easysimd_vcnt_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  struct {
    uint8_t a[8];
    uint8_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x8_t r = easysimd_vcnt_u8(a);
...
static int
test_easysimd_vcntq_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  struct {
    int8_t a[16];
    int8_t r[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16_t r = easysimd_vcntq_s8(a);
...
static int
test_easysimd_vcntq_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  struct {
    uint8_t a[16];
    uint8_t r[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x16_t r = easysimd_vcntq_u8(a);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcnt_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcnt_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcntq_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vcntq_u8)
