#define EASYSIMD_TEST_ARM_NEON_INSN ld1_lane
...
static int
test_easysimd_vld1_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t src[8];
    int8_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int8x8_t r;
    easysimd_int8x8_t src = easysimd_vld1_s8(test_vec[i].src);

    EASYSIMD_CONSTIFY_8_(easysimd_vld1_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_int8x8_t expected = easysimd_vld1_s8(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i8x8(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int8x8_t src = easysimd_test_arm_neon_random_i8x8();
    easysimd_test_arm_neon_write_i8x8(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int8_t buf = easysimd_test_codegen_random_i8();
    easysimd_test_codegen_write_i8(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int8x8_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld1_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t src[4];
    int16_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x4_t src = easysimd_vld1_s16(test_vec[i].src);

    EASYSIMD_CONSTIFY_4_(easysimd_vld1_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_int16x4_t expected = easysimd_vld1_s16(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i16x4(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_int16x4_t src = easysimd_test_arm_neon_random_i16x4();
    easysimd_test_arm_neon_write_i16x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int16_t buf = easysimd_test_codegen_random_i16();
    easysimd_test_codegen_write_i16(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int16x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld1_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t src[2];
    int32_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x2_t src = easysimd_vld1_s32(test_vec[i].src);

    EASYSIMD_CONSTIFY_2_(easysimd_vld1_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_int32x2_t expected = easysimd_vld1_s32(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i32x2(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int32x2_t src = easysimd_test_arm_neon_random_i32x2();
    easysimd_test_arm_neon_write_i32x2(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int32_t buf = easysimd_test_codegen_random_i32();
    easysimd_test_codegen_write_i32(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int32x2_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld1_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t src[1];
    int64_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int64x1_t src = easysimd_vld1_s64(test_vec[i].src);
    easysimd_int64x1_t r = easysimd_vld1_lane_s64(&test_vec[i].buf, src, 0);

    easysimd_int64x1_t expected = easysimd_vld1_s64(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i64x1(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int64x1_t src = easysimd_test_arm_neon_random_i64x1();
    easysimd_test_arm_neon_write_i64x1(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int64_t buf = easysimd_test_codegen_random_i64();
    easysimd_test_codegen_write_i64(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int64x1_t r;
    r = easysimd_vld1_lane_s64(&buf, src, 0);
...
static int
test_easysimd_vld1_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t src[8];
    uint8_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x8_t src = easysimd_vld1_u8(test_vec[i].src);

    EASYSIMD_CONSTIFY_8_(easysimd_vld1_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_uint8x8_t expected = easysimd_vld1_u8(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u8x8(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint8x8_t src = easysimd_test_arm_neon_random_u8x8();
    easysimd_test_arm_neon_write_u8x8(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint8_t buf = easysimd_test_codegen_random_u8();
    easysimd_test_codegen_write_u8(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint8x8_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld1_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t src[4];
    uint16_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x4_t src = easysimd_vld1_u16(test_vec[i].src);

    EASYSIMD_CONSTIFY_4_(easysimd_vld1_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_uint16x4_t expected = easysimd_vld1_u16(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u16x4(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_uint16x4_t src = easysimd_test_arm_neon_random_u16x4();
    easysimd_test_arm_neon_write_u16x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint16_t buf = easysimd_test_codegen_random_u16();
    easysimd_test_codegen_write_u16(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint16x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld1_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t src[2];
    uint32_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x2_t src = easysimd_vld1_u32(test_vec[i].src);

    EASYSIMD_CONSTIFY_2_(easysimd_vld1_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_uint32x2_t expected = easysimd_vld1_u32(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u32x2(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_uint32x2_t src = easysimd_test_arm_neon_random_u32x2();
    easysimd_test_arm_neon_write_u32x2(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint32_t buf = easysimd_test_codegen_random_u32();
    easysimd_test_codegen_write_u32(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint32x2_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld1_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t src[1];
    uint64_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint64x1_t src = easysimd_vld1_u64(test_vec[i].src);
    easysimd_uint64x1_t r = easysimd_vld1_lane_u64(&test_vec[i].buf, src, 0);

    easysimd_uint64x1_t expected = easysimd_vld1_u64(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u64x1(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_uint64x1_t src = easysimd_test_arm_neon_random_u64x1();
    easysimd_test_arm_neon_write_u64x1(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint64_t buf = easysimd_test_codegen_random_u64();
    easysimd_test_codegen_write_u64(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint64x1_t r = easysimd_vld1_lane_u64(&buf, src, 0);
...
static int
test_easysimd_vld1_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t src[2];
    easysimd_float32_t buf;
    easysimd_float32_t r[2];
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(    -5.93), EASYSIMD_FLOAT32_C(    20.02) },
      EASYSIMD_FLOAT32_C(    20.61),
      { EASYSIMD_FLOAT32_C(    20.61), EASYSIMD_FLOAT32_C(    20.02) } },
    { { EASYSIMD_FLOAT32_C(    71.16), EASYSIMD_FLOAT32_C(   -92.70) },
      EASYSIMD_FLOAT32_C(   -43.23),
      { EASYSIMD_FLOAT32_C(    71.16), EASYSIMD_FLOAT32_C(   -43.23) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x2_t r;
    easysimd_float32x2_t src = easysimd_vld1_f32(test_vec[i].src);

    EASYSIMD_CONSTIFY_2_(easysimd_vld1_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_float32x2_t expected = easysimd_vld1_f32(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_f32x2(r, expected, 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32x2_t src = easysimd_test_arm_neon_random_f32x2(-100.0f, 100.0f);
    easysimd_test_arm_neon_write_f32x2(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float32_t buf = easysimd_test_codegen_random_f32(-100.0f, 100.0f);
    easysimd_test_codegen_write_f32(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float32x2_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld1_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t src[1];
    easysimd_float64_t buf;
    easysimd_float64_t r[1];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(    20.18) },
      EASYSIMD_FLOAT64_C(   -60.60),
      { EASYSIMD_FLOAT64_C(   -60.60) } },
    { { EASYSIMD_FLOAT64_C(   -37.50) },
      EASYSIMD_FLOAT64_C(    77.41),
      { EASYSIMD_FLOAT64_C(    77.41) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x1_t src = easysimd_vld1_f64(test_vec[i].src);
    easysimd_float64x1_t r = easysimd_vld1_lane_f64(&test_vec[i].buf, src, 0);

    easysimd_float64x1_t expected = easysimd_vld1_f64(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_f64x1(r, expected, 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64x1_t src = easysimd_test_arm_neon_random_f64x1(-100.0, 100.0);
    easysimd_test_arm_neon_write_f64x1(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float64_t buf = easysimd_test_codegen_random_f64(-100.0, 100.0);
    easysimd_test_codegen_write_f64(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float64x1_t r = easysimd_vld1_lane_f64(&buf, src, 0);
...
static int
test_easysimd_vld1q_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t src[16];
    int8_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16_t src = easysimd_vld1q_s8(test_vec[i].src);

    EASYSIMD_CONSTIFY_16_(easysimd_vld1q_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_int8x16_t expected = easysimd_vld1q_s8(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i8x16(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 16 ; i++) {
    easysimd_int8x16_t src = easysimd_test_arm_neon_random_i8x16();
    easysimd_test_arm_neon_write_i8x16(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int8_t buf = easysimd_test_codegen_random_i8();
    easysimd_test_codegen_write_i8(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int8x16_t r;
    EASYSIMD_CONSTIFY_16_(easysimd_vld1q_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t src[8];
    int16_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8_t src = easysimd_vld1q_s16(test_vec[i].src);

    EASYSIMD_CONSTIFY_8_(easysimd_vld1q_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_int16x8_t expected = easysimd_vld1q_s16(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i16x8(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int16x8_t src = easysimd_test_arm_neon_random_i16x8();
    easysimd_test_arm_neon_write_i16x8(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int16_t buf = easysimd_test_codegen_random_i16();
    easysimd_test_codegen_write_i16(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int16x8_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld1q_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t src[4];
    int32_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t src = easysimd_vld1q_s32(test_vec[i].src);

    EASYSIMD_CONSTIFY_4_(easysimd_vld1q_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_int32x4_t expected = easysimd_vld1q_s32(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i32x4(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_int32x4_t src = easysimd_test_arm_neon_random_i32x4();
    easysimd_test_arm_neon_write_i32x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int32_t buf = easysimd_test_codegen_random_i32();
    easysimd_test_codegen_write_i32(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int32x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld1q_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t src[2];
    int64_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int64x2_t src = easysimd_vld1q_s64(test_vec[i].src);
    easysimd_int64x2_t r = easysimd_vld1q_lane_s64(&test_vec[i].buf, src, 0);

    easysimd_int64x2_t expected = easysimd_vld1q_s64(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_i64x2(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int64x2_t src = easysimd_test_arm_neon_random_i64x2();
    easysimd_test_arm_neon_write_i64x2(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int64_t buf = easysimd_test_codegen_random_i64();
    easysimd_test_codegen_write_i64(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int64x2_t r;
    r = easysimd_vld1q_lane_s64(&buf, src, 0);
...
static int
test_easysimd_vld1q_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t src[16];
    uint8_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x16_t src = easysimd_vld1q_u8(test_vec[i].src);

    EASYSIMD_CONSTIFY_16_(easysimd_vld1q_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_uint8x16_t expected = easysimd_vld1q_u8(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u8x16(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 16 ; i++) {
    easysimd_uint8x16_t src = easysimd_test_arm_neon_random_u8x16();
    easysimd_test_arm_neon_write_u8x16(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint8_t buf = easysimd_test_codegen_random_u8();
    easysimd_test_codegen_write_u8(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint8x16_t r;
    EASYSIMD_CONSTIFY_16_(easysimd_vld1q_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t src[8];
    uint16_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x8_t src = easysimd_vld1q_u16(test_vec[i].src);

    EASYSIMD_CONSTIFY_8_(easysimd_vld1q_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_uint16x8_t expected = easysimd_vld1q_u16(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u16x8(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint16x8_t src = easysimd_test_arm_neon_random_u16x8();
    easysimd_test_arm_neon_write_u16x8(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint16_t buf = easysimd_test_codegen_random_u16();
    easysimd_test_codegen_write_u16(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint16x8_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld1q_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t src[4];
    uint32_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x4_t src = easysimd_vld1q_u32(test_vec[i].src);

    EASYSIMD_CONSTIFY_4_(easysimd_vld1q_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_uint32x4_t expected = easysimd_vld1q_u32(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u32x4(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_uint32x4_t src = easysimd_test_arm_neon_random_u32x4();
    easysimd_test_arm_neon_write_u32x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint32_t buf = easysimd_test_codegen_random_u32();
    easysimd_test_codegen_write_u32(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint32x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld1q_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t src[2];
    uint64_t buf;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint64x2_t src = easysimd_vld1q_u64(test_vec[i].src);
    easysimd_uint64x2_t r = easysimd_vld1q_lane_u64(&test_vec[i].buf, src, 0);

    easysimd_uint64x2_t expected = easysimd_vld1q_u64(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_u64x2(r, expected);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_uint64x2_t src = easysimd_test_arm_neon_random_u64x2();
    easysimd_test_arm_neon_write_u64x2(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint64_t buf = easysimd_test_codegen_random_u64();
    easysimd_test_codegen_write_u64(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint64x2_t r = easysimd_vld1q_lane_u64(&buf, src, 0);
...
static int
test_easysimd_vld1q_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t src[4];
    easysimd_float32_t buf;
    easysimd_float32_t r[4];
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(    66.78), EASYSIMD_FLOAT32_C(   -23.96), EASYSIMD_FLOAT32_C(    23.06), EASYSIMD_FLOAT32_C(    55.69) },
      EASYSIMD_FLOAT32_C(   -28.53),
      { EASYSIMD_FLOAT32_C(   -28.53), EASYSIMD_FLOAT32_C(   -23.96), EASYSIMD_FLOAT32_C(    23.06), EASYSIMD_FLOAT32_C(    55.69) } },
    { { EASYSIMD_FLOAT32_C(   -99.61), EASYSIMD_FLOAT32_C(   -58.57), EASYSIMD_FLOAT32_C(   -81.59), EASYSIMD_FLOAT32_C(   -91.49) },
      EASYSIMD_FLOAT32_C(    47.16),
      { EASYSIMD_FLOAT32_C(   -99.61), EASYSIMD_FLOAT32_C(    47.16), EASYSIMD_FLOAT32_C(   -81.59), EASYSIMD_FLOAT32_C(   -91.49) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x4_t r;
    easysimd_float32x4_t src = easysimd_vld1q_f32(test_vec[i].src);

    EASYSIMD_CONSTIFY_2_(easysimd_vld1q_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, &test_vec[i].buf, src);

    easysimd_float32x4_t expected = easysimd_vld1q_f32(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_f32x4(r, expected, 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32x4_t src = easysimd_test_arm_neon_random_f32x4(-100.0f, 100.0f);
    easysimd_test_arm_neon_write_f32x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float32_t buf = easysimd_test_codegen_random_f32(-100.0f, 100.0f);
    easysimd_test_codegen_write_f32(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float32x4_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld1q_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, &buf, src);
...
static int
test_easysimd_vld1q_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t src[2];
    easysimd_float64_t buf;
    easysimd_float64_t r[2];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(    46.67), EASYSIMD_FLOAT64_C(   -34.89) },
      EASYSIMD_FLOAT64_C(   -24.31),
      { EASYSIMD_FLOAT64_C(   -24.31), EASYSIMD_FLOAT64_C(   -34.89) } },
    { { EASYSIMD_FLOAT64_C(    43.52), EASYSIMD_FLOAT64_C(   -35.64) },
      EASYSIMD_FLOAT64_C(   -28.94),
      { EASYSIMD_FLOAT64_C(   -28.94), EASYSIMD_FLOAT64_C(   -35.64) } }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x2_t src = easysimd_vld1q_f64(test_vec[i].src);
    easysimd_float64x2_t r = easysimd_vld1q_lane_f64(&test_vec[i].buf, src, 0);

    easysimd_float64x2_t expected = easysimd_vld1q_f64(test_vec[i].r);

    easysimd_test_arm_neon_assert_equal_f64x2(r, expected, 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64x2_t src = easysimd_test_arm_neon_random_f64x2(-100.0, 100.0);
    easysimd_test_arm_neon_write_f64x2(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float64_t buf = easysimd_test_codegen_random_f64(-100.0, 100.0);
    easysimd_test_codegen_write_f64(2, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float64x2_t r = easysimd_vld1q_lane_f64(&buf, src, 0);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1_lane_f64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld1q_lane_f64)
