#define EASYSIMD_TEST_ARM_NEON_INSN ld2

#include "test-neon.h"
#include "../../../easysimd/arm/neon/ld2.h"

#if !defined(EASYSIMD_BUG_INTEL_857088)

static int
test_easysimd_vld2_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[8];
    int8_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int8x8x2_t r = easysimd_vld2_s8(test_vec[i].a);

    easysimd_int8x8x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int8_t buf[16];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int8x8x2_t r = easysimd_vld2_s8(buf);

    easysimd_test_arm_neon_write_i8x8x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int16x4x2_t r = easysimd_vld2_s16(test_vec[i].a);

    easysimd_int16x4x2_t expected = {
  ...
  for (int i = 0 ; i < 4 ; i++) {
    ...
    int16_t buf[8];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int16x4x2_t r = easysimd_vld2_s16(buf);

    easysimd_test_arm_neon_write_i16x4x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int32x2x2_t r = easysimd_vld2_s32(test_vec[i].a);

    easysimd_int32x2x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int32_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int32x2x2_t r = easysimd_vld2_s32(buf);

    easysimd_test_arm_neon_write_i32x2x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[1];
    int64_t b[1];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int64x1x2_t r = easysimd_vld2_s64(test_vec[i].a);

    easysimd_int64x1x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int64_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int64x1x2_t r = easysimd_vld2_s64(buf);

    easysimd_test_arm_neon_write_i64x1x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[8];
    uint8_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint8x8x2_t r = easysimd_vld2_u8(test_vec[i].a);

    easysimd_uint8x8x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint8_t buf[16];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint8x8x2_t r = easysimd_vld2_u8(buf);

    easysimd_test_arm_neon_write_u8x8x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[4];
    uint16_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint16x4x2_t r = easysimd_vld2_u16(test_vec[i].a);

    easysimd_uint16x4x2_t expected = {
  ...
  for (int i = 0 ; i < 4 ; i++) {
    ...
    uint16_t buf[8];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint16x4x2_t r = easysimd_vld2_u16(buf);

    easysimd_test_arm_neon_write_u16x4x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[2];
    uint32_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint32x2x2_t r = easysimd_vld2_u32(test_vec[i].a);

    easysimd_uint32x2x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint32_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint32x2x2_t r = easysimd_vld2_u32(buf);

    easysimd_test_arm_neon_write_u32x2x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[1];
    uint64_t b[1];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint64x1x2_t r = easysimd_vld2_u64(test_vec[i].a);

    easysimd_uint64x1x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint64_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint64x1x2_t r = easysimd_vld2_u64(buf);

    easysimd_test_arm_neon_write_u64x1x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[2];
    easysimd_float32_t b[2];
    easysimd_float32_t r[2][2];
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(   -22.58), EASYSIMD_FLOAT32_C(   -87.59) },
      { EASYSIMD_FLOAT32_C(     1.12), EASYSIMD_FLOAT32_C(   -68.18) },
        { { EASYSIMD_FLOAT32_C(   -22.58), EASYSIMD_FLOAT32_C(     1.12) },
          { EASYSIMD_FLOAT32_C(   -87.59), EASYSIMD_FLOAT32_C(   -68.18) } },
    },
    { { EASYSIMD_FLOAT32_C(   -19.64), EASYSIMD_FLOAT32_C(   -79.11) },
      { EASYSIMD_FLOAT32_C(   -13.56), EASYSIMD_FLOAT32_C(    59.24) },
        { { EASYSIMD_FLOAT32_C(   -19.64), EASYSIMD_FLOAT32_C(   -13.56) },
          { EASYSIMD_FLOAT32_C(   -79.11), EASYSIMD_FLOAT32_C(    59.24) } },
    },
    { { EASYSIMD_FLOAT32_C(    84.01), EASYSIMD_FLOAT32_C(   -49.79) },
      { EASYSIMD_FLOAT32_C(    69.20), EASYSIMD_FLOAT32_C(    82.05) },
        { { EASYSIMD_FLOAT32_C(    84.01), EASYSIMD_FLOAT32_C(    69.20) },
          { EASYSIMD_FLOAT32_C(   -49.79), EASYSIMD_FLOAT32_C(    82.05) } },
    },
    { { EASYSIMD_FLOAT32_C(   -12.11), EASYSIMD_FLOAT32_C(    93.29) },
      { EASYSIMD_FLOAT32_C(    70.29), EASYSIMD_FLOAT32_C(    70.67) },
        { { EASYSIMD_FLOAT32_C(   -12.11), EASYSIMD_FLOAT32_C(    70.29) },
          { EASYSIMD_FLOAT32_C(    93.29), EASYSIMD_FLOAT32_C(    70.67) } },
    },
    { { EASYSIMD_FLOAT32_C(    14.62), EASYSIMD_FLOAT32_C(    51.53) },
      { EASYSIMD_FLOAT32_C(    11.77), EASYSIMD_FLOAT32_C(    81.69) },
        { { EASYSIMD_FLOAT32_C(    14.62), EASYSIMD_FLOAT32_C(    11.77) },
          { EASYSIMD_FLOAT32_C(    51.53), EASYSIMD_FLOAT32_C(    81.69) } },
    },
    { { EASYSIMD_FLOAT32_C(   -19.28), EASYSIMD_FLOAT32_C(    59.50) },
      { EASYSIMD_FLOAT32_C(   -77.17), EASYSIMD_FLOAT32_C(   -41.73) },
        { { EASYSIMD_FLOAT32_C(   -19.28), EASYSIMD_FLOAT32_C(   -77.17) },
          { EASYSIMD_FLOAT32_C(    59.50), EASYSIMD_FLOAT32_C(   -41.73) } },
    },
    { { EASYSIMD_FLOAT32_C(   -85.96), EASYSIMD_FLOAT32_C(    68.07) },
      { EASYSIMD_FLOAT32_C(   -11.65), EASYSIMD_FLOAT32_C(    31.99) },
        { { EASYSIMD_FLOAT32_C(   -85.96), EASYSIMD_FLOAT32_C(   -11.65) },
          { EASYSIMD_FLOAT32_C(    68.07), EASYSIMD_FLOAT32_C(    31.99) } },
    },
    { { EASYSIMD_FLOAT32_C(   -11.72), EASYSIMD_FLOAT32_C(   -86.96) },
      { EASYSIMD_FLOAT32_C(    52.84), EASYSIMD_FLOAT32_C(    65.70) },
        { { EASYSIMD_FLOAT32_C(   -11.72), EASYSIMD_FLOAT32_C(    52.84) },
          { EASYSIMD_FLOAT32_C(   -86.96), EASYSIMD_FLOAT32_C(    65.70) } }
    },
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x2x2_t r = easysimd_vld2_f32(test_vec[i].a);

    easysimd_float32x2x2_t expected = {
...
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_float32x2_t a = easysimd_test_arm_neon_random_f32x2(-100.0f, 100.0f);
    easysimd_float32x2_t b = easysimd_test_arm_neon_random_f32x2(-100.0f, 100.0f);
    easysimd_float32x2x2_t c = {{a, b}};
    ...
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_float32x2x2_t r = easysimd_vld2_f32(buf);

    easysimd_test_arm_neon_write_f32x2x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[1];
    easysimd_float64_t b[1];
    easysimd_float64_t r[2][1];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(   -60.52) },
      { EASYSIMD_FLOAT64_C(   -27.97) },
        { { EASYSIMD_FLOAT64_C(   -60.52) },
          { EASYSIMD_FLOAT64_C(   -27.97) } },
    },
    { { EASYSIMD_FLOAT64_C(    41.23) },
      { EASYSIMD_FLOAT64_C(   -82.15) },
        { { EASYSIMD_FLOAT64_C(    41.23) },
          { EASYSIMD_FLOAT64_C(   -82.15) } },
    },
    { { EASYSIMD_FLOAT64_C(   -45.22) },
      { EASYSIMD_FLOAT64_C(   -82.20) },
        { { EASYSIMD_FLOAT64_C(   -45.22) },
          { EASYSIMD_FLOAT64_C(   -82.20) } },
    },
    { { EASYSIMD_FLOAT64_C(    20.47) },
      { EASYSIMD_FLOAT64_C(   -91.57) },
        { { EASYSIMD_FLOAT64_C(    20.47) },
          { EASYSIMD_FLOAT64_C(   -91.57) } },
    },
    { { EASYSIMD_FLOAT64_C(    87.79) },
      { EASYSIMD_FLOAT64_C(    27.03) },
        { { EASYSIMD_FLOAT64_C(    87.79) },
          { EASYSIMD_FLOAT64_C(    27.03) } },
    },
    { { EASYSIMD_FLOAT64_C(   -13.17) },
      { EASYSIMD_FLOAT64_C(    89.71) },
        { { EASYSIMD_FLOAT64_C(   -13.17) },
          { EASYSIMD_FLOAT64_C(    89.71) } },
    },
    { { EASYSIMD_FLOAT64_C(   -91.04) },
      { EASYSIMD_FLOAT64_C(    54.16) },
        { { EASYSIMD_FLOAT64_C(   -91.04) },
          { EASYSIMD_FLOAT64_C(    54.16) } },
    },
    { { EASYSIMD_FLOAT64_C(    49.63) },
      { EASYSIMD_FLOAT64_C(    71.75) },
        { { EASYSIMD_FLOAT64_C(    49.63) },
          { EASYSIMD_FLOAT64_C(    71.75) } },
    }
  };
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x1x2_t r = easysimd_vld2_f64(test_vec[i].a);

    easysimd_float64x1x2_t expected = {
...
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_float64x1_t a = easysimd_test_arm_neon_random_f64x1(-100.0, 100.0);
    easysimd_float64x1_t b = easysimd_test_arm_neon_random_f64x1(-100.0, 100.0);
    easysimd_float64x1x2_t c = {{a, b}};
    ...
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_float64x1x2_t r = easysimd_vld2_f64(buf);

    easysimd_test_arm_neon_write_f64x1x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[16];
    int8_t b[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int8x16x2_t r = easysimd_vld2q_s8(test_vec[i].a);

    easysimd_int8x16x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int8_t buf[32];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int8x16x2_t r = easysimd_vld2q_s8(buf);

    easysimd_test_arm_neon_write_i8x16x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[8];
    int16_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int16x8x2_t r = easysimd_vld2q_s16(test_vec[i].a);

    easysimd_int16x8x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int16_t buf[16];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int16x8x2_t r = easysimd_vld2q_s16(buf);

    easysimd_test_arm_neon_write_i16x8x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    int32_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int32x4x2_t r = easysimd_vld2q_s32(test_vec[i].a);

    easysimd_int32x4x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int32_t buf[8];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int32x4x2_t r = easysimd_vld2q_s32(buf);

    easysimd_test_arm_neon_write_i32x4x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[2];
    int64_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int64x2x2_t r = easysimd_vld2q_s64(test_vec[i].a);

    easysimd_int64x2x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    int64_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_int64x2x2_t r = easysimd_vld2q_s64(buf);

    easysimd_test_arm_neon_write_i64x2x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[16];
    uint8_t b[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint8x16x2_t r = easysimd_vld2q_u8(test_vec[i].a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint8_t buf[32];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint8x16x2_t r = easysimd_vld2q_u8(buf);

    easysimd_test_arm_neon_write_u8x16x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[8];
    uint16_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint16x8x2_t r = easysimd_vld2q_u16(test_vec[i].a);

    easysimd_uint16x8x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint16_t buf[16];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint16x8x2_t r = easysimd_vld2q_u16(buf);

    easysimd_test_arm_neon_write_u16x8x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[4];
    uint32_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint32x4x2_t r = easysimd_vld2q_u32(test_vec[i].a);

    easysimd_uint32x4x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint32_t buf[8];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint32x4x2_t r = easysimd_vld2q_u32(buf);

    easysimd_test_arm_neon_write_u32x4x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[2];
    uint64_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint64x2x2_t r = easysimd_vld2q_u64(test_vec[i].a);

    easysimd_uint64x2x2_t expected = {
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    uint64_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_uint64x2x2_t r = easysimd_vld2q_u64(buf);

    easysimd_test_arm_neon_write_u64x2x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[4];
    easysimd_float32_t b[4];
    easysimd_float32_t r[2][4];
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(   793.71), EASYSIMD_FLOAT32_C(   221.62), EASYSIMD_FLOAT32_C(   781.57), EASYSIMD_FLOAT32_C(  -525.52) },
      { EASYSIMD_FLOAT32_C(   800.99), EASYSIMD_FLOAT32_C(  -411.87), EASYSIMD_FLOAT32_C(   483.99), EASYSIMD_FLOAT32_C(   852.73) },
        { { EASYSIMD_FLOAT32_C(   793.71), EASYSIMD_FLOAT32_C(   781.57), EASYSIMD_FLOAT32_C(   800.99), EASYSIMD_FLOAT32_C(   483.99) },
          { EASYSIMD_FLOAT32_C(   221.62), EASYSIMD_FLOAT32_C(  -525.52), EASYSIMD_FLOAT32_C(  -411.87), EASYSIMD_FLOAT32_C(   852.73) } },
    },
    { { EASYSIMD_FLOAT32_C(  -433.25), EASYSIMD_FLOAT32_C(  -811.76), EASYSIMD_FLOAT32_C(  -463.04), EASYSIMD_FLOAT32_C(   671.94) },
      { EASYSIMD_FLOAT32_C(   -60.94), EASYSIMD_FLOAT32_C(  -434.20), EASYSIMD_FLOAT32_C(  -263.89), EASYSIMD_FLOAT32_C(   754.63) },
        { { EASYSIMD_FLOAT32_C(  -433.25), EASYSIMD_FLOAT32_C(  -463.04), EASYSIMD_FLOAT32_C(   -60.94), EASYSIMD_FLOAT32_C(  -263.89) },
          { EASYSIMD_FLOAT32_C(  -811.76), EASYSIMD_FLOAT32_C(   671.94), EASYSIMD_FLOAT32_C(  -434.20), EASYSIMD_FLOAT32_C(   754.63) } },
    },
    { { EASYSIMD_FLOAT32_C(  -877.85), EASYSIMD_FLOAT32_C(  -225.98), EASYSIMD_FLOAT32_C(  -292.03), EASYSIMD_FLOAT32_C(   932.32) },
      { EASYSIMD_FLOAT32_C(  -463.10), EASYSIMD_FLOAT32_C(   171.18), EASYSIMD_FLOAT32_C(  -115.23), EASYSIMD_FLOAT32_C(  -867.52) },
        { { EASYSIMD_FLOAT32_C(  -877.85), EASYSIMD_FLOAT32_C(  -292.03), EASYSIMD_FLOAT32_C(  -463.10), EASYSIMD_FLOAT32_C(  -115.23) },
          { EASYSIMD_FLOAT32_C(  -225.98), EASYSIMD_FLOAT32_C(   932.32), EASYSIMD_FLOAT32_C(   171.18), EASYSIMD_FLOAT32_C(  -867.52) } },
    },
    { { EASYSIMD_FLOAT32_C(  -182.14), EASYSIMD_FLOAT32_C(   874.57), EASYSIMD_FLOAT32_C(  -306.44), EASYSIMD_FLOAT32_C(   180.87) },
      { EASYSIMD_FLOAT32_C(  -827.59), EASYSIMD_FLOAT32_C(  -449.53), EASYSIMD_FLOAT32_C(  -593.39), EASYSIMD_FLOAT32_C(   966.11) },
        { { EASYSIMD_FLOAT32_C(  -182.14), EASYSIMD_FLOAT32_C(  -306.44), EASYSIMD_FLOAT32_C(  -827.59), EASYSIMD_FLOAT32_C(  -593.39) },
          { EASYSIMD_FLOAT32_C(   874.57), EASYSIMD_FLOAT32_C(   180.87), EASYSIMD_FLOAT32_C(  -449.53), EASYSIMD_FLOAT32_C(   966.11) } },
    },
    { { EASYSIMD_FLOAT32_C(   772.09), EASYSIMD_FLOAT32_C(  -811.82), EASYSIMD_FLOAT32_C(  -559.40), EASYSIMD_FLOAT32_C(   573.08) },
      { EASYSIMD_FLOAT32_C(  -223.69), EASYSIMD_FLOAT32_C(   924.59), EASYSIMD_FLOAT32_C(   425.81), EASYSIMD_FLOAT32_C(   343.06) },
        { { EASYSIMD_FLOAT32_C(   772.09), EASYSIMD_FLOAT32_C(  -559.40), EASYSIMD_FLOAT32_C(  -223.69), EASYSIMD_FLOAT32_C(   425.81) },
          { EASYSIMD_FLOAT32_C(  -811.82), EASYSIMD_FLOAT32_C(   573.08), EASYSIMD_FLOAT32_C(   924.59), EASYSIMD_FLOAT32_C(   343.06) } },
    },
    { { EASYSIMD_FLOAT32_C(  -887.17), EASYSIMD_FLOAT32_C(   962.76), EASYSIMD_FLOAT32_C(    15.01), EASYSIMD_FLOAT32_C(    51.89) },
      { EASYSIMD_FLOAT32_C(  -471.44), EASYSIMD_FLOAT32_C(   751.11), EASYSIMD_FLOAT32_C(  -193.48), EASYSIMD_FLOAT32_C(  -349.29) },
        { { EASYSIMD_FLOAT32_C(  -887.17), EASYSIMD_FLOAT32_C(    15.01), EASYSIMD_FLOAT32_C(  -471.44), EASYSIMD_FLOAT32_C(  -193.48) },
          { EASYSIMD_FLOAT32_C(   962.76), EASYSIMD_FLOAT32_C(    51.89), EASYSIMD_FLOAT32_C(   751.11), EASYSIMD_FLOAT32_C(  -349.29) } },
    },
    { { EASYSIMD_FLOAT32_C(  -474.87), EASYSIMD_FLOAT32_C(   514.49), EASYSIMD_FLOAT32_C(  -416.97), EASYSIMD_FLOAT32_C(    62.03) },
      { EASYSIMD_FLOAT32_C(  -314.33), EASYSIMD_FLOAT32_C(   467.80), EASYSIMD_FLOAT32_C(   194.51), EASYSIMD_FLOAT32_C(   503.53) },
        { { EASYSIMD_FLOAT32_C(  -474.87), EASYSIMD_FLOAT32_C(  -416.97), EASYSIMD_FLOAT32_C(  -314.33), EASYSIMD_FLOAT32_C(   194.51) },
          { EASYSIMD_FLOAT32_C(   514.49), EASYSIMD_FLOAT32_C(    62.03), EASYSIMD_FLOAT32_C(   467.80), EASYSIMD_FLOAT32_C(   503.53) } },
    },
    { { EASYSIMD_FLOAT32_C(   342.37), EASYSIMD_FLOAT32_C(   888.07), EASYSIMD_FLOAT32_C(  -315.60), EASYSIMD_FLOAT32_C(   514.77) },
      { EASYSIMD_FLOAT32_C(  -561.46), EASYSIMD_FLOAT32_C(    91.01), EASYSIMD_FLOAT32_C(   480.89), EASYSIMD_FLOAT32_C(  -789.37) },
        { { EASYSIMD_FLOAT32_C(   342.37), EASYSIMD_FLOAT32_C(  -315.60), EASYSIMD_FLOAT32_C(  -561.46), EASYSIMD_FLOAT32_C(   480.89) },
          { EASYSIMD_FLOAT32_C(   888.07), EASYSIMD_FLOAT32_C(   514.77), EASYSIMD_FLOAT32_C(    91.01), EASYSIMD_FLOAT32_C(  -789.37) } },
    },
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x4x2_t r = easysimd_vld2q_f32(test_vec[i].a);

    easysimd_float32x4x2_t expected = {
        {easysimd_vld1q_f32(test_vec[i].r[0]), easysimd_vld1q_f32(test_vec[i].r[1])}};

    easysimd_test_arm_neon_assert_equal_f32x4(r.val[0], expected.val[0], INT_MAX);
    easysimd_test_arm_neon_assert_equal_f32x4(r.val[1], expected.val[1], INT_MAX);
  }
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_float32x4x2_t r = easysimd_vld2q_f32(buf);

    easysimd_test_arm_neon_write_f32x4x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld2q_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[2];
    easysimd_float64_t b[2];
    easysimd_float64_t r[2][2];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(   -81.12), EASYSIMD_FLOAT64_C(   -90.46) },
      { EASYSIMD_FLOAT64_C(   -83.90), EASYSIMD_FLOAT64_C(    20.75) },
        { { EASYSIMD_FLOAT64_C(   -81.12), EASYSIMD_FLOAT64_C(   -83.90) },
          { EASYSIMD_FLOAT64_C(   -90.46), EASYSIMD_FLOAT64_C(    20.75) } },
    },
    { { EASYSIMD_FLOAT64_C(   -91.92), EASYSIMD_FLOAT64_C(     5.15) },
      { EASYSIMD_FLOAT64_C(   -58.53), EASYSIMD_FLOAT64_C(   -40.61) },
        { { EASYSIMD_FLOAT64_C(   -91.92), EASYSIMD_FLOAT64_C(   -58.53) },
          { EASYSIMD_FLOAT64_C(     5.15), EASYSIMD_FLOAT64_C(   -40.61) } },
    },
    { { EASYSIMD_FLOAT64_C(    92.56), EASYSIMD_FLOAT64_C(    91.44) },
      { EASYSIMD_FLOAT64_C(    67.84), EASYSIMD_FLOAT64_C(   -58.14) },
        { { EASYSIMD_FLOAT64_C(    92.56), EASYSIMD_FLOAT64_C(    67.84) },
          { EASYSIMD_FLOAT64_C(    91.44), EASYSIMD_FLOAT64_C(   -58.14) } },
    },
    { { EASYSIMD_FLOAT64_C(    63.47), EASYSIMD_FLOAT64_C(    42.43) },
      { EASYSIMD_FLOAT64_C(    29.58), EASYSIMD_FLOAT64_C(     4.03) },
        { { EASYSIMD_FLOAT64_C(    63.47), EASYSIMD_FLOAT64_C(    29.58) },
          { EASYSIMD_FLOAT64_C(    42.43), EASYSIMD_FLOAT64_C(     4.03) } },
    },
    { { EASYSIMD_FLOAT64_C(     0.29), EASYSIMD_FLOAT64_C(   -69.19) },
      { EASYSIMD_FLOAT64_C(   -73.56), EASYSIMD_FLOAT64_C(   -97.91) },
        { { EASYSIMD_FLOAT64_C(     0.29), EASYSIMD_FLOAT64_C(   -73.56) },
          { EASYSIMD_FLOAT64_C(   -69.19), EASYSIMD_FLOAT64_C(   -97.91) } },
    },
    { { EASYSIMD_FLOAT64_C(   -62.67), EASYSIMD_FLOAT64_C(   -66.03) },
      { EASYSIMD_FLOAT64_C(   -17.85), EASYSIMD_FLOAT64_C(   -62.88) },
        { { EASYSIMD_FLOAT64_C(   -62.67), EASYSIMD_FLOAT64_C(   -17.85) },
          { EASYSIMD_FLOAT64_C(   -66.03), EASYSIMD_FLOAT64_C(   -62.88) } },
    },
    { { EASYSIMD_FLOAT64_C(   -24.49), EASYSIMD_FLOAT64_C(   -93.09) },
      { EASYSIMD_FLOAT64_C(    12.23), EASYSIMD_FLOAT64_C(    80.71) },
        { { EASYSIMD_FLOAT64_C(   -24.49), EASYSIMD_FLOAT64_C(    12.23) },
          { EASYSIMD_FLOAT64_C(   -93.09), EASYSIMD_FLOAT64_C(    80.71) } },
    },
    { { EASYSIMD_FLOAT64_C(   -90.54), EASYSIMD_FLOAT64_C(    20.34) },
      { EASYSIMD_FLOAT64_C(   -11.66), EASYSIMD_FLOAT64_C(   -71.66) },
        { { EASYSIMD_FLOAT64_C(   -90.54), EASYSIMD_FLOAT64_C(   -11.66) },
          { EASYSIMD_FLOAT64_C(    20.34), EASYSIMD_FLOAT64_C(   -71.66) } },
    }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x2x2_t r = easysimd_vld2q_f64(test_vec[i].a);

    easysimd_float64x2x2_t expected = {
...
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_float64x2_t a = easysimd_test_arm_neon_random_f64x2(-100.0, 100.0);
    easysimd_float64x2_t b = easysimd_test_arm_neon_random_f64x2(-100.0, 100.0);
    easysimd_float64x2x2_t c = {{a, b}};
    ...
    easysimd_float64_t buf[4];
    easysimd_memcpy(buf, c.val, sizeof(buf));
    easysimd_float64x2x2_t r = easysimd_vld2q_f64(buf);

    easysimd_test_arm_neon_write_f64x2x2(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
#if !defined(EASYSIMD_BUG_INTEL_857088)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2_f64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld2q_f64)
#endif /* !defined(EASYSIMD_BUG_INTEL_857088) */
