#define EASYSIMD_TEST_ARM_NEON_INSN ld4_lane

#include "test-neon.h"
#include "../../../easysimd/arm/neon/ld4_lane.h"
#include "../../../easysimd/arm/neon/ld1.h"

#if !defined(EASYSIMD_BUG_INTEL_857088)

static int
test_easysimd_vld4_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t src[4][8];
    int8_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x8x4_t src = {
        {easysimd_vld1_s8(test_vec[i].src[0]), easysimd_vld1_s8(test_vec[i].src[1]),
         easysimd_vld1_s8(test_vec[i].src[2]), easysimd_vld1_s8(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_8_(easysimd_vld4_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_int8x8x4_t expected = {
        {easysimd_vld1_s8(test_vec[i].r[0]), easysimd_vld1_s8(test_vec[i].r[1]),
         easysimd_vld1_s8(test_vec[i].r[2]), easysimd_vld1_s8(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i8x8(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i8x8(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i8x8(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i8x8(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int8x8x4_t src = easysimd_test_arm_neon_random_i8x8x4();
    easysimd_test_arm_neon_write_i8x8x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int8_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi8(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int8x8x4_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld4_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_i8x8x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t src[4][4];
    int16_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x4x4_t src = {
        {easysimd_vld1_s16(test_vec[i].src[0]), easysimd_vld1_s16(test_vec[i].src[1]),
         easysimd_vld1_s16(test_vec[i].src[2]), easysimd_vld1_s16(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_4_(easysimd_vld4_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_int16x4x4_t expected = {
        {easysimd_vld1_s16(test_vec[i].r[0]), easysimd_vld1_s16(test_vec[i].r[1]),
         easysimd_vld1_s16(test_vec[i].r[2]), easysimd_vld1_s16(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i16x4(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i16x4(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i16x4(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i16x4(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_int16x4x4_t src = easysimd_test_arm_neon_random_i16x4x4();
    easysimd_test_arm_neon_write_i16x4x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int16_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi16(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int16x4x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld4_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_i16x4x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t src[4][8];
    int32_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x2x4_t src = {
        {easysimd_vld1_s32(test_vec[i].src[0]), easysimd_vld1_s32(test_vec[i].src[1]),
         easysimd_vld1_s32(test_vec[i].src[2]), easysimd_vld1_s32(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_2_(easysimd_vld4_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_int32x2x4_t expected = {
        {easysimd_vld1_s32(test_vec[i].r[0]), easysimd_vld1_s32(test_vec[i].r[1]),
         easysimd_vld1_s32(test_vec[i].r[2]), easysimd_vld1_s32(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i32x2(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i32x2(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i32x2(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i32x2(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int32x2x4_t src = easysimd_test_arm_neon_random_i32x2x4();
    easysimd_test_arm_neon_write_i32x2x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int32_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi32(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int32x2x4_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld4_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_i32x2x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t src[4][8];
    int64_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int64x1x4_t src = {
        {easysimd_vld1_s64(test_vec[i].src[0]), easysimd_vld1_s64(test_vec[i].src[1]),
         easysimd_vld1_s64(test_vec[i].src[2]), easysimd_vld1_s64(test_vec[i].src[3])}};
    easysimd_int64x1x4_t r = easysimd_vld4_lane_s64(test_vec[i].buf, src, 0);

    easysimd_int64x1x4_t expected = {
        {easysimd_vld1_s64(test_vec[i].r[0]), easysimd_vld1_s64(test_vec[i].r[1]),
         easysimd_vld1_s64(test_vec[i].r[2]), easysimd_vld1_s64(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i64x1(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i64x1(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i64x1(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i64x1(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int64x1x4_t src = easysimd_test_arm_neon_random_i64x1x4();
    easysimd_test_arm_neon_write_i64x1x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int64_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi64(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int64x1x4_t r = easysimd_vld4_lane_s64(buf, src, 0);
    easysimd_test_arm_neon_write_i64x1x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t src[4][8];
    uint8_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x8x4_t src = {
        {easysimd_vld1_u8(test_vec[i].src[0]), easysimd_vld1_u8(test_vec[i].src[1]),
         easysimd_vld1_u8(test_vec[i].src[2]), easysimd_vld1_u8(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_8_(easysimd_vld4_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_uint8x8x4_t expected = {
        {easysimd_vld1_u8(test_vec[i].r[0]), easysimd_vld1_u8(test_vec[i].r[1]),
         easysimd_vld1_u8(test_vec[i].r[2]), easysimd_vld1_u8(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u8x8(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u8x8(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u8x8(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u8x8(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint8x8x4_t src = easysimd_test_arm_neon_random_u8x8x4();
    easysimd_test_arm_neon_write_u8x8x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint8_t buf[4];
    easysimd_test_codegen_random_memory(4, buf);
    easysimd_test_codegen_write_vu8(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint8x8x4_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld4_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_u8x8x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t src[4][4];
    uint16_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x4x4_t src = {
        {easysimd_vld1_u16(test_vec[i].src[0]), easysimd_vld1_u16(test_vec[i].src[1]),
         easysimd_vld1_u16(test_vec[i].src[2]), easysimd_vld1_u16(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_4_(easysimd_vld4_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_uint16x4x4_t expected = {
        {easysimd_vld1_u16(test_vec[i].r[0]), easysimd_vld1_u16(test_vec[i].r[1]),
         easysimd_vld1_u16(test_vec[i].r[2]), easysimd_vld1_u16(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u16x4(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u16x4(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u16x4(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u16x4(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_uint16x4x4_t src = easysimd_test_arm_neon_random_u16x4x4();
    easysimd_test_arm_neon_write_u16x4x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint16_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vu16(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint16x4x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld4_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_u16x4x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t src[4][8];
    uint32_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x2x4_t src = {
        {easysimd_vld1_u32(test_vec[i].src[0]), easysimd_vld1_u32(test_vec[i].src[1]),
         easysimd_vld1_u32(test_vec[i].src[2]), easysimd_vld1_u32(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_2_(easysimd_vld4_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_uint32x2x4_t expected = {
        {easysimd_vld1_u32(test_vec[i].r[0]), easysimd_vld1_u32(test_vec[i].r[1]),
         easysimd_vld1_u32(test_vec[i].r[2]), easysimd_vld1_u32(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u32x2(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u32x2(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u32x2(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u32x2(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_uint32x2x4_t src = easysimd_test_arm_neon_random_u32x2x4();
    easysimd_test_arm_neon_write_u32x2x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint32_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vu32(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint32x2x4_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld4_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_u32x2x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t src[4][8];
    uint64_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint64x1x4_t src = {
        {easysimd_vld1_u64(test_vec[i].src[0]), easysimd_vld1_u64(test_vec[i].src[1]),
         easysimd_vld1_u64(test_vec[i].src[2]), easysimd_vld1_u64(test_vec[i].src[3])}};

    easysimd_uint64x1x4_t r = easysimd_vld4_lane_u64(test_vec[i].buf, src, 0);

    easysimd_uint64x1x4_t expected = {
        {easysimd_vld1_u64(test_vec[i].r[0]), easysimd_vld1_u64(test_vec[i].r[1]),
         easysimd_vld1_u64(test_vec[i].r[2]), easysimd_vld1_u64(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u64x1(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u64x1(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u64x1(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u64x1(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_uint64x1x4_t src = easysimd_test_arm_neon_random_u64x1x4();
    easysimd_test_arm_neon_write_u64x1x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint64_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vu64(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint64x1x4_t r = easysimd_vld4_lane_u64(buf, src, 0);
    easysimd_test_arm_neon_write_u64x1x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t src[4][8];
    easysimd_float32_t buf[4];
    easysimd_float32_t r[4][8];
  } test_vec[] = {
    {
        { { EASYSIMD_FLOAT32_C(    -4.66), EASYSIMD_FLOAT32_C(   -18.00) },
          { EASYSIMD_FLOAT32_C(    54.81), EASYSIMD_FLOAT32_C(   -21.07) },
          { EASYSIMD_FLOAT32_C(    94.81), EASYSIMD_FLOAT32_C(    81.11) },
          { EASYSIMD_FLOAT32_C(   -83.82), EASYSIMD_FLOAT32_C(    74.22) } },
      { EASYSIMD_FLOAT32_C(    33.78), EASYSIMD_FLOAT32_C(    14.41), EASYSIMD_FLOAT32_C(    79.56), EASYSIMD_FLOAT32_C(   -37.96) },
        { { EASYSIMD_FLOAT32_C(    33.78), EASYSIMD_FLOAT32_C(   -18.00) },
          { EASYSIMD_FLOAT32_C(    14.41), EASYSIMD_FLOAT32_C(   -21.07) },
          { EASYSIMD_FLOAT32_C(    79.56), EASYSIMD_FLOAT32_C(    81.11) },
          { EASYSIMD_FLOAT32_C(   -37.96), EASYSIMD_FLOAT32_C(    74.22) } },
    },
    {
        { { EASYSIMD_FLOAT32_C(    18.96), EASYSIMD_FLOAT32_C(   -48.04) },
          { EASYSIMD_FLOAT32_C(   -15.72), EASYSIMD_FLOAT32_C(   -52.18) },
          { EASYSIMD_FLOAT32_C(    73.37), EASYSIMD_FLOAT32_C(   -29.70) },
          { EASYSIMD_FLOAT32_C(    40.56), EASYSIMD_FLOAT32_C(   -24.15) } },
      { EASYSIMD_FLOAT32_C(   -10.02), EASYSIMD_FLOAT32_C(    72.45), EASYSIMD_FLOAT32_C(    50.76), EASYSIMD_FLOAT32_C(   -87.18) },
        { { EASYSIMD_FLOAT32_C(    18.96), EASYSIMD_FLOAT32_C(   -10.02) },
          { EASYSIMD_FLOAT32_C(   -15.72), EASYSIMD_FLOAT32_C(    72.45) },
          { EASYSIMD_FLOAT32_C(    73.37), EASYSIMD_FLOAT32_C(    50.76) },
          { EASYSIMD_FLOAT32_C(    40.56), EASYSIMD_FLOAT32_C(   -87.18) } },
    }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x2x4_t r;
    easysimd_float32x2x4_t src = {
        {easysimd_vld1_f32(test_vec[i].src[0]), easysimd_vld1_f32(test_vec[i].src[1]),
         easysimd_vld1_f32(test_vec[i].src[2]), easysimd_vld1_f32(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_2_(easysimd_vld4_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_float32x2x4_t expected = {
        {easysimd_vld1_f32(test_vec[i].r[0]), easysimd_vld1_f32(test_vec[i].r[1]),
         easysimd_vld1_f32(test_vec[i].r[2]), easysimd_vld1_f32(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_f32x2(r.val[0], expected.val[0], 1);
    easysimd_test_arm_neon_assert_equal_f32x2(r.val[1], expected.val[1], 1);
    easysimd_test_arm_neon_assert_equal_f32x2(r.val[2], expected.val[2], 1);
    easysimd_test_arm_neon_assert_equal_f32x2(r.val[3], expected.val[3], 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32x2x4_t src = easysimd_test_arm_neon_random_f32x2x4(-100.0f, 100.0f);
    easysimd_test_arm_neon_write_f32x2x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float32_t buf[4];
    easysimd_test_codegen_random_vf32(4, buf, -100.0f, 100.0f);
    easysimd_test_codegen_write_vf32(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float32x2x4_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld4_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_f32x2x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t src[4][8];
    easysimd_float64_t buf[4];
    easysimd_float64_t r[4][8];
  } test_vec[] = {
    {
        { { EASYSIMD_FLOAT64_C(    62.84) },
          { EASYSIMD_FLOAT64_C(   -97.22) },
          { EASYSIMD_FLOAT64_C(    17.10) },
          { EASYSIMD_FLOAT64_C(    83.43) } },
      { EASYSIMD_FLOAT64_C(    17.28), EASYSIMD_FLOAT64_C(    -9.03), EASYSIMD_FLOAT64_C(   -57.78), EASYSIMD_FLOAT64_C(   -87.37) },
        { { EASYSIMD_FLOAT64_C(    17.28) },
          { EASYSIMD_FLOAT64_C(    -9.03) },
          { EASYSIMD_FLOAT64_C(   -57.78) },
          { EASYSIMD_FLOAT64_C(   -87.37) } },
    },
    {
        { { EASYSIMD_FLOAT64_C(    72.97) },
          { EASYSIMD_FLOAT64_C(    97.03) },
          { EASYSIMD_FLOAT64_C(    -8.44) },
          { EASYSIMD_FLOAT64_C(    67.78) } },
      { EASYSIMD_FLOAT64_C(    78.14), EASYSIMD_FLOAT64_C(     7.74), EASYSIMD_FLOAT64_C(    42.00), EASYSIMD_FLOAT64_C(    11.92) },
        { { EASYSIMD_FLOAT64_C(    78.14) },
          { EASYSIMD_FLOAT64_C(     7.74) },
          { EASYSIMD_FLOAT64_C(    42.00) },
          { EASYSIMD_FLOAT64_C(    11.92) } },
    }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x1x4_t src = {
        {easysimd_vld1_f64(test_vec[i].src[0]), easysimd_vld1_f64(test_vec[i].src[1]),
         easysimd_vld1_f64(test_vec[i].src[2]), easysimd_vld1_f64(test_vec[i].src[3])}};
    easysimd_float64x1x4_t r = easysimd_vld4_lane_f64(test_vec[i].buf, src, 0);

    easysimd_float64x1x4_t expected = {
        {easysimd_vld1_f64(test_vec[i].r[0]), easysimd_vld1_f64(test_vec[i].r[1]),
         easysimd_vld1_f64(test_vec[i].r[2]), easysimd_vld1_f64(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_f64x1(r.val[0], expected.val[0], 1);
    easysimd_test_arm_neon_assert_equal_f64x1(r.val[1], expected.val[1], 1);
    easysimd_test_arm_neon_assert_equal_f64x1(r.val[2], expected.val[2], 1);
    easysimd_test_arm_neon_assert_equal_f64x1(r.val[3], expected.val[3], 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64x1x4_t src = easysimd_test_arm_neon_random_f64x1x4(-100.0, 100.0);
    easysimd_test_arm_neon_write_f64x1x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float64_t buf[4];
    easysimd_test_codegen_random_vf64(4, buf, -100.0, 100.0);
    easysimd_test_codegen_write_vf64(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float64x1x4_t r = easysimd_vld4_lane_f64(buf, src, 0);
    easysimd_test_arm_neon_write_f64x1x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t src[4][16];
    int8_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16x4_t src = {
        {easysimd_vld1q_s8(test_vec[i].src[0]), easysimd_vld1q_s8(test_vec[i].src[1]),
         easysimd_vld1q_s8(test_vec[i].src[2]), easysimd_vld1q_s8(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_8_(easysimd_vld4q_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_int8x16x4_t expected = {
        {easysimd_vld1q_s8(test_vec[i].r[0]), easysimd_vld1q_s8(test_vec[i].r[1]),
         easysimd_vld1q_s8(test_vec[i].r[2]), easysimd_vld1q_s8(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i8x16(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i8x16(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i8x16(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i8x16(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int8x16x4_t src = easysimd_test_arm_neon_random_i8x16x4();
    easysimd_test_arm_neon_write_i8x16x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int8_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi8(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int8x16x4_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld4q_lane_s8, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_i8x16x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t src[4][8];
    int16_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8x4_t src = {
        {easysimd_vld1q_s16(test_vec[i].src[0]), easysimd_vld1q_s16(test_vec[i].src[1]),
         easysimd_vld1q_s16(test_vec[i].src[2]), easysimd_vld1q_s16(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_4_(easysimd_vld4q_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_int16x8x4_t expected = {
        {easysimd_vld1q_s16(test_vec[i].r[0]), easysimd_vld1q_s16(test_vec[i].r[1]),
         easysimd_vld1q_s16(test_vec[i].r[2]), easysimd_vld1q_s16(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i16x8(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i16x8(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i16x8(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i16x8(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_int16x8x4_t src = easysimd_test_arm_neon_random_i16x8x4();
    easysimd_test_arm_neon_write_i16x8x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int16_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi16(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int16x8x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld4q_lane_s16, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_i16x8x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t src[4][8];
    int32_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4x4_t src = {
        {easysimd_vld1q_s32(test_vec[i].src[0]), easysimd_vld1q_s32(test_vec[i].src[1]),
         easysimd_vld1q_s32(test_vec[i].src[2]), easysimd_vld1q_s32(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_2_(easysimd_vld4q_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_int32x4x4_t expected = {
        {easysimd_vld1q_s32(test_vec[i].r[0]), easysimd_vld1q_s32(test_vec[i].r[1]),
         easysimd_vld1q_s32(test_vec[i].r[2]), easysimd_vld1q_s32(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i32x4(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i32x4(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i32x4(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i32x4(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int32x4x4_t src = easysimd_test_arm_neon_random_i32x4x4();
    easysimd_test_arm_neon_write_i32x4x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int32_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi32(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int32x4x4_t r;
    EASYSIMD_CONSTIFY_2_(easysimd_vld4q_lane_s32, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_i32x4x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t src[4][8];
    int64_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int64x2x4_t src = {
        {easysimd_vld1q_s64(test_vec[i].src[0]), easysimd_vld1q_s64(test_vec[i].src[1]),
         easysimd_vld1q_s64(test_vec[i].src[2]), easysimd_vld1q_s64(test_vec[i].src[3])}};
    easysimd_int64x2x4_t r = easysimd_vld4q_lane_s64(test_vec[i].buf, src, 0);

    easysimd_int64x2x4_t expected = {
        {easysimd_vld1q_s64(test_vec[i].r[0]), easysimd_vld1q_s64(test_vec[i].r[1]),
         easysimd_vld1q_s64(test_vec[i].r[2]), easysimd_vld1q_s64(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_i64x2(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_i64x2(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_i64x2(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_i64x2(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_int64x2x4_t src = easysimd_test_arm_neon_random_i64x2x4();
    easysimd_test_arm_neon_write_i64x2x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    int64_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vi64(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_int64x2x4_t r = easysimd_vld4q_lane_s64(buf, src, 0);
    easysimd_test_arm_neon_write_i64x2x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t src[4][16];
    uint8_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x16x4_t src = {
        {easysimd_vld1q_u8(test_vec[i].src[0]), easysimd_vld1q_u8(test_vec[i].src[1]),
         easysimd_vld1q_u8(test_vec[i].src[2]), easysimd_vld1q_u8(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_16_(easysimd_vld4q_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_uint8x16x4_t expected = {
        {easysimd_vld1q_u8(test_vec[i].r[0]), easysimd_vld1q_u8(test_vec[i].r[1]),
         easysimd_vld1q_u8(test_vec[i].r[2]), easysimd_vld1q_u8(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u8x16(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u8x16(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u8x16(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u8x16(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint8x16x4_t src = easysimd_test_arm_neon_random_u8x16x4();
    easysimd_test_arm_neon_write_u8x16x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint8_t buf[4];
    easysimd_test_codegen_random_memory(4, buf);
    easysimd_test_codegen_write_vu8(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint8x16x4_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld4q_lane_u8, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_u8x16x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t src[4][8];
    uint16_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x8x4_t src = {
        {easysimd_vld1q_u16(test_vec[i].src[0]), easysimd_vld1q_u16(test_vec[i].src[1]),
         easysimd_vld1q_u16(test_vec[i].src[2]), easysimd_vld1q_u16(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_8_(easysimd_vld4q_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_uint16x8x4_t expected = {
        {easysimd_vld1q_u16(test_vec[i].r[0]), easysimd_vld1q_u16(test_vec[i].r[1]),
         easysimd_vld1q_u16(test_vec[i].r[2]), easysimd_vld1q_u16(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u16x8(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u16x8(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u16x8(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u16x8(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint16x8x4_t src = easysimd_test_arm_neon_random_u16x8x4();
    easysimd_test_arm_neon_write_u16x8x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint16_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vu16(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint16x8x4_t r;
    EASYSIMD_CONSTIFY_8_(easysimd_vld4q_lane_u16, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_u16x8x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t src[4][4];
    uint32_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x4x4_t src = {
        {easysimd_vld1q_u32(test_vec[i].src[0]), easysimd_vld1q_u32(test_vec[i].src[1]),
         easysimd_vld1q_u32(test_vec[i].src[2]), easysimd_vld1q_u32(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_4_(easysimd_vld4q_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_uint32x4x4_t expected = {
        {easysimd_vld1q_u32(test_vec[i].r[0]), easysimd_vld1q_u32(test_vec[i].r[1]),
         easysimd_vld1q_u32(test_vec[i].r[2]), easysimd_vld1q_u32(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u32x4(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u32x4(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u32x4(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u32x4(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_uint32x4x4_t src = easysimd_test_arm_neon_random_u32x4x4();
    easysimd_test_arm_neon_write_u32x4x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint32_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vu32(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint32x4x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld4q_lane_u32, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_u32x4x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t src[4][2];
    uint64_t buf[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint64x2x4_t src = {
        {easysimd_vld1q_u64(test_vec[i].src[0]), easysimd_vld1q_u64(test_vec[i].src[1]),
         easysimd_vld1q_u64(test_vec[i].src[2]), easysimd_vld1q_u64(test_vec[i].src[3])}};
    easysimd_uint64x2x4_t r = easysimd_vld4q_lane_u64(test_vec[i].buf, src, 0);

    easysimd_uint64x2x4_t expected = {
        {easysimd_vld1q_u64(test_vec[i].r[0]), easysimd_vld1q_u64(test_vec[i].r[1]),
         easysimd_vld1q_u64(test_vec[i].r[2]), easysimd_vld1q_u64(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_u64x2(r.val[0], expected.val[0]);
    easysimd_test_arm_neon_assert_equal_u64x2(r.val[1], expected.val[1]);
    easysimd_test_arm_neon_assert_equal_u64x2(r.val[2], expected.val[2]);
    easysimd_test_arm_neon_assert_equal_u64x2(r.val[3], expected.val[3]);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_uint64x2x4_t src = easysimd_test_arm_neon_random_u64x2x4();
    easysimd_test_arm_neon_write_u64x2x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    uint64_t buf[4];
    easysimd_test_codegen_random_memory(4, HEDLEY_REINTERPRET_CAST(uint8_t*, buf));
    easysimd_test_codegen_write_vu64(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_uint64x2x4_t r = easysimd_vld4q_lane_u64(buf, src, 0);
    easysimd_test_arm_neon_write_u64x2x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t src[4][4];
    easysimd_float32_t buf[4];
    easysimd_float32_t r[4][4];
  } test_vec[] = {
    {
        { { EASYSIMD_FLOAT32_C(    19.27), EASYSIMD_FLOAT32_C(    11.72), EASYSIMD_FLOAT32_C(   -12.20), EASYSIMD_FLOAT32_C(   -36.38) },
          { EASYSIMD_FLOAT32_C(   -37.63), EASYSIMD_FLOAT32_C(   -45.48), EASYSIMD_FLOAT32_C(   -45.93), EASYSIMD_FLOAT32_C(    47.64) },
          { EASYSIMD_FLOAT32_C(   -32.47), EASYSIMD_FLOAT32_C(    33.82), EASYSIMD_FLOAT32_C(    49.51), EASYSIMD_FLOAT32_C(   -22.89) },
          { EASYSIMD_FLOAT32_C(   -39.76), EASYSIMD_FLOAT32_C(    33.96), EASYSIMD_FLOAT32_C(    29.59), EASYSIMD_FLOAT32_C(   -38.64) } },
      { EASYSIMD_FLOAT32_C(   -24.64), EASYSIMD_FLOAT32_C(   -43.67), EASYSIMD_FLOAT32_C(    96.38), EASYSIMD_FLOAT32_C(    53.18) },
        { { EASYSIMD_FLOAT32_C(   -24.64), EASYSIMD_FLOAT32_C(    11.72), EASYSIMD_FLOAT32_C(   -12.20), EASYSIMD_FLOAT32_C(   -36.38) },
          { EASYSIMD_FLOAT32_C(   -43.67), EASYSIMD_FLOAT32_C(   -45.48), EASYSIMD_FLOAT32_C(   -45.93), EASYSIMD_FLOAT32_C(    47.64) },
          { EASYSIMD_FLOAT32_C(    96.38), EASYSIMD_FLOAT32_C(    33.82), EASYSIMD_FLOAT32_C(    49.51), EASYSIMD_FLOAT32_C(   -22.89) },
          { EASYSIMD_FLOAT32_C(    53.18), EASYSIMD_FLOAT32_C(    33.96), EASYSIMD_FLOAT32_C(    29.59), EASYSIMD_FLOAT32_C(   -38.64) } },
    },
    {
        { { EASYSIMD_FLOAT32_C(    27.35), EASYSIMD_FLOAT32_C(   -28.81), EASYSIMD_FLOAT32_C(    36.44), EASYSIMD_FLOAT32_C(   -23.07) },
          { EASYSIMD_FLOAT32_C(    41.76), EASYSIMD_FLOAT32_C(    -5.78), EASYSIMD_FLOAT32_C(    33.86), EASYSIMD_FLOAT32_C(   -14.17) },
          { EASYSIMD_FLOAT32_C(   -18.89), EASYSIMD_FLOAT32_C(    13.92), EASYSIMD_FLOAT32_C(    18.17), EASYSIMD_FLOAT32_C(   -49.62) },
          { EASYSIMD_FLOAT32_C(   -24.36), EASYSIMD_FLOAT32_C(   -44.03), EASYSIMD_FLOAT32_C(   -35.99), EASYSIMD_FLOAT32_C(   -11.98) } },
      { EASYSIMD_FLOAT32_C(   -79.03), EASYSIMD_FLOAT32_C(   -63.84), EASYSIMD_FLOAT32_C(   -28.69), EASYSIMD_FLOAT32_C(   -43.97) },
        { { EASYSIMD_FLOAT32_C(    27.35), EASYSIMD_FLOAT32_C(   -79.03), EASYSIMD_FLOAT32_C(    36.44), EASYSIMD_FLOAT32_C(   -23.07) },
          { EASYSIMD_FLOAT32_C(    41.76), EASYSIMD_FLOAT32_C(   -63.84), EASYSIMD_FLOAT32_C(    33.86), EASYSIMD_FLOAT32_C(   -14.17) },
          { EASYSIMD_FLOAT32_C(   -18.89), EASYSIMD_FLOAT32_C(   -28.69), EASYSIMD_FLOAT32_C(    18.17), EASYSIMD_FLOAT32_C(   -49.62) },
          { EASYSIMD_FLOAT32_C(   -24.36), EASYSIMD_FLOAT32_C(   -43.97), EASYSIMD_FLOAT32_C(   -35.99), EASYSIMD_FLOAT32_C(   -11.98) } },
    },
    {
        { { EASYSIMD_FLOAT32_C(   -48.10), EASYSIMD_FLOAT32_C(   -14.83), EASYSIMD_FLOAT32_C(     5.13), EASYSIMD_FLOAT32_C(   -37.86) },
          { EASYSIMD_FLOAT32_C(   -30.87), EASYSIMD_FLOAT32_C(   -15.28), EASYSIMD_FLOAT32_C(   -26.50), EASYSIMD_FLOAT32_C(     6.81) },
          { EASYSIMD_FLOAT32_C(    12.88), EASYSIMD_FLOAT32_C(   -28.31), EASYSIMD_FLOAT32_C(   -16.60), EASYSIMD_FLOAT32_C(    -9.77) },
          { EASYSIMD_FLOAT32_C(    -7.12), EASYSIMD_FLOAT32_C(   -30.16), EASYSIMD_FLOAT32_C(    17.17), EASYSIMD_FLOAT32_C(   -15.36) } },
      { EASYSIMD_FLOAT32_C(    28.12), EASYSIMD_FLOAT32_C(     2.05), EASYSIMD_FLOAT32_C(    40.94), EASYSIMD_FLOAT32_C(    90.35) },
        { { EASYSIMD_FLOAT32_C(   -48.10), EASYSIMD_FLOAT32_C(   -14.83), EASYSIMD_FLOAT32_C(    28.12), EASYSIMD_FLOAT32_C(   -37.86) },
          { EASYSIMD_FLOAT32_C(   -30.87), EASYSIMD_FLOAT32_C(   -15.28), EASYSIMD_FLOAT32_C(     2.05), EASYSIMD_FLOAT32_C(     6.81) },
          { EASYSIMD_FLOAT32_C(    12.88), EASYSIMD_FLOAT32_C(   -28.31), EASYSIMD_FLOAT32_C(    40.94), EASYSIMD_FLOAT32_C(    -9.77) },
          { EASYSIMD_FLOAT32_C(    -7.12), EASYSIMD_FLOAT32_C(   -30.16), EASYSIMD_FLOAT32_C(    90.35), EASYSIMD_FLOAT32_C(   -15.36) } },
    },
    {
        { { EASYSIMD_FLOAT32_C(   -35.05), EASYSIMD_FLOAT32_C(   -11.36), EASYSIMD_FLOAT32_C(    45.56), EASYSIMD_FLOAT32_C(    -9.41) },
          { EASYSIMD_FLOAT32_C(    -5.39), EASYSIMD_FLOAT32_C(   -40.44), EASYSIMD_FLOAT32_C(    28.61), EASYSIMD_FLOAT32_C(     5.10) },
          { EASYSIMD_FLOAT32_C(   -22.36), EASYSIMD_FLOAT32_C(   -35.74), EASYSIMD_FLOAT32_C(    33.11), EASYSIMD_FLOAT32_C(   -20.46) },
          { EASYSIMD_FLOAT32_C(    -0.57), EASYSIMD_FLOAT32_C(   -11.76), EASYSIMD_FLOAT32_C(    -8.32), EASYSIMD_FLOAT32_C(    18.56) } },
      { EASYSIMD_FLOAT32_C(    45.92), EASYSIMD_FLOAT32_C(    30.37), EASYSIMD_FLOAT32_C(   -49.27), EASYSIMD_FLOAT32_C(   -28.32) },
        { { EASYSIMD_FLOAT32_C(   -35.05), EASYSIMD_FLOAT32_C(   -11.36), EASYSIMD_FLOAT32_C(    45.56), EASYSIMD_FLOAT32_C(    45.92) },
          { EASYSIMD_FLOAT32_C(    -5.39), EASYSIMD_FLOAT32_C(   -40.44), EASYSIMD_FLOAT32_C(    28.61), EASYSIMD_FLOAT32_C(    30.37) },
          { EASYSIMD_FLOAT32_C(   -22.36), EASYSIMD_FLOAT32_C(   -35.74), EASYSIMD_FLOAT32_C(    33.11), EASYSIMD_FLOAT32_C(   -49.27) },
          { EASYSIMD_FLOAT32_C(    -0.57), EASYSIMD_FLOAT32_C(   -11.76), EASYSIMD_FLOAT32_C(    -8.32), EASYSIMD_FLOAT32_C(   -28.32) } },
    }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float32x4x4_t r;
    easysimd_float32x4x4_t src = {
        {easysimd_vld1q_f32(test_vec[i].src[0]), easysimd_vld1q_f32(test_vec[i].src[1]),
         easysimd_vld1q_f32(test_vec[i].src[2]), easysimd_vld1q_f32(test_vec[i].src[3])}};

    EASYSIMD_CONSTIFY_4_(easysimd_vld4q_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, test_vec[i].buf, src);

    easysimd_float32x4x4_t expected = {
        {easysimd_vld1q_f32(test_vec[i].r[0]), easysimd_vld1q_f32(test_vec[i].r[1]),
         easysimd_vld1q_f32(test_vec[i].r[2]), easysimd_vld1q_f32(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_f32x4(r.val[0], expected.val[0], 1);
    easysimd_test_arm_neon_assert_equal_f32x4(r.val[1], expected.val[1], 1);
    easysimd_test_arm_neon_assert_equal_f32x4(r.val[2], expected.val[2], 1);
    easysimd_test_arm_neon_assert_equal_f32x4(r.val[3], expected.val[3], 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 4 ; i++) {
    easysimd_float32x4x4_t src = easysimd_test_arm_neon_random_f32x4x4(-50.0f, 50.0f);
    easysimd_test_arm_neon_write_f32x4x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float32_t buf[4];
    easysimd_test_codegen_random_vf32(4, buf, -100.0f, 100.0f);
    easysimd_test_codegen_write_vf32(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float32x4x4_t r;
    EASYSIMD_CONSTIFY_4_(easysimd_vld4q_lane_f32, r, (HEDLEY_UNREACHABLE(), r), i, buf, src);
    easysimd_test_arm_neon_write_f32x4x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vld4q_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t src[4][2];
    easysimd_float64_t buf[4];
    easysimd_float64_t r[4][2];
  } test_vec[] = {
    {
        { { EASYSIMD_FLOAT64_C(   -10.58), EASYSIMD_FLOAT64_C(   -17.83) },
          { EASYSIMD_FLOAT64_C(    59.36), EASYSIMD_FLOAT64_C(    53.26) },
          { EASYSIMD_FLOAT64_C(   -32.00), EASYSIMD_FLOAT64_C(   -72.88) },
          { EASYSIMD_FLOAT64_C(    -8.63), EASYSIMD_FLOAT64_C(   -98.94) } },
      { EASYSIMD_FLOAT64_C(   -55.98), EASYSIMD_FLOAT64_C(   -22.55), EASYSIMD_FLOAT64_C(    21.40), EASYSIMD_FLOAT64_C(     2.22) },
        { { EASYSIMD_FLOAT64_C(   -55.98), EASYSIMD_FLOAT64_C(   -17.83) },
          { EASYSIMD_FLOAT64_C(   -22.55), EASYSIMD_FLOAT64_C(    53.26) },
          { EASYSIMD_FLOAT64_C(    21.40), EASYSIMD_FLOAT64_C(   -72.88) },
          { EASYSIMD_FLOAT64_C(     2.22), EASYSIMD_FLOAT64_C(   -98.94) } },
    },
    {
        { { EASYSIMD_FLOAT64_C(    60.29), EASYSIMD_FLOAT64_C(    60.20) },
          { EASYSIMD_FLOAT64_C(   -69.98), EASYSIMD_FLOAT64_C(    34.61) },
          { EASYSIMD_FLOAT64_C(    66.90), EASYSIMD_FLOAT64_C(   -35.54) },
          { EASYSIMD_FLOAT64_C(    19.63), EASYSIMD_FLOAT64_C(    63.63) } },
      { EASYSIMD_FLOAT64_C(   -21.98), EASYSIMD_FLOAT64_C(    98.64), EASYSIMD_FLOAT64_C(   -95.62), EASYSIMD_FLOAT64_C(    54.86) },
        { { EASYSIMD_FLOAT64_C(   -21.98), EASYSIMD_FLOAT64_C(    60.20) },
          { EASYSIMD_FLOAT64_C(    98.64), EASYSIMD_FLOAT64_C(    34.61) },
          { EASYSIMD_FLOAT64_C(   -95.62), EASYSIMD_FLOAT64_C(   -35.54) },
          { EASYSIMD_FLOAT64_C(    54.86), EASYSIMD_FLOAT64_C(    63.63) } },
    }
  };

  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_float64x2x4_t src = {
        {easysimd_vld1q_f64(test_vec[i].src[0]), easysimd_vld1q_f64(test_vec[i].src[1]),
         easysimd_vld1q_f64(test_vec[i].src[2]), easysimd_vld1q_f64(test_vec[i].src[3])}};
    easysimd_float64x2x4_t r = easysimd_vld4q_lane_f64(test_vec[i].buf, src, 0);

    easysimd_float64x2x4_t expected = {
        {easysimd_vld1q_f64(test_vec[i].r[0]), easysimd_vld1q_f64(test_vec[i].r[1]),
         easysimd_vld1q_f64(test_vec[i].r[2]), easysimd_vld1q_f64(test_vec[i].r[3])}};

    easysimd_test_arm_neon_assert_equal_f64x2(r.val[0], expected.val[0], 1);
    easysimd_test_arm_neon_assert_equal_f64x2(r.val[1], expected.val[1], 1);
    easysimd_test_arm_neon_assert_equal_f64x2(r.val[2], expected.val[2], 1);
    easysimd_test_arm_neon_assert_equal_f64x2(r.val[3], expected.val[3], 1);
  }

  return 0;
#else
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64x2x4_t src = easysimd_test_arm_neon_random_f64x2x4(-100.0, 100.0);
    easysimd_test_arm_neon_write_f64x2x4(2, src, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_float64_t buf[4];
    easysimd_test_codegen_random_vf64(4, buf, -100.0, 100.0);
    easysimd_test_codegen_write_vf64(2, 4, buf, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_float64x2x4_t r = easysimd_vld4q_lane_f64(buf, src, 0);
    easysimd_test_arm_neon_write_f64x2x4(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
#if !defined(EASYSIMD_BUG_INTEL_857088)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4_lane_f64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vld4q_lane_f64)
#endif /* !defined(EASYSIMD_BUG_INTEL_857088) */
