#define EASYSIMD_TEST_ARM_NEON_INSN movl

#include "test-neon.h"
#include <easysimd/arm/neon/movl.h>

static int
test_easysimd_vmovl_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
  struct {
    int8_t a[8];
    int16_t r[8];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8_t r = easysimd_vmovl_s8(a);
...
static int
test_easysimd_vmovl_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
  struct {
    int16_t a[4];
    int32_t r[4];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t r = easysimd_vmovl_s16(a);
...
static int
test_easysimd_vmovl_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
  struct {
    int32_t a[2];
    int64_t r[2];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x2_t r = easysimd_vmovl_s32(a);
...
static int
test_easysimd_vmovl_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
  struct {
    uint8_t a[8];
    uint16_t r[8];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x8_t r = easysimd_vmovl_u8(a);
...
static int
test_easysimd_vmovl_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
  struct {
    uint16_t a[4];
    uint32_t r[4];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x4_t r = easysimd_vmovl_u16(a);
...
static int
test_easysimd_vmovl_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
  struct {
    uint32_t a[2];
    uint64_t r[2];
  } test_vec[] = {
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint64x2_t r = easysimd_vmovl_u32(a);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vmovl_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vmovl_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vmovl_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vmovl_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vmovl_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vmovl_u32)
