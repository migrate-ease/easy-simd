#define EASYSIMD_TEST_ARM_NEON_INSN qdmulh

#include "test-neon.h"
#include "../../../easysimd/arm/neon/qdmulh.h"

static int
test_easysimd_vqdmulh_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x4_t r = easysimd_vqdmulh_s16(a, b);
...
static int
test_easysimd_vqdmulh_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x2_t r = easysimd_vqdmulh_s32(a, b);
...
static int
test_easysimd_vqdmulhq_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[8];
    int16_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8_t r = easysimd_vqdmulhq_s16(a, b);
...
static int
test_easysimd_vqdmulhq_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    int32_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t r = easysimd_vqdmulhq_s32(a, b);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqdmulh_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqdmulh_s32)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vqdmulhq_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqdmulhq_s32)
