#define EASYSIMD_TEST_ARM_NEON_INSN qdmull

#include "test-neon.h"
#include "../../../easysimd/arm/neon/qdmull.h"

static int
test_easysimd_vqdmull_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t r = easysimd_vqdmull_s16(a, b);
...
static int
test_easysimd_vqdmull_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x2_t r = easysimd_vqdmull_s32(a, b);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqdmull_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqdmull_s32)
