#define EASYSIMD_TEST_ARM_NEON_INSN qmovn_high

#include "test-neon.h"
#include "../../../easysimd/arm/neon/and.h"
#include "../../../easysimd/arm/neon/dup_n.h"
#include "../../../easysimd/arm/neon/neg.h"
#include "../../../easysimd/arm/neon/qmovn_high.h"
#include "../../../easysimd/arm/neon/shl.h"


static int
test_easysimd_vqmovn_high_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t r[8];
    int16_t a[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int8x8_t r = easysimd_vld1_s8(test_vec[i].r);
    easysimd_int16x8_t a = easysimd_vld1q_s16(test_vec[i].a);
    easysimd_int8x16_t r_ = easysimd_vqmovn_high_s16(r, a);

    easysimd_test_arm_neon_assert_equal_i8x16(r_, easysimd_vld1q_s8(test_vec[i].r_));
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int8x8_t r = easysimd_test_arm_neon_random_i8x8();
    easysimd_int16x8_t s = easysimd_vandq_s16(easysimd_test_arm_neon_random_i16x8(), easysimd_vdupq_n_s16(15));
    easysimd_int16x8_t a = easysimd_vshlq_s16(easysimd_test_arm_neon_random_i16x8(), easysimd_vnegq_s16(s));
    easysimd_int8x16_t r_ = easysimd_vqmovn_high_s16(r, a);

    easysimd_test_arm_neon_write_i8x8(2, r, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x8(2, a, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_arm_neon_write_i8x16(2, r_, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqmovn_high_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t r[4];
    int32_t a[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int16x4_t r = easysimd_vld1_s16(test_vec[i].r);
    easysimd_int32x4_t a = easysimd_vld1q_s32(test_vec[i].a);
    easysimd_int16x8_t r_ = easysimd_vqmovn_high_s32(r, a);

    easysimd_test_arm_neon_assert_equal_i16x8(r_, easysimd_vld1q_s16(test_vec[i].r_));
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int16x4_t r = easysimd_test_arm_neon_random_i16x4();
    easysimd_int32x4_t s = easysimd_vandq_s32(easysimd_test_arm_neon_random_i32x4(), easysimd_vdupq_n_s32(31));
    easysimd_int32x4_t a = easysimd_vshlq_s32(easysimd_test_arm_neon_random_i32x4(), easysimd_vnegq_s32(s));
    easysimd_int16x8_t r_ = easysimd_vqmovn_high_s32(r, a);

    easysimd_test_arm_neon_write_i16x4(2, r, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x4(2, a, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_arm_neon_write_i16x8(2, r_, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqmovn_high_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t r[2];
    int64_t a[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_int32x2_t r = easysimd_vld1_s32(test_vec[i].r);
    easysimd_int64x2_t a = easysimd_vld1q_s64(test_vec[i].a);
    easysimd_int32x4_t r_ = easysimd_vqmovn_high_s64(r, a);

    easysimd_test_arm_neon_assert_equal_i32x4(r_, easysimd_vld1q_s32(test_vec[i].r_));
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_int32x2_t r = easysimd_test_arm_neon_random_i32x2();
    easysimd_int64x2_t s = easysimd_vandq_s64(easysimd_test_arm_neon_random_i64x2(), easysimd_vdupq_n_s64(63));
    easysimd_int64x2_t a = easysimd_vshlq_s64(easysimd_test_arm_neon_random_i64x2(), easysimd_vnegq_s64(s));
    easysimd_int32x4_t r_ = easysimd_vqmovn_high_s64(r, a);

    easysimd_test_arm_neon_write_i32x2(2, r, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x2(2, a, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_arm_neon_write_i32x4(2, r_, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqmovn_high_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t r[8];
    uint16_t a[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint8x8_t r = easysimd_vld1_u8(test_vec[i].r);
    easysimd_uint16x8_t a = easysimd_vld1q_u16(test_vec[i].a);
    easysimd_uint8x16_t r_ = easysimd_vqmovn_high_u16(r, a);

    easysimd_test_arm_neon_assert_equal_u8x16(r_, easysimd_vld1q_u8(test_vec[i].r_));
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint8x8_t r = easysimd_test_arm_neon_random_u8x8();
    easysimd_int16x8_t s = easysimd_vandq_s16(easysimd_test_arm_neon_random_i16x8(), easysimd_vdupq_n_s16(15));
    easysimd_uint16x8_t a = easysimd_vshlq_u16(easysimd_test_arm_neon_random_u16x8(), easysimd_vnegq_s16(s));
    easysimd_uint8x16_t r_ = easysimd_vqmovn_high_u16(r, a);

    easysimd_test_arm_neon_write_u8x8(2, r, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x8(2, a, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_arm_neon_write_u8x16(2, r_, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqmovn_high_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t r[4];
    uint32_t a[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x8_t r_ = easysimd_vqmovn_high_u32(r, a);

    easysimd_test_arm_neon_assert_equal_u16x8(r_, easysimd_vld1q_u16(test_vec[i].r_));
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint16x4_t r = easysimd_test_arm_neon_random_u16x4();
    easysimd_int32x4_t s = easysimd_vandq_s32(easysimd_test_arm_neon_random_i32x4(), easysimd_vdupq_n_s32(31));
    easysimd_uint32x4_t a = easysimd_vshlq_u32(easysimd_test_arm_neon_random_u32x4(), easysimd_vnegq_s32(s));
    easysimd_uint16x8_t r_ = easysimd_vqmovn_high_u32(r, a);

    easysimd_test_arm_neon_write_u16x4(2, r, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x4(2, a, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_arm_neon_write_u16x8(2, r_, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqmovn_high_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t r[2];
    uint64_t a[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    easysimd_uint32x2_t r = easysimd_vld1_u32(test_vec[i].r);
    easysimd_uint64x2_t a = easysimd_vld1q_u64(test_vec[i].a);
    easysimd_uint32x4_t r_ = easysimd_vqmovn_high_u64(r, a);

    easysimd_test_arm_neon_assert_equal_u32x4(r_, easysimd_vld1q_u32(test_vec[i].r_));
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    easysimd_uint32x2_t r = easysimd_test_arm_neon_random_u32x2();
    easysimd_int64x2_t s = easysimd_vandq_s64(easysimd_test_arm_neon_random_i64x2(), easysimd_vdupq_n_s64(63));
    easysimd_uint64x2_t a = easysimd_vshlq_u64(easysimd_test_arm_neon_random_u64x2(), easysimd_vnegq_s64(s));
    easysimd_uint32x4_t r_ = easysimd_vqmovn_high_u64(r, a);

    easysimd_test_arm_neon_write_u32x2(2, r, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x2(2, a, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_arm_neon_write_u32x4(2, r_, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovn_high_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovn_high_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovn_high_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovn_high_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovn_high_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovn_high_u64)
