#define EASYSIMD_TEST_ARM_NEON_INSN qmovun

#include "test-neon.h"
#include "../../../easysimd/arm/neon/qmovun.h"

static int
test_easysimd_vqmovun_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[8];
    uint8_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x8_t r = easysimd_vqmovun_s16(a);
...
static int
test_easysimd_vqmovun_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    uint16_t r[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint16x4_t r = easysimd_vqmovun_s32(a);
...
static int
test_easysimd_vqmovun_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[2];
    uint32_t r[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint32x2_t r = easysimd_vqmovun_s64(a);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovun_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovun_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqmovun_s64)
