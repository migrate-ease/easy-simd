#define EASYSIMD_TEST_ARM_NEON_INSN qneg

#include "test-neon.h"
#include "../../../easysimd/arm/neon/qneg.h"
#include "../../../easysimd/arm/neon/set_lane.h"

static int
test_easysimd_vqnegb_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a;
    int8_t r;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int8_t r = easysimd_vqnegb_s8(test_vec[i].a);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a = (i == 0) ? INT8_MIN : easysimd_test_codegen_random_i8();
    int8_t r = easysimd_vqnegb_s8(a);
...
static int
test_easysimd_vqnegh_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a;
    int16_t r;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int16_t r = easysimd_vqnegh_s16(test_vec[i].a);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int16_t a = (i == 0) ? INT16_MIN : easysimd_test_codegen_random_i16();
    int16_t r = easysimd_vqnegh_s16(a);
...
static int
test_easysimd_vqnegs_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a;
    int32_t r;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int32_t r = easysimd_vqnegs_s32(test_vec[i].a);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int32_t a = (i == 0) ? INT32_MIN : easysimd_test_codegen_random_i32();
    int32_t r = easysimd_vqnegs_s32(a);
...
static int
test_easysimd_vqnegd_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a;
    int64_t r;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int64_t r = easysimd_vqnegd_s64(test_vec[i].a);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int64_t a = (i == 0) ? INT64_MIN : easysimd_test_codegen_random_i64();
    int64_t r = easysimd_vqnegd_s64(a);
...
static int
test_easysimd_vqneg_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[8];
    int8_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x8_t r = easysimd_vqneg_s8(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vset_lane_s8(INT8_MIN, a, easysimd_test_codegen_random_i8() & 7);
    easysimd_int8x8_t r = easysimd_vqneg_s8(a);
...
static int
test_easysimd_vqneg_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t r[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x4_t r = easysimd_vqneg_s16(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vset_lane_s16(INT16_MIN, a, easysimd_test_codegen_random_i8() & 3);
    easysimd_int16x4_t r = easysimd_vqneg_s16(a);
...
static int
test_easysimd_vqneg_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t r[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x2_t r = easysimd_vqneg_s32(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vset_lane_s32(INT32_MIN, a, easysimd_test_codegen_random_i8() & 1);
    easysimd_int32x2_t r = easysimd_vqneg_s32(a);
...
static int
test_easysimd_vqneg_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[1];
    int64_t r[1];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x1_t r = easysimd_vqneg_s64(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    if ((easysimd_test_codegen_random_i8() & 3) == 3)
      a = easysimd_vset_lane_s64(INT64_MIN, a, 0);
    easysimd_int64x1_t r = easysimd_vqneg_s64(a);
...
static int
test_easysimd_vqnegq_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[16];
    int8_t r[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16_t r = easysimd_vqnegq_s8(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vsetq_lane_s8(INT8_MIN, a, easysimd_test_codegen_random_i8() & 15);
    easysimd_int8x16_t r = easysimd_vqnegq_s8(a);
...
static int
test_easysimd_vqnegq_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[8];
    int16_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8_t r = easysimd_vqnegq_s16(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vsetq_lane_s16(INT16_MIN, a, easysimd_test_codegen_random_i8() & 7);
    easysimd_int16x8_t r = easysimd_vqnegq_s16(a);
...
static int
test_easysimd_vqnegq_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    int32_t r[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t r = easysimd_vqnegq_s32(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vsetq_lane_s32(INT32_MIN, a, easysimd_test_codegen_random_i8() & 3);
    easysimd_int32x4_t r = easysimd_vqnegq_s32(a);
...
static int
test_easysimd_vqnegq_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[2];
    int64_t r[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x2_t r = easysimd_vqnegq_s64(a);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    if ((easysimd_test_codegen_random_i8() & 3) == 3)
      a = easysimd_vsetq_lane_s64(INT64_MIN, a, 0);
    easysimd_int64x2_t r = easysimd_vqnegq_s64(a);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegb_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegh_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegs_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegd_s64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vqneg_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqneg_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqneg_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqneg_s64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegq_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegq_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegq_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqnegq_s64)
