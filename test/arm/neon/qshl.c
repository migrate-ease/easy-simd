#define EASYSIMD_TEST_ARM_NEON_INSN qshl

#include "test-neon.h"
#include "../../../easysimd/arm/neon/qshl.h"
#include "../../../easysimd/arm/neon/and.h"
#include "../../../easysimd/arm/neon/dup_n.h"
#include "../../../easysimd/arm/neon/neg.h"
#include "../../../easysimd/arm/neon/shl.h"
 ...
 * consider using easysimd_vqshl* instead of vqshl* so you can avoid an
 * ifdef. */

#if HEDLEY_HAS_WARNING("-Wsign-conversion") && defined(EASYSIMD_NATIVE_ALIASES_TESTING)
  #pragma clang diagnostic ignored "-Wsign-conversion"
#endif

static int
test_easysimd_vqshlb_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a;
    int8_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int8_t r = easysimd_vqshlb_s8(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a = easysimd_test_codegen_random_i8() >> (easysimd_test_codegen_random_i8() & 7);
    int8_t b = easysimd_test_codegen_random_i8() & 15;
    int8_t r = easysimd_vqshlb_s8(a, b);

    easysimd_test_codegen_write_i8(2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_codegen_write_i8(2, b, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i8(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqshlh_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a;
    int16_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int16_t r = easysimd_vqshlh_s16(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int16_t a = easysimd_test_codegen_random_i16() >> (easysimd_test_codegen_random_i16() & 15);
    int16_t b = easysimd_test_codegen_random_i16() & 31;
    int16_t r = easysimd_vqshlh_s16(a, b);

    easysimd_test_codegen_write_i16(2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_codegen_write_i16(2, b, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i16(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqshls_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a;
    int32_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int32_t r = easysimd_vqshls_s32(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int32_t a = easysimd_test_codegen_random_i32() >> (easysimd_test_codegen_random_i32() & 31);
    int32_t b = easysimd_test_codegen_random_i32() & 63;
    int32_t r = easysimd_vqshls_s32(a, b);

    easysimd_test_codegen_write_i32(2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_codegen_write_i32(2, b, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
static int
test_easysimd_vqshld_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a;
    int64_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    int64_t r = easysimd_vqshld_s64(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    int64_t a = easysimd_test_codegen_random_i64() >> (easysimd_test_codegen_random_i64() & 31);
    int64_t b = easysimd_test_codegen_random_i64() & 63;
    int64_t r = easysimd_vqshld_s64(a, b);
...
static int
test_easysimd_vqshlb_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a;
    int8_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint8_t r = easysimd_vqshlb_u8(test_vec[i].a, test_vec[i].b);

    easysimd_assert_equal_u8(r, test_vec[i].r);
  }
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    uint8_t a = easysimd_test_codegen_random_u8() >> (easysimd_test_codegen_random_u8() & 7);
    int8_t b = easysimd_test_codegen_random_i8() % 16;
    uint8_t r = easysimd_vqshlb_u8(a, b);

    easysimd_test_codegen_write_u8(2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_codegen_write_i8(2, b, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_u8(2, r, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vqshlh_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a;
    int16_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint16_t r = easysimd_vqshlh_u16(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    uint16_t a = easysimd_test_codegen_random_u16() >> (easysimd_test_codegen_random_u16() & 15);
    int16_t b = easysimd_test_codegen_random_i16() % 32;
    uint16_t r = easysimd_vqshlh_u16(a, b);
...
static int
test_easysimd_vqshls_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a;
    int32_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint32_t r = easysimd_vqshls_u32(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    uint32_t a = easysimd_test_codegen_random_u32() >> (easysimd_test_codegen_random_u32() & 31);
    int32_t b = easysimd_test_codegen_random_u32() % 64;
    uint32_t r = easysimd_vqshls_u32(a, b);
...
static int
test_easysimd_vqshld_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a;
    int64_t b;
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    uint64_t r = easysimd_vqshld_u64(test_vec[i].a, test_vec[i].b);
  ...
  fputc('\n', stdout);
  for (int i = 0 ; i < 8 ; i++) {
    uint64_t a = easysimd_test_codegen_random_u64() >> (easysimd_test_codegen_random_u64() & 31);
    int64_t b = easysimd_test_codegen_random_i64() % 64;
    uint64_t r = easysimd_vqshld_u64(a, b);
...
static int
test_easysimd_vqshl_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[8];
    int8_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x8_t r = easysimd_vqshl_s8(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_s8(a, easysimd_vneg_s8(easysimd_vand_s8(easysimd_test_arm_neon_random_i8x8(), easysimd_vdup_n_s8(7))));
    b = easysimd_vand_s8(b, easysimd_vdup_n_s8(15));
...
static int
test_easysimd_vqshl_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x4_t r = easysimd_vqshl_s16(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_s16(a, easysimd_vneg_s16(easysimd_vand_s16(easysimd_test_arm_neon_random_i16x4(), easysimd_vdup_n_s16(15))));
    b = easysimd_vand_s16(b, easysimd_vdup_n_s16(31));
...
static int
test_easysimd_vqshl_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x2_t r = easysimd_vqshl_s32(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_s32(a, easysimd_vneg_s32(easysimd_vand_s32(easysimd_test_arm_neon_random_i32x2(), easysimd_vdup_n_s32(31))));
    b = easysimd_vand_s32(b, easysimd_vdup_n_s32(63));
...
static int
test_easysimd_vqshl_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[1];
    int64_t b[1];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x1_t r = easysimd_vqshl_s64(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_s64(a, easysimd_vneg_s64(easysimd_vand_s64(easysimd_test_arm_neon_random_i64x1(), easysimd_vdup_n_s64(63))));
    b = easysimd_vand_s64(b, easysimd_vdup_n_s64(63));
...
static int
test_easysimd_vqshl_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[8];
    int8_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x8_t b = easysimd_vld1_s8(test_vec[i].b);
    easysimd_uint8x8_t r = easysimd_vqshl_u8(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_u8(a, easysimd_vneg_s8(easysimd_vand_s8(easysimd_test_arm_neon_random_i8x8(), easysimd_vdup_n_s8(7))));
    b = easysimd_vand_s8(b, easysimd_vdup_n_s8(15));
...
static int
test_easysimd_vqshl_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[4];
    int16_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x4_t b = easysimd_vld1_s16(test_vec[i].b);
    easysimd_uint16x4_t r = easysimd_vqshl_u16(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_u16(a, easysimd_vneg_s16(easysimd_vand_s16(easysimd_test_arm_neon_random_i16x4(), easysimd_vdup_n_s16(15))));
    b = easysimd_vand_s16(b, easysimd_vdup_n_s16(31));
...
static int
test_easysimd_vqshl_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[2];
    int32_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x2_t b = easysimd_vld1_s32(test_vec[i].b);
    easysimd_uint32x2_t r = easysimd_vqshl_u32(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_u32(a, easysimd_vneg_s32(easysimd_vand_s32(easysimd_test_arm_neon_random_i32x2(), easysimd_vdup_n_s32(31))));
    b = easysimd_vand_s32(b, easysimd_vdup_n_s32(63));
...
static int
test_easysimd_vqshl_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[1];
    int64_t b[1];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x1_t b = easysimd_vld1_s64(test_vec[i].b);
    easysimd_uint64x1_t r = easysimd_vqshl_u64(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshl_u64(a, easysimd_vneg_s64(easysimd_vand_s64(easysimd_test_arm_neon_random_i64x1(), easysimd_vdup_n_s64(63))));
    b = easysimd_vand_s64(b, easysimd_vdup_n_s64(63));
...
static int
test_easysimd_vqshlq_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[16];
    int8_t b[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16_t r = easysimd_vqshlq_s8(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_s8(a, easysimd_vnegq_s8(easysimd_vandq_s8(easysimd_test_arm_neon_random_i8x16(), easysimd_vdupq_n_s8(7))));
    b = easysimd_vandq_s8(b, easysimd_vdupq_n_s8(15));
...
static int
test_easysimd_vqshlq_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[8];
    int16_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8_t r = easysimd_vqshlq_s16(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_s16(a, easysimd_vnegq_s16(easysimd_vandq_s16(easysimd_test_arm_neon_random_i16x8(), easysimd_vdupq_n_s16(15))));
    b = easysimd_vandq_s16(b, easysimd_vdupq_n_s16(31));
...
static int
test_easysimd_vqshlq_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    int32_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t r = easysimd_vqshlq_s32(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_s32(a, easysimd_vnegq_s32(easysimd_vandq_s32(easysimd_test_arm_neon_random_i32x4(), easysimd_vdupq_n_s32(31))));
    b = easysimd_vandq_s32(b, easysimd_vdupq_n_s32(63));
...
static int
test_easysimd_vqshlq_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[2];
    int64_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x2_t r = easysimd_vqshlq_s64(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_s64(a, easysimd_vnegq_s64(easysimd_vandq_s64(easysimd_test_arm_neon_random_i64x2(), easysimd_vdupq_n_s64(63))));
    b = easysimd_vandq_s64(b, easysimd_vdupq_n_s64(63));
...
static int
test_easysimd_vqshlq_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[16];
    int8_t b[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16_t b = easysimd_vld1q_s8(test_vec[i].b);
    easysimd_uint8x16_t r = easysimd_vqshlq_u8(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_u8(a, easysimd_vnegq_s8(easysimd_vandq_s8(easysimd_test_arm_neon_random_i8x16(), easysimd_vdupq_n_s8(7))));
    b = easysimd_vandq_s8(b, easysimd_vdupq_n_s8(15));
...
static int
test_easysimd_vqshlq_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[8];
    int16_t b[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int16x8_t b = easysimd_vld1q_s16(test_vec[i].b);
    easysimd_uint16x8_t r = easysimd_vqshlq_u16(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_u16(a, easysimd_vnegq_s16(easysimd_vandq_s16(easysimd_test_arm_neon_random_i16x8(), easysimd_vdupq_n_s16(15))));
    b = easysimd_vandq_s16(b, easysimd_vdupq_n_s16(31));
...
static int
test_easysimd_vqshlq_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[4];
    int32_t b[4];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int32x4_t b = easysimd_vld1q_s32(test_vec[i].b);
    easysimd_uint32x4_t r = easysimd_vqshlq_u32(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_u32(a, easysimd_vnegq_s32(easysimd_vandq_s32(easysimd_test_arm_neon_random_i32x4(), easysimd_vdupq_n_s32(31))));
    b = easysimd_vandq_s32(b, easysimd_vdupq_n_s32(63));
...
static int
test_easysimd_vqshlq_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[2];
    int64_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int64x2_t b = easysimd_vld1q_s64(test_vec[i].b);
    easysimd_uint64x2_t r = easysimd_vqshlq_u64(a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    a = easysimd_vshlq_u64(a, easysimd_vnegq_s64(easysimd_vandq_s64(easysimd_test_arm_neon_random_i64x2(), easysimd_vdupq_n_s64(63))));
    b = easysimd_vandq_s64(b, easysimd_vdupq_n_s64(63));
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlb_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlh_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshls_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshld_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlb_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlh_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshls_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshld_u64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshl_u64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vqshlq_u64)
