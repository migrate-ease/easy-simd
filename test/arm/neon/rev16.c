#define EASYSIMD_TEST_ARM_NEON_INSN rev16

#include "test-neon.h"

/* Check that both of these work */
#if defined(__cplusplus)
  #include "../../../easysimd/arm/neon/rev16.h"
#else
  #include "../../../easysimd/arm/neon.h"
#endif

static int
test_easysimd_vrev16_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[8];
    int8_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x8_t r = easysimd_vrev16_s8(a);
...
static int
test_easysimd_vrev16_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[8];
    uint8_t r[8];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x8_t r = easysimd_vrev16_u8(a);
...
static int
test_easysimd_vrev16q_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[16];
    int8_t r[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_int8x16_t r = easysimd_vrev16q_s8(a);
...
static int
test_easysimd_vrev16q_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[16];
    uint8_t r[16];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    easysimd_uint8x16_t r = easysimd_vrev16q_u8(a);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vrev16_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vrev16_u8)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vrev16q_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vrev16q_u8)
