#define EASYSIMD_TEST_ARM_NEON_INSN st2_lane

#include "test-neon.h"
#include "../../../easysimd/arm/neon/st2_lane.h"

#if !defined(EASYSIMD_BUG_INTEL_857088)

static int
test_easysimd_vst2_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[2];
    int8_t val[2][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int8x8x2_t val = {
        {easysimd_vld1_s8(test_vec[i].val[0]), easysimd_vld1_s8(test_vec[i].val[1])}};
    int8_t a[2];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst2_lane_s8, HEDLEY_UNREACHABLE(),
  ...
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a[2];
    easysimd_int8x8x2_t val = easysimd_test_arm_neon_random_i8x8x2();
    const int lane = lanes[i];
    easysimd_vst2_lane_s8(a, val, lane);

    easysimd_test_codegen_write_vi8(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i8x8x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
static int
test_easysimd_vst2_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[2];
    int16_t val[2][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int16x4x2_t val = {
        {easysimd_vld1_s16(test_vec[i].val[0]), easysimd_vld1_s16(test_vec[i].val[1])}};
    int16_t a[2];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2_lane_s16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    int16_t a[2];
    easysimd_int16x4x2_t val = easysimd_test_arm_neon_random_i16x4x2();
    const int lane = lanes[i];
    easysimd_vst2_lane_s16(a, val, lane);

    easysimd_test_codegen_write_vi16(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x4x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t val[2][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int32x2x2_t val = {
        {easysimd_vld1_s32(test_vec[i].val[0]), easysimd_vld1_s32(test_vec[i].val[1])}};
    int32_t a[2];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2_lane_s32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int32_t a[2];
    easysimd_int32x2x2_t val = easysimd_test_arm_neon_random_i32x2x2();
    const int lane = lanes[i];
    easysimd_vst2_lane_s32(a, val, lane);

    easysimd_test_codegen_write_vi32(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x2x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[2];
    int64_t val[2][1];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int64x1x2_t val = {
        {easysimd_vld1_s64(test_vec[i].val[0]), easysimd_vld1_s64(test_vec[i].val[1])}};
    int64_t a[2];
    easysimd_vst2_lane_s64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int64_t a[2];
    easysimd_int64x1x2_t val = easysimd_test_arm_neon_random_i64x1x2();
    easysimd_vst2_lane_s64(a, val, 0);

    easysimd_test_codegen_write_vi64(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x1x2(2, val, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst2_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[2];
    uint8_t val[2][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint8x8x2_t val = {
        {easysimd_vld1_u8(test_vec[i].val[0]), easysimd_vld1_u8(test_vec[i].val[1])}};
    uint8_t a[2];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst2_lane_u8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    uint8_t a[2];
    easysimd_uint8x8x2_t val = easysimd_test_arm_neon_random_u8x8x2();
    const int lane = lanes[i];
    easysimd_vst2_lane_u8(a, val, lane);

    easysimd_test_codegen_write_vu8(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u8x8x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[2];
    uint16_t val[2][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint16x4x2_t val = {
        {easysimd_vld1_u16(test_vec[i].val[0]), easysimd_vld1_u16(test_vec[i].val[1])}};
    uint16_t a[2];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2_lane_u16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    uint16_t a[2];
    easysimd_uint16x4x2_t val = easysimd_test_arm_neon_random_u16x4x2();
    const int lane = lanes[i];
    easysimd_vst2_lane_u16(a, val, lane);

    easysimd_test_codegen_write_vu16(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x4x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[2];
    uint32_t val[2][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint32x2x2_t val = {
        {easysimd_vld1_u32(test_vec[i].val[0]), easysimd_vld1_u32(test_vec[i].val[1])}};
    uint32_t a[2];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2_lane_u32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint32_t a[2];
    easysimd_uint32x2x2_t val = easysimd_test_arm_neon_random_u32x2x2();
    const int lane = lanes[i];
    easysimd_vst2_lane_u32(a, val, lane);

    easysimd_test_codegen_write_vu32(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x2x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
static int
test_easysimd_vst2_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[2];
    uint64_t val[2][1];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint64x1x2_t val = {
        {easysimd_vld1_u64(test_vec[i].val[0]), easysimd_vld1_u64(test_vec[i].val[1])}};
    uint64_t a[2];
    easysimd_vst2_lane_u64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint64_t a[2];
    easysimd_uint64x1x2_t val = easysimd_test_arm_neon_random_u64x1x2();
    easysimd_vst2_lane_u64(a, val, 0);

    easysimd_test_codegen_write_vu64(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x1x2(2, val, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst2_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[2];
    easysimd_float32_t val[2][4];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(    72.16), EASYSIMD_FLOAT32_C(    51.97) },
        { { EASYSIMD_FLOAT32_C(    72.16), EASYSIMD_FLOAT32_C(   -27.20) },
          { EASYSIMD_FLOAT32_C(    51.97), EASYSIMD_FLOAT32_C(   -29.61) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT32_C(   -89.88), EASYSIMD_FLOAT32_C(    88.55) },
        { { EASYSIMD_FLOAT32_C(   -75.64), EASYSIMD_FLOAT32_C(   -89.88) },
          { EASYSIMD_FLOAT32_C(    52.86), EASYSIMD_FLOAT32_C(    88.55) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float32x2x2_t val = {
        {easysimd_vld1_f32(test_vec[i].val[0]), easysimd_vld1_f32(test_vec[i].val[1])}};
    easysimd_float32_t a[2];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2_lane_f32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32_t a[2];
    easysimd_float32x2x2_t val = easysimd_test_arm_neon_random_f32x2x2(-100.0f, 100.0f);
    const int lane = lanes[i];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2_lane_f32, HEDLEY_UNREACHABLE(),
                                lane, a, val);

    easysimd_test_codegen_write_vf32(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f32x2x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[2];
    easysimd_float64_t val[2][1];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(   -59.48), EASYSIMD_FLOAT64_C(    20.76) },
        { { EASYSIMD_FLOAT64_C(   -59.48) },
          { EASYSIMD_FLOAT64_C(    20.76) } },
    },
    { { EASYSIMD_FLOAT64_C(   -99.14), EASYSIMD_FLOAT64_C(   -99.40) },
        { { EASYSIMD_FLOAT64_C(   -99.14) },
          { EASYSIMD_FLOAT64_C(   -99.40) } },
    }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float64x1x2_t val = {
        {easysimd_vld1_f64(test_vec[i].val[0]), easysimd_vld1_f64(test_vec[i].val[1])}};
    easysimd_float64_t a[2];
    easysimd_vst2_lane_f64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64_t a[2];
    easysimd_float64x1x2_t val = easysimd_test_arm_neon_random_f64x1x2(-100.0, 100.0);
    easysimd_vst2_lane_f64(a, val, 0);

    easysimd_test_codegen_write_vf64(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f64x1x2(2, val, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst2q_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[2];
    int8_t val[2][16];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int8x16x2_t val = {
        {easysimd_vld1q_s8(test_vec[i].val[0]), easysimd_vld1q_s8(test_vec[i].val[1])}};
    int8_t a[2];
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst2q_lane_s8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_i8(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i8(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 16 ; i++) {
    int8_t a[2];
    easysimd_int8x16x2_t val = easysimd_test_arm_neon_random_i8x16x2();
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst2q_lane_s8, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vi8(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i8x16x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
static int
test_easysimd_vst2q_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[2];
    int16_t val[2][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int16x8x2_t val = {
        {easysimd_vld1q_s16(test_vec[i].val[0]), easysimd_vld1q_s16(test_vec[i].val[1])}};
    int16_t a[2];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst2q_lane_s16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_i16(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i16(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 4 ; i++) {
    int16_t a[2];
    easysimd_int16x8x2_t val = easysimd_test_arm_neon_random_i16x8x2();
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst2q_lane_s16, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vi16(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x8x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[2];
    int32_t val[2][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int32x4x2_t val = {
        {easysimd_vld1q_s32(test_vec[i].val[0]), easysimd_vld1q_s32(test_vec[i].val[1])}};
    int32_t a[2];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2q_lane_s32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_i32(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i32(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int32_t a[2];
    easysimd_int32x4x2_t val = easysimd_test_arm_neon_random_i32x4x2();
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2q_lane_s32, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vi32(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x4x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[2];
    int64_t val[2][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int64x2x2_t val = {
        {easysimd_vld1q_s64(test_vec[i].val[0]), easysimd_vld1q_s64(test_vec[i].val[1])}};
    int64_t a[2];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2q_lane_s64, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_i64(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i64(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int64_t a[2];
    easysimd_int64x2x2_t val = easysimd_test_arm_neon_random_i64x2x2();
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2q_lane_s64, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vi64(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x2x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[2];
    uint8_t val[2][16];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint8x16x2_t val = {
        {easysimd_vld1q_u8(test_vec[i].val[0]), easysimd_vld1q_u8(test_vec[i].val[1])}};
    uint8_t a[2];
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst2q_lane_u8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_u8(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u8(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 16 ; i++) {
    uint8_t a[2];
    easysimd_uint8x16x2_t val = easysimd_test_arm_neon_random_u8x16x2();
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst2q_lane_u8, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vu8(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u8x16x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[2];
    uint16_t val[2][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint16x8x2_t val = {
        {easysimd_vld1q_u16(test_vec[i].val[0]), easysimd_vld1q_u16(test_vec[i].val[1])}};
    uint16_t a[2];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst2q_lane_u16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_u16(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u16(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 4 ; i++) {
    uint16_t a[2];
    easysimd_uint16x8x2_t val = easysimd_test_arm_neon_random_u16x8x2();
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst2q_lane_u16, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vu16(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x8x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[2];
    uint32_t val[2][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint32x4x2_t val = {
        {easysimd_vld1q_u32(test_vec[i].val[0]), easysimd_vld1q_u32(test_vec[i].val[1])}};
    uint32_t a[2];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2q_lane_u32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_u32(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u32(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint32_t a[2];
    easysimd_uint32x4x2_t val = easysimd_test_arm_neon_random_u32x4x2();
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2q_lane_u32, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vu32(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x4x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[2];
    uint64_t val[2][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint64x2x2_t val = {
        {easysimd_vld1q_u64(test_vec[i].val[0]), easysimd_vld1q_u64(test_vec[i].val[1])}};
    uint64_t a[2];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2q_lane_u64, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_u64(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u64(a[1], test_vec[i].a[1]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint64_t a[2];
    easysimd_uint64x2x2_t val = easysimd_test_arm_neon_random_u64x2x2();
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2q_lane_u64, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vu64(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x2x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[2];
    easysimd_float32_t val[2][4];
    ...
    { { EASYSIMD_FLOAT32_C(    19.08), EASYSIMD_FLOAT32_C(   -83.93) },
        { { EASYSIMD_FLOAT32_C(    19.08), EASYSIMD_FLOAT32_C(    22.83), EASYSIMD_FLOAT32_C(   -68.21), EASYSIMD_FLOAT32_C(    37.78) },
          { EASYSIMD_FLOAT32_C(   -83.93), EASYSIMD_FLOAT32_C(    24.97), EASYSIMD_FLOAT32_C(    22.40), EASYSIMD_FLOAT32_C(   -46.93) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT32_C(    41.83), EASYSIMD_FLOAT32_C(   -27.39) },
        { { EASYSIMD_FLOAT32_C(    22.72), EASYSIMD_FLOAT32_C(    41.83), EASYSIMD_FLOAT32_C(    38.34), EASYSIMD_FLOAT32_C(   -34.59) },
          { EASYSIMD_FLOAT32_C(   -11.12), EASYSIMD_FLOAT32_C(   -27.39), EASYSIMD_FLOAT32_C(    65.58), EASYSIMD_FLOAT32_C(    49.02) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float32x4x2_t val = {
        {easysimd_vld1q_f32(test_vec[i].val[0]), easysimd_vld1q_f32(test_vec[i].val[1])}};
    easysimd_float32_t a[2];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2q_lane_f32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_f32(a[0], test_vec[i].a[0], 1);
    easysimd_assert_equal_f32(a[1], test_vec[i].a[1], 1);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32_t a[2];
    easysimd_float32x4x2_t val = easysimd_test_arm_neon_random_f32x4x2(-100.0f, 100.0f);
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst2q_lane_f32, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vf32(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f32x4x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst2q_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[2];
    easysimd_float64_t val[2][2];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(    53.06), EASYSIMD_FLOAT64_C(    71.80) },
        { { EASYSIMD_FLOAT64_C(    53.06), EASYSIMD_FLOAT64_C(   -60.06) },
          { EASYSIMD_FLOAT64_C(    71.80), EASYSIMD_FLOAT64_C(   -75.03) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT64_C(    29.19), EASYSIMD_FLOAT64_C(    37.77) },
        { { EASYSIMD_FLOAT64_C(   -29.33), EASYSIMD_FLOAT64_C(    29.19) },
          { EASYSIMD_FLOAT64_C(   -15.61), EASYSIMD_FLOAT64_C(    37.77) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float64x2x2_t val = {
        {easysimd_vld1q_f64(test_vec[i].val[0]), easysimd_vld1q_f64(test_vec[i].val[1])}};
    easysimd_float64_t a[2];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2q_lane_f64, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
    easysimd_assert_equal_f64(a[0], test_vec[i].a[0], 1);
    easysimd_assert_equal_f64(a[1], test_vec[i].a[1], 1);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64_t a[2];
    easysimd_float64x2x2_t val = easysimd_test_arm_neon_random_f64x2x2(-100.0, 100.0);
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst2q_lane_f64, HEDLEY_UNREACHABLE(),
                                i, a, val);

    easysimd_test_codegen_write_vf64(2, 2, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f64x2x2(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
#if !defined(EASYSIMD_BUG_INTEL_857088)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2_lane_f64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst2q_lane_f64)
#endif /* !defined(EASYSIMD_BUG_INTEL_857088) */
