#define EASYSIMD_TEST_ARM_NEON_INSN st3_lane

#include "test-neon.h"
#include "../../../easysimd/arm/neon/st3_lane.h"

#if !defined(EASYSIMD_BUG_INTEL_857088)

static int
test_easysimd_vst3_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[3];
    int8_t val[3][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int8x8x3_t val = {{easysimd_vld1_s8(test_vec[i].val[0]),
                             easysimd_vld1_s8(test_vec[i].val[1]),
                             easysimd_vld1_s8(test_vec[i].val[2])}};
    int8_t a[3];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst3_lane_s8, HEDLEY_UNREACHABLE(),
  ...
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a[3];
    easysimd_int8x8x3_t val = easysimd_test_arm_neon_random_i8x8x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_s8(a, val, lane);

    easysimd_test_codegen_write_vi8(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i8x8x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
static int
test_easysimd_vst3_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[3];
    int16_t val[3][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int16x4x3_t val = {{easysimd_vld1_s16(test_vec[i].val[0]),
                              easysimd_vld1_s16(test_vec[i].val[1]),
                              easysimd_vld1_s16(test_vec[i].val[2])}};
    int16_t a[3];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst3_lane_s16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    int16_t a[3];
    easysimd_int16x4x3_t val = easysimd_test_arm_neon_random_i16x4x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_s16(a, val, lane);

    easysimd_test_codegen_write_vi16(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x4x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[3];
    int32_t val[3][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int32x2x3_t val = {{easysimd_vld1_s32(test_vec[i].val[0]),
                              easysimd_vld1_s32(test_vec[i].val[1]),
                              easysimd_vld1_s32(test_vec[i].val[2])}};
    int32_t a[3];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3_lane_s32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int32_t a[3];
    easysimd_int32x2x3_t val = easysimd_test_arm_neon_random_i32x2x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_s32(a, val, lane);

    easysimd_test_codegen_write_vi32(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x2x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[3];
    int64_t val[3][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int64x1x3_t val = {{easysimd_vld1_s64(test_vec[i].val[0]),
                              easysimd_vld1_s64(test_vec[i].val[1]),
                              easysimd_vld1_s64(test_vec[i].val[2])}};
    int64_t a[3];
    easysimd_vst3_lane_s64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int64_t a[3];
    easysimd_int64x1x3_t val = easysimd_test_arm_neon_random_i64x1x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_s64(a, val, 0);

    easysimd_test_codegen_write_vi64(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x1x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[3];
    uint8_t val[3][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint8x8x3_t val = {{easysimd_vld1_u8(test_vec[i].val[0]),
                              easysimd_vld1_u8(test_vec[i].val[1]),
                              easysimd_vld1_u8(test_vec[i].val[2])}};
    uint8_t a[3];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst3_lane_u8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    uint8_t a[3];
    easysimd_uint8x8x3_t val = easysimd_test_arm_neon_random_u8x8x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_u8(a, val, lane);

    easysimd_test_codegen_write_vu8(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u8x8x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[3];
    uint16_t val[3][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint16x4x3_t val = {{easysimd_vld1_u16(test_vec[i].val[0]),
                               easysimd_vld1_u16(test_vec[i].val[1]),
                               easysimd_vld1_u16(test_vec[i].val[2])}};
    uint16_t a[3];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst3_lane_u16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    uint16_t a[3];
    easysimd_uint16x4x3_t val = easysimd_test_arm_neon_random_u16x4x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_u16(a, val, lane);

    easysimd_test_codegen_write_vu16(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x4x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[3];
    uint32_t val[3][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint32x2x3_t val = {{easysimd_vld1_u32(test_vec[i].val[0]),
                               easysimd_vld1_u32(test_vec[i].val[1]),
                               easysimd_vld1_u32(test_vec[i].val[2])}};
    uint32_t a[3];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3_lane_u32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint32_t a[3];
    easysimd_uint32x2x3_t val = easysimd_test_arm_neon_random_u32x2x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_u32(a, val, lane);

    easysimd_test_codegen_write_vu32(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x2x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
static int
test_easysimd_vst3_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[3];
    uint64_t val[3][1];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint64x1x3_t val = {{easysimd_vld1_u64(test_vec[i].val[0]),
                              easysimd_vld1_u64(test_vec[i].val[1]),
                              easysimd_vld1_u64(test_vec[i].val[2])}};
    uint64_t a[3];
    easysimd_vst3_lane_u64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint64_t a[3];
    easysimd_uint64x1x3_t val = easysimd_test_arm_neon_random_u64x1x3();
    const int lane = lanes[i];
    easysimd_vst3_lane_u64(a, val, 0);

    easysimd_test_codegen_write_vu64(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x1x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[3];
    easysimd_float32_t val[3][2];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(    23.12), EASYSIMD_FLOAT32_C(   -89.02), EASYSIMD_FLOAT32_C(   -58.84) },
        { { EASYSIMD_FLOAT32_C(    23.12), EASYSIMD_FLOAT32_C(   -86.41) },
          { EASYSIMD_FLOAT32_C(   -89.02), EASYSIMD_FLOAT32_C(    26.63) },
          { EASYSIMD_FLOAT32_C(   -58.84), EASYSIMD_FLOAT32_C(   -62.75) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT32_C(   -25.70), EASYSIMD_FLOAT32_C(    92.17), EASYSIMD_FLOAT32_C(    13.73) },
        { { EASYSIMD_FLOAT32_C(    73.20), EASYSIMD_FLOAT32_C(   -25.70) },
          { EASYSIMD_FLOAT32_C(    20.19), EASYSIMD_FLOAT32_C(    92.17) },
          { EASYSIMD_FLOAT32_C(   -17.26), EASYSIMD_FLOAT32_C(    13.73) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float32x2x3_t val = {{easysimd_vld1_f32(test_vec[i].val[0]),
                               easysimd_vld1_f32(test_vec[i].val[1]),
                               easysimd_vld1_f32(test_vec[i].val[2])}};
    easysimd_float32_t a[3];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3_lane_f32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32_t a[3];
    easysimd_float32x2x3_t val = easysimd_test_arm_neon_random_f32x2x3(-100.0f, 100.0f);
    const int lane = lanes[i];
    easysimd_vst3_lane_f32(a, val, lane);

    easysimd_test_codegen_write_vf32(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f32x2x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[3];
    easysimd_float64_t val[3][1];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(    99.90), EASYSIMD_FLOAT64_C(    75.46), EASYSIMD_FLOAT64_C(   -94.38) },
        { { EASYSIMD_FLOAT64_C(    99.90) },
          { EASYSIMD_FLOAT64_C(    75.46) },
          { EASYSIMD_FLOAT64_C(   -94.38) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT64_C(   -29.91), EASYSIMD_FLOAT64_C(    66.51), EASYSIMD_FLOAT64_C(   -36.58) },
        { { EASYSIMD_FLOAT64_C(   -29.91) },
          { EASYSIMD_FLOAT64_C(    66.51) },
          { EASYSIMD_FLOAT64_C(   -36.58) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float64x1x3_t val = {{easysimd_vld1_f64(test_vec[i].val[0]),
                              easysimd_vld1_f64(test_vec[i].val[1]),
                              easysimd_vld1_f64(test_vec[i].val[2])}};
    easysimd_float64_t a[3];
    easysimd_vst3_lane_f64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64_t a[3];
    easysimd_float64x1x3_t val = easysimd_test_arm_neon_random_f64x1x3(-100.0, 100.0);
    const int lane = lanes[i];
    easysimd_vst3_lane_f64(a, val, 0);

    easysimd_test_codegen_write_vf64(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f64x1x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[3];
    int8_t val[3][16];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int8x16x3_t val = {{easysimd_vld1q_s8(test_vec[i].val[0]),
                             easysimd_vld1q_s8(test_vec[i].val[1]),
                             easysimd_vld1q_s8(test_vec[i].val[2])}};
    int8_t a[3];
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst3q_lane_s8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a[3];
    easysimd_int8x16x3_t val = easysimd_test_arm_neon_random_i8x16x3();
    const int lane = lanes[i];
    easysimd_vst3q_lane_s8(a, val, lane);

    easysimd_test_codegen_write_vi8(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i8x16x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[3];
    int16_t val[3][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int16x8x3_t val = {{easysimd_vld1q_s16(test_vec[i].val[0]),
                              easysimd_vld1q_s16(test_vec[i].val[1]),
                              easysimd_vld1q_s16(test_vec[i].val[2])}};
    int16_t a[3];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst3q_lane_s16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    int16_t a[3];
    easysimd_int16x8x3_t val = easysimd_test_arm_neon_random_i16x8x3();
    const int lane = lanes[i];
    easysimd_vst3q_lane_s16(a, val, lane);

    easysimd_test_codegen_write_vi16(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x8x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[3];
    int32_t val[3][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int32x4x3_t val = {{easysimd_vld1q_s32(test_vec[i].val[0]),
                              easysimd_vld1q_s32(test_vec[i].val[1]),
                              easysimd_vld1q_s32(test_vec[i].val[2])}};
    int32_t a[3];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst3q_lane_s32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int32_t a[3];
    easysimd_int32x4x3_t val = easysimd_test_arm_neon_random_i32x4x3();
    const int lane = lanes[i];
    easysimd_vst3q_lane_s32(a, val, lane);

    easysimd_test_codegen_write_vi32(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x4x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[3];
    int64_t val[3][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int64x2x3_t val = {{easysimd_vld1q_s64(test_vec[i].val[0]),
                              easysimd_vld1q_s64(test_vec[i].val[1]),
                              easysimd_vld1q_s64(test_vec[i].val[2])}};
    int64_t a[3];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3q_lane_s64, HEDLEY_UNREACHABLE(), test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int64_t a[3];
    easysimd_int64x2x3_t val = easysimd_test_arm_neon_random_i64x2x3();
    const int lane = lanes[i];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3q_lane_s64, HEDLEY_UNREACHABLE(), lane, a, val);

    easysimd_test_codegen_write_vi64(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x2x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[3];
    uint8_t val[3][16];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint8x16x3_t val = {{easysimd_vld1q_u8(test_vec[i].val[0]),
                             easysimd_vld1q_u8(test_vec[i].val[1]),
                             easysimd_vld1q_u8(test_vec[i].val[2])}};
    uint8_t a[3];
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst3q_lane_u8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    uint8_t a[3];
    easysimd_uint8x16x3_t val = easysimd_test_arm_neon_random_u8x16x3();
    const int lane = lanes[i];
    easysimd_vst3q_lane_u8(a, val, lane);

    easysimd_test_codegen_write_vu8(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u8x16x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[3];
    uint16_t val[3][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint16x8x3_t val = {{easysimd_vld1q_u16(test_vec[i].val[0]),
                              easysimd_vld1q_u16(test_vec[i].val[1]),
                              easysimd_vld1q_u16(test_vec[i].val[2])}};
    uint16_t a[3];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst3q_lane_u16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    uint16_t a[3];
    easysimd_uint16x8x3_t val = easysimd_test_arm_neon_random_u16x8x3();
    const int lane = lanes[i];
    easysimd_vst3q_lane_u16(a, val, lane);

    easysimd_test_codegen_write_vu16(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x8x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[3];
    uint32_t val[3][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint32x4x3_t val = {{easysimd_vld1q_u32(test_vec[i].val[0]),
                              easysimd_vld1q_u32(test_vec[i].val[1]),
                              easysimd_vld1q_u32(test_vec[i].val[2])}};
    uint32_t a[3];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst3q_lane_u32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint32_t a[3];
    easysimd_uint32x4x3_t val = easysimd_test_arm_neon_random_u32x4x3();
    const int lane = lanes[i];
    easysimd_vst3q_lane_u32(a, val, lane);

    easysimd_test_codegen_write_vu32(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x4x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[3];
    uint64_t val[3][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint64x2x3_t val = {{easysimd_vld1q_u64(test_vec[i].val[0]),
                              easysimd_vld1q_u64(test_vec[i].val[1]),
                              easysimd_vld1q_u64(test_vec[i].val[2])}};
    uint64_t a[3];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3q_lane_u64, HEDLEY_UNREACHABLE(), test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint64_t a[3];
    easysimd_uint64x2x3_t val = easysimd_test_arm_neon_random_u64x2x3();
    const int lane = lanes[i];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3q_lane_u64, HEDLEY_UNREACHABLE(), lane, a, val);

    easysimd_test_codegen_write_vu64(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x2x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[3];
    easysimd_float32_t val[3][4];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(    52.99), EASYSIMD_FLOAT32_C(    86.46), EASYSIMD_FLOAT32_C(   -89.46) },
        { { EASYSIMD_FLOAT32_C(    52.99), EASYSIMD_FLOAT32_C(   -17.68), EASYSIMD_FLOAT32_C(    89.79), EASYSIMD_FLOAT32_C(   -47.89) },
          { EASYSIMD_FLOAT32_C(    86.46), EASYSIMD_FLOAT32_C(    86.93), EASYSIMD_FLOAT32_C(   -39.01), EASYSIMD_FLOAT32_C(    26.65) },
          { EASYSIMD_FLOAT32_C(   -89.46), EASYSIMD_FLOAT32_C(    -3.48), EASYSIMD_FLOAT32_C(    44.25), EASYSIMD_FLOAT32_C(    86.88) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT32_C(    80.11), EASYSIMD_FLOAT32_C(    59.12), EASYSIMD_FLOAT32_C(    37.68) },
        { { EASYSIMD_FLOAT32_C(    32.77), EASYSIMD_FLOAT32_C(    80.11), EASYSIMD_FLOAT32_C(   -33.81), EASYSIMD_FLOAT32_C(   -10.16) },
          { EASYSIMD_FLOAT32_C(    25.78), EASYSIMD_FLOAT32_C(    59.12), EASYSIMD_FLOAT32_C(    60.97), EASYSIMD_FLOAT32_C(    23.96) },
          { EASYSIMD_FLOAT32_C(    90.97), EASYSIMD_FLOAT32_C(    37.68), EASYSIMD_FLOAT32_C(    20.90), EASYSIMD_FLOAT32_C(   -85.54) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float32x4x3_t val = {{easysimd_vld1q_f32(test_vec[i].val[0]),
                               easysimd_vld1q_f32(test_vec[i].val[1]),
                               easysimd_vld1q_f32(test_vec[i].val[2])}};
    easysimd_float32_t a[3];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst3q_lane_f32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32_t a[3];
    easysimd_float32x4x3_t val = easysimd_test_arm_neon_random_f32x4x3(-100.0f, 100.0f);
    const int lane = lanes[i];
    easysimd_vst3q_lane_f32(a, val, lane);

    easysimd_test_codegen_write_vf32(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f32x4x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst3q_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[3];
    easysimd_float64_t val[3][2];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(    73.35), EASYSIMD_FLOAT64_C(   -88.49), EASYSIMD_FLOAT64_C(    36.60) },
        { { EASYSIMD_FLOAT64_C(    73.35), EASYSIMD_FLOAT64_C(   -12.64) },
          { EASYSIMD_FLOAT64_C(   -88.49), EASYSIMD_FLOAT64_C(    11.61) },
          { EASYSIMD_FLOAT64_C(    36.60), EASYSIMD_FLOAT64_C(   -46.96) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT64_C(    44.08), EASYSIMD_FLOAT64_C(    -5.73), EASYSIMD_FLOAT64_C(     9.48) },
        { { EASYSIMD_FLOAT64_C(   -78.09), EASYSIMD_FLOAT64_C(    44.08) },
          { EASYSIMD_FLOAT64_C(   -78.20), EASYSIMD_FLOAT64_C(    -5.73) },
          { EASYSIMD_FLOAT64_C(   -95.05), EASYSIMD_FLOAT64_C(     9.48) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float64x2x3_t val = {{easysimd_vld1q_f64(test_vec[i].val[0]),
                              easysimd_vld1q_f64(test_vec[i].val[1]),
                              easysimd_vld1q_f64(test_vec[i].val[2])}};
    easysimd_float64_t a[3];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3q_lane_f64, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64_t a[3];
    easysimd_float64x2x3_t val = easysimd_test_arm_neon_random_f64x2x3(-100.0, 100.0);
    const int lane = lanes[i];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst3q_lane_f64, HEDLEY_UNREACHABLE(),
                                lane, a, val);

    easysimd_test_codegen_write_vf64(2, 3, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f64x2x3(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
#if !defined(EASYSIMD_BUG_INTEL_857088)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3_lane_f64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst3q_lane_f64)
#endif /* !defined(EASYSIMD_BUG_INTEL_857088) */
