#define EASYSIMD_TEST_ARM_NEON_INSN st4_lane

#include "test-neon.h"
#include "../../../easysimd/arm/neon/st4_lane.h"
...
static int
test_easysimd_vst4_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[4];
    int8_t val[4][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int8x8x4_t val = {{easysimd_vld1_s8(test_vec[i].val[0]),
                             easysimd_vld1_s8(test_vec[i].val[1]),
                             easysimd_vld1_s8(test_vec[i].val[2]),
                             easysimd_vld1_s8(test_vec[i].val[3])}};
    int8_t a[4];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst4_lane_s8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_i8(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i8(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_i8(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_i8(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a[4];
    easysimd_int8x8x4_t val = easysimd_test_arm_neon_random_i8x8x4();
    const int lane = lanes[i];
    easysimd_vst4_lane_s8(a, val, lane);

    easysimd_test_codegen_write_vi8(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i8x8x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst4_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t val[4][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int16x4x4_t val = {{easysimd_vld1_s16(test_vec[i].val[0]),
                              easysimd_vld1_s16(test_vec[i].val[1]),
                              easysimd_vld1_s16(test_vec[i].val[2]),
                              easysimd_vld1_s16(test_vec[i].val[3])}};
    int16_t a[4];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst4_lane_s16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_i16(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i16(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_i16(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_i16(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 4 ; i++) {
    int16_t a[4];
    easysimd_int16x4x4_t val = easysimd_test_arm_neon_random_i16x4x4();
    const int lane = lanes[i];
    easysimd_vst4_lane_s16(a, val, lane);

    easysimd_test_codegen_write_vi16(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x4x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    int32_t val[4][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int32x2x4_t val = {{easysimd_vld1_s32(test_vec[i].val[0]),
                              easysimd_vld1_s32(test_vec[i].val[1]),
                              easysimd_vld1_s32(test_vec[i].val[2]),
                              easysimd_vld1_s32(test_vec[i].val[3])}};
    int32_t a[4];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst4_lane_s32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_i32(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i32(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_i32(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_i32(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int32_t a[4];
    easysimd_int32x2x4_t val = easysimd_test_arm_neon_random_i32x2x4();
    const int lane = lanes[i];
    easysimd_vst4_lane_s32(a, val, lane);

    easysimd_test_codegen_write_vi32(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x2x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[4];
    int64_t val[4][1];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int64x1x4_t val = {{easysimd_vld1_s64(test_vec[i].val[0]),
                              easysimd_vld1_s64(test_vec[i].val[1]),
                              easysimd_vld1_s64(test_vec[i].val[2]),
                              easysimd_vld1_s64(test_vec[i].val[3])}};
    int64_t a[4];
    easysimd_vst4_lane_s64(a, val, 0);

    easysimd_assert_equal_i64(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_i64(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_i64(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_i64(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int64_t a[4];
    easysimd_int64x1x4_t val = easysimd_test_arm_neon_random_i64x1x4();
    easysimd_vst4_lane_s64(a, val, 0);

    easysimd_test_codegen_write_vi64(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x1x4(2, val, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst4_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[4];
    uint8_t val[4][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint8x8x4_t val = {{easysimd_vld1_u8(test_vec[i].val[0]),
                              easysimd_vld1_u8(test_vec[i].val[1]),
                              easysimd_vld1_u8(test_vec[i].val[2]),
                              easysimd_vld1_u8(test_vec[i].val[3])}};
    uint8_t a[4];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst4_lane_u8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_u8(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u8(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_u8(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_u8(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 8 ; i++) {
    uint8_t a[4];
    easysimd_uint8x8x4_t val = easysimd_test_arm_neon_random_u8x8x4();
    const int lane = lanes[i];
    easysimd_vst4_lane_u8(a, val, lane);

    easysimd_test_codegen_write_vu8(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u8x8x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[4];
    uint16_t val[4][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint16x4x4_t val = {{easysimd_vld1_u16(test_vec[i].val[0]),
                               easysimd_vld1_u16(test_vec[i].val[1]),
                               easysimd_vld1_u16(test_vec[i].val[2]),
                               easysimd_vld1_u16(test_vec[i].val[3])}};
    uint16_t a[4];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst4_lane_u16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_u16(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u16(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_u16(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_u16(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 4 ; i++) {
    uint16_t a[4];
    easysimd_uint16x4x4_t val = easysimd_test_arm_neon_random_u16x4x4();
    const int lane = lanes[i];
    easysimd_vst4_lane_u16(a, val, lane);

    easysimd_test_codegen_write_vu16(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x4x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[4];
    uint32_t val[4][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint32x2x4_t val = {{easysimd_vld1_u32(test_vec[i].val[0]),
                               easysimd_vld1_u32(test_vec[i].val[1]),
                               easysimd_vld1_u32(test_vec[i].val[2]),
                               easysimd_vld1_u32(test_vec[i].val[3])}};
    uint32_t a[4];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst4_lane_u32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_u32(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u32(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_u32(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_u32(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint32_t a[4];
    easysimd_uint32x2x4_t val = easysimd_test_arm_neon_random_u32x2x4();
    const int lane = lanes[i];
    easysimd_vst4_lane_u32(a, val, lane);

    easysimd_test_codegen_write_vu32(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x2x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_u32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst4_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[4];
    uint64_t val[4][1];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint64x1x4_t val = {{easysimd_vld1_u64(test_vec[i].val[0]),
                              easysimd_vld1_u64(test_vec[i].val[1]),
                              easysimd_vld1_u64(test_vec[i].val[2]),
                              easysimd_vld1_u64(test_vec[i].val[3])}};
    uint64_t a[4];
    easysimd_vst4_lane_u64(a, val, 0);

    easysimd_assert_equal_u64(a[0], test_vec[i].a[0]);
    easysimd_assert_equal_u64(a[1], test_vec[i].a[1]);
    easysimd_assert_equal_u64(a[2], test_vec[i].a[2]);
    easysimd_assert_equal_u64(a[3], test_vec[i].a[3]);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint64_t a[4];
    easysimd_uint64x1x4_t val = easysimd_test_arm_neon_random_u64x1x4();
    easysimd_vst4_lane_u64(a, val, 0);

    easysimd_test_codegen_write_vu64(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x1x4(2, val, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst4_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[4];
    easysimd_float32_t val[4][2];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(   -16.19), EASYSIMD_FLOAT32_C(    92.30), EASYSIMD_FLOAT32_C(    70.06), EASYSIMD_FLOAT32_C(   -85.19) },
        { { EASYSIMD_FLOAT32_C(   -16.19), EASYSIMD_FLOAT32_C(    32.21) },
          { EASYSIMD_FLOAT32_C(    92.30), EASYSIMD_FLOAT32_C(    25.96) },
          { EASYSIMD_FLOAT32_C(    70.06), EASYSIMD_FLOAT32_C(   -58.97) },
          { EASYSIMD_FLOAT32_C(   -85.19), EASYSIMD_FLOAT32_C(    94.51) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT32_C(   -43.37), EASYSIMD_FLOAT32_C(     8.08), EASYSIMD_FLOAT32_C(   -14.46), EASYSIMD_FLOAT32_C(   -35.49) },
        { { EASYSIMD_FLOAT32_C(    73.37), EASYSIMD_FLOAT32_C(   -43.37) },
          { EASYSIMD_FLOAT32_C(   -77.70), EASYSIMD_FLOAT32_C(     8.08) },
          { EASYSIMD_FLOAT32_C(    69.71), EASYSIMD_FLOAT32_C(   -14.46) },
          { EASYSIMD_FLOAT32_C(   -36.48), EASYSIMD_FLOAT32_C(   -35.49) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float32x2x4_t val = {{easysimd_vld1_f32(test_vec[i].val[0]),
                               easysimd_vld1_f32(test_vec[i].val[1]),
                               easysimd_vld1_f32(test_vec[i].val[2]),
                               easysimd_vld1_f32(test_vec[i].val[3])}};
    easysimd_float32_t a[4];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst4_lane_f32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);

    easysimd_assert_equal_f32(a[0], test_vec[i].a[0], 1);
    easysimd_assert_equal_f32(a[1], test_vec[i].a[1], 1);
    easysimd_assert_equal_f32(a[2], test_vec[i].a[2], 1);
    easysimd_assert_equal_f32(a[3], test_vec[i].a[3], 1);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32_t a[4];
    easysimd_float32x2x4_t val = easysimd_test_arm_neon_random_f32x2x4(-100.0f, 100.0f);
    const int lane = lanes[i];
    easysimd_vst4_lane_f32(a, val, lane);

    easysimd_test_codegen_write_vf32(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f32x2x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[4];
    easysimd_float64_t val[4][1];
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(   -82.99), EASYSIMD_FLOAT64_C(   -60.99), EASYSIMD_FLOAT64_C(    50.39), EASYSIMD_FLOAT64_C(   -86.91) },
        { { EASYSIMD_FLOAT64_C(   -82.99) },
          { EASYSIMD_FLOAT64_C(   -60.99) },
          { EASYSIMD_FLOAT64_C(    50.39) },
          { EASYSIMD_FLOAT64_C(   -86.91) } },
    },
    { { EASYSIMD_FLOAT64_C(    -7.34), EASYSIMD_FLOAT64_C(    34.75), EASYSIMD_FLOAT64_C(   -85.55), EASYSIMD_FLOAT64_C(    68.92) },
        { { EASYSIMD_FLOAT64_C(    -7.34) },
          { EASYSIMD_FLOAT64_C(    34.75) },
          { EASYSIMD_FLOAT64_C(   -85.55) },
          { EASYSIMD_FLOAT64_C(    68.92) } },
    }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float64x1x4_t val = {{easysimd_vld1_f64(test_vec[i].val[0]),
                              easysimd_vld1_f64(test_vec[i].val[1]),
                              easysimd_vld1_f64(test_vec[i].val[2]),
                              easysimd_vld1_f64(test_vec[i].val[3])}};
    easysimd_float64_t a[4];
    easysimd_vst4_lane_f64(a, val, 0);

    easysimd_assert_equal_f64(a[0], test_vec[i].a[0], 1);
    easysimd_assert_equal_f64(a[1], test_vec[i].a[1], 1);
    easysimd_assert_equal_f64(a[2], test_vec[i].a[2], 1);
    easysimd_assert_equal_f64(a[3], test_vec[i].a[3], 1);
  }
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64_t a[4];
    easysimd_float64x1x4_t val = easysimd_test_arm_neon_random_f64x1x4(-100.0, 100.0);
    easysimd_vst4_lane_f64(a, val, 0);

    easysimd_test_codegen_write_vf64(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f64x1x4(2, val, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst4q_lane_s8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int8_t a[4];
    int8_t val[4][16];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int8x16x4_t val = {{easysimd_vld1q_s8(test_vec[i].val[0]),
                             easysimd_vld1q_s8(test_vec[i].val[1]),
                             easysimd_vld1q_s8(test_vec[i].val[2]),
                             easysimd_vld1q_s8(test_vec[i].val[3])}};
    int8_t a[4];
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst4q_lane_s8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    int8_t a[4];
    easysimd_int8x16x4_t val = easysimd_test_arm_neon_random_i8x16x4();
    const int lane = lanes[i];
    easysimd_vst4q_lane_s8(a, val, lane);

    easysimd_test_codegen_write_vi8(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i8x16x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_s16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int16_t a[4];
    int16_t val[4][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int16x8x4_t val = {{easysimd_vld1q_s16(test_vec[i].val[0]),
                              easysimd_vld1q_s16(test_vec[i].val[1]),
                              easysimd_vld1q_s16(test_vec[i].val[2]),
                              easysimd_vld1q_s16(test_vec[i].val[3])}};
    int16_t a[4];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst4q_lane_s16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    int16_t a[4];
    easysimd_int16x8x4_t val = easysimd_test_arm_neon_random_i16x8x4();
    const int lane = lanes[i];
    easysimd_vst4q_lane_s16(a, val, lane);

    easysimd_test_codegen_write_vi16(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i16x8x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_s32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int32_t a[4];
    int32_t val[4][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int32x4x4_t val = {{easysimd_vld1q_s32(test_vec[i].val[0]),
                              easysimd_vld1q_s32(test_vec[i].val[1]),
                              easysimd_vld1q_s32(test_vec[i].val[2]),
                              easysimd_vld1q_s32(test_vec[i].val[3])}};
    int32_t a[4];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst4q_lane_s32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int32_t a[4];
    easysimd_int32x4x4_t val = easysimd_test_arm_neon_random_i32x4x4();
    const int lane = lanes[i];
    easysimd_vst4q_lane_s32(a, val, lane);

    easysimd_test_codegen_write_vi32(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i32x4x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_s64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    int64_t a[4];
    int64_t val[4][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_int64x2x4_t val = {{easysimd_vld1q_s64(test_vec[i].val[0]),
                              easysimd_vld1q_s64(test_vec[i].val[1]),
                              easysimd_vld1q_s64(test_vec[i].val[2]),
                              easysimd_vld1q_s64(test_vec[i].val[3])}};
    int64_t a[4];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst4q_lane_s64, HEDLEY_UNREACHABLE(), test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    int64_t a[4];
    easysimd_int64x2x4_t val = easysimd_test_arm_neon_random_i64x2x4();
    easysimd_vst4q_lane_s64(a, val, i);

    easysimd_test_codegen_write_vi64(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_i64x2x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
}
...
static int
test_easysimd_vst4q_lane_u8 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint8_t a[4];
    uint8_t val[4][16];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint8x16x4_t val = {{easysimd_vld1q_u8(test_vec[i].val[0]),
                             easysimd_vld1q_u8(test_vec[i].val[1]),
                             easysimd_vld1q_u8(test_vec[i].val[2]),
                             easysimd_vld1q_u8(test_vec[i].val[3])}};
    uint8_t a[4];
    EASYSIMD_CONSTIFY_16_NO_RESULT_(easysimd_vst4q_lane_u8, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    uint8_t a[4];
    easysimd_uint8x16x4_t val = easysimd_test_arm_neon_random_u8x16x4();
    const int lane = lanes[i];
    easysimd_vst4q_lane_u8(a, val, lane);

    easysimd_test_codegen_write_vu8(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u8x16x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_u16 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint16_t a[4];
    uint16_t val[4][8];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint16x8x4_t val = {{easysimd_vld1q_u16(test_vec[i].val[0]),
                              easysimd_vld1q_u16(test_vec[i].val[1]),
                              easysimd_vld1q_u16(test_vec[i].val[2]),
                              easysimd_vld1q_u16(test_vec[i].val[3])}};
    uint16_t a[4];
    EASYSIMD_CONSTIFY_8_NO_RESULT_(easysimd_vst4q_lane_u16, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 4 ; i++) {
    uint16_t a[4];
    easysimd_uint16x8x4_t val = easysimd_test_arm_neon_random_u16x8x4();
    const int lane = lanes[i];
    easysimd_vst4q_lane_u16(a, val, lane);

    easysimd_test_codegen_write_vu16(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u16x8x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_u32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint32_t a[4];
    uint32_t val[4][4];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint32x4x4_t val = {{easysimd_vld1q_u32(test_vec[i].val[0]),
                              easysimd_vld1q_u32(test_vec[i].val[1]),
                              easysimd_vld1q_u32(test_vec[i].val[2]),
                              easysimd_vld1q_u32(test_vec[i].val[3])}};
    uint32_t a[4];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst4q_lane_u32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint32_t a[4];
    easysimd_uint32x4x4_t val = easysimd_test_arm_neon_random_u32x4x4();
    const int lane = lanes[i];
    easysimd_vst4q_lane_u32(a, val, lane);

    easysimd_test_codegen_write_vu32(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u32x4x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[4];
    uint64_t val[4][2];
  ...
  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_uint64x2x4_t val = {{easysimd_vld1q_u64(test_vec[i].val[0]),
                              easysimd_vld1q_u64(test_vec[i].val[1]),
                              easysimd_vld1q_u64(test_vec[i].val[2]),
                              easysimd_vld1q_u64(test_vec[i].val[3])}};
    uint64_t a[4];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst4q_lane_u64, HEDLEY_UNREACHABLE(), test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    uint64_t a[4];
    easysimd_uint64x2x4_t val = easysimd_test_arm_neon_random_u64x2x4();
    easysimd_vst4q_lane_u64(a, val, i);

    easysimd_test_codegen_write_vu64(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_u64x2x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_f32 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float32_t a[4];
    easysimd_float32_t val[4][4];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT32_C(    45.64), EASYSIMD_FLOAT32_C(   -12.54), EASYSIMD_FLOAT32_C(   -13.87), EASYSIMD_FLOAT32_C(   -54.45) },
        { { EASYSIMD_FLOAT32_C(    45.64), EASYSIMD_FLOAT32_C(    15.04), EASYSIMD_FLOAT32_C(    42.40), EASYSIMD_FLOAT32_C(   -29.53) },
          { EASYSIMD_FLOAT32_C(   -12.54), EASYSIMD_FLOAT32_C(   -11.31), EASYSIMD_FLOAT32_C(   -67.73), EASYSIMD_FLOAT32_C(    55.09) },
          { EASYSIMD_FLOAT32_C(   -13.87), EASYSIMD_FLOAT32_C(   -63.08), EASYSIMD_FLOAT32_C(    23.37), EASYSIMD_FLOAT32_C(     7.88) },
          { EASYSIMD_FLOAT32_C(   -54.45), EASYSIMD_FLOAT32_C(    -4.07), EASYSIMD_FLOAT32_C(   -42.90), EASYSIMD_FLOAT32_C(   -15.92) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT32_C(   -57.77), EASYSIMD_FLOAT32_C(    40.38), EASYSIMD_FLOAT32_C(    51.54), EASYSIMD_FLOAT32_C(    19.48) },
        { { EASYSIMD_FLOAT32_C(   -14.94), EASYSIMD_FLOAT32_C(   -57.77), EASYSIMD_FLOAT32_C(   -50.44), EASYSIMD_FLOAT32_C(    15.21) },
          { EASYSIMD_FLOAT32_C(     9.91), EASYSIMD_FLOAT32_C(    40.38), EASYSIMD_FLOAT32_C(   -60.19), EASYSIMD_FLOAT32_C(   -93.59) },
          { EASYSIMD_FLOAT32_C(   -66.94), EASYSIMD_FLOAT32_C(    51.54), EASYSIMD_FLOAT32_C(   -24.67), EASYSIMD_FLOAT32_C(   -20.06) },
          { EASYSIMD_FLOAT32_C(   -45.39), EASYSIMD_FLOAT32_C(    19.48), EASYSIMD_FLOAT32_C(    90.47), EASYSIMD_FLOAT32_C(   -99.75) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float32x4x4_t val = {{easysimd_vld1q_f32(test_vec[i].val[0]),
                               easysimd_vld1q_f32(test_vec[i].val[1]),
                               easysimd_vld1q_f32(test_vec[i].val[2]),
                               easysimd_vld1q_f32(test_vec[i].val[3])}};
    easysimd_float32_t a[4];
    EASYSIMD_CONSTIFY_4_NO_RESULT_(easysimd_vst4q_lane_f32, HEDLEY_UNREACHABLE(),
                                test_vec[i].lane, a, val);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float32_t a[4];
    easysimd_float32x4x4_t val = easysimd_test_arm_neon_random_f32x4x4(-100.0f, 100.0f);
    const int lane = lanes[i];
    easysimd_vst4q_lane_f32(a, val, lane);

    easysimd_test_codegen_write_vf32(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f32x4x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, lane, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
static int
test_easysimd_vst4q_lane_f64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    easysimd_float64_t a[4];
    easysimd_float64_t val[4][2];
    int lane;
  } test_vec[] = {
    { { EASYSIMD_FLOAT64_C(     6.25), EASYSIMD_FLOAT64_C(   -44.85), EASYSIMD_FLOAT64_C(    27.50), EASYSIMD_FLOAT64_C(    34.64) },
        { { EASYSIMD_FLOAT64_C(     6.25), EASYSIMD_FLOAT64_C(    43.57) },
          { EASYSIMD_FLOAT64_C(   -44.85), EASYSIMD_FLOAT64_C(   -84.14) },
          { EASYSIMD_FLOAT64_C(    27.50), EASYSIMD_FLOAT64_C(    77.26) },
          { EASYSIMD_FLOAT64_C(    34.64), EASYSIMD_FLOAT64_C(    -6.79) } },
       INT32_C(           0) },
    { { EASYSIMD_FLOAT64_C(   -30.77), EASYSIMD_FLOAT64_C(   -64.90), EASYSIMD_FLOAT64_C(   -72.21), EASYSIMD_FLOAT64_C(    75.84) },
        { { EASYSIMD_FLOAT64_C(   -30.77), EASYSIMD_FLOAT64_C(   -11.75) },
          { EASYSIMD_FLOAT64_C(   -64.90), EASYSIMD_FLOAT64_C(    26.58) },
          { EASYSIMD_FLOAT64_C(   -72.21), EASYSIMD_FLOAT64_C(    -5.43) },
          { EASYSIMD_FLOAT64_C(    75.84), EASYSIMD_FLOAT64_C(    10.83) } },
       INT32_C(           1) }
  };

  for (size_t i = 0; i < (sizeof(test_vec) / sizeof(test_vec[0])); i++) {
    easysimd_float64x2x4_t val = {{easysimd_vld1q_f64(test_vec[i].val[0]),
                              easysimd_vld1q_f64(test_vec[i].val[1]),
                              easysimd_vld1q_f64(test_vec[i].val[2]),
                              easysimd_vld1q_f64(test_vec[i].val[3])}};
    easysimd_float64_t a[4];
    EASYSIMD_CONSTIFY_2_NO_RESULT_(easysimd_vst4q_lane_f64, HEDLEY_UNREACHABLE(), test_vec[i].lane, a, val);
    easysimd_vst4q_lane_f64(a, val, 0);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    easysimd_float64_t a[4];
    easysimd_float64x2x4_t val = easysimd_test_arm_neon_random_f64x2x4(-100.0, 100.0);
    easysimd_vst4q_lane_f64(a, val, i);

    easysimd_test_codegen_write_vf64(2, 4, a, EASYSIMD_TEST_VEC_POS_FIRST);
    easysimd_test_arm_neon_write_f64x2x4(2, val, EASYSIMD_TEST_VEC_POS_MIDDLE);
    easysimd_test_codegen_write_i32(2, i, EASYSIMD_TEST_VEC_POS_LAST);
  }
  return 1;
#endif
...
#if !defined(EASYSIMD_BUG_INTEL_857088)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4_lane_f64)

EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_s8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_s16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_s32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_s64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_u8)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_u16)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_u32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_u64)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_f32)
EASYSIMD_TEST_FUNC_LIST_ENTRY(vst4q_lane_f64)
#endif /* !defined(EASYSIMD_BUG_INTEL_857088) */
