#define EASYSIMD_TEST_ARM_NEON_INSN xar

#include "test-neon.h"
#include "../../../easysimd/arm/neon/xar.h"

static int
test_easysimd_vxarq_u64 (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const struct {
    uint64_t a[2];
    uint64_t b[2];
  ...
  for (size_t i = 0 ; i < (sizeof(test_vec) / sizeof(test_vec[0])) ; i++) {
    ...
    EASYSIMD_CONSTIFY_64_(easysimd_vxarq_u64, r, (HEDLEY_UNREACHABLE(), r), test_vec[i].d, a, b);
  ...
  for (int i = 0 ; i < 8 ; i++) {
    ...
    const int d = easysimd_math_abs(easysimd_test_codegen_random_i32() % 64);
    easysimd_uint64x2_t r;
    EASYSIMD_CONSTIFY_64_(easysimd_vxarq_u64, r, (HEDLEY_UNREACHABLE(), r), d, a, b);
    ...
    easysimd_test_codegen_write_i32(2, d, EASYSIMD_TEST_VEC_POS_MIDDLE);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(vxarq_u64)
