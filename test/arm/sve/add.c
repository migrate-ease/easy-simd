#define EASYSIMD_TEST_ARM_SVE_INSN add

#include "test-sve.h"
#include "../../../easysimd/arm/sve/add.h"

#include "../../../easysimd/arm/sve/cmplt.h"

static int
test_easysimd_svadd_s8_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  ...
  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b8_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    easysimd_svint8_t
      rv,
      av = easysimd_svld1_s8(pg, &(a[i])),
      bv = easysimd_svld1_s8(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_s8_z(pv, av, bv);

    easysimd_svst1_s8(pg, &(r[i]), rv);

    i += easysimd_svcntb();
    pg = easysimd_svwhilelt_b8_s32(i, len);
  }

  easysimd_assert_equal_vi8(len, e, r);

  return 0;
#else
  int8_t p[1024], a[1024], b[1024], e[1024];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));

  fputc('\n', stdout);

  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  easysimd_test_codegen_random_memory(sizeof(b), HEDLEY_REINTERPRET_CAST(uint8_t*, b));

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b8_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    easysimd_svint8_t av, bv, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(0));
    av = easysimd_svld1_s8(pg, &(a[i]));
    bv = easysimd_svld1_s8(pg, &(b[i]));

    ev = easysimd_svadd_s8_z(pv, av, bv);

    easysimd_svst1_s8(pg, &(e[i]), ev);
    easysimd_svst1_s8(pg, &(p[i]), easysimd_svdup_n_s8_z(pv, ~INT8_C(0)));

    i += easysimd_svcntb();
    pg = easysimd_svwhilelt_b8_s32(i, len);
  }

  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_s8_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_s8_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    ev = easysimd_svadd_s8_m(pv, av, bv);
...
static int
test_easysimd_svadd_s16_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  ...
  static const int32_t len = HEDLEY_STATIC_CAST(int32_t, sizeof(e) / sizeof(e[0]));
  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b16_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    easysimd_svint16_t
      rv,
      av = easysimd_svld1_s16(pg, &(a[i])),
      bv = easysimd_svld1_s16(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_s16_z(pv, av, bv);

    easysimd_svst1_s16(pg, &(r[i]), rv);

    i += easysimd_svcnth();
    pg = easysimd_svwhilelt_b16_s32(i, len);
  }

  easysimd_assert_equal_vi16(len, e, r);
  ...
  easysimd_svbool_t pg = easysimd_svwhilelt_b16_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    easysimd_svint16_t av, bv, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(0));
    av = easysimd_svld1_s16(pg, &(a[i]));
    bv = easysimd_svld1_s16(pg, &(b[i]));

    ev = easysimd_svadd_s16_z(pv, av, bv);

    easysimd_svst1_s16(pg, &(e[i]), ev);
    easysimd_svst1_s16(pg, &(p[i]), easysimd_svdup_n_s16_z(pv, ~INT16_C(0)));

    i += easysimd_svcnth();
    pg = easysimd_svwhilelt_b16_s32(i, len);
  }

  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_s16_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_s16_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    ev = easysimd_svadd_s16_m(pv, av, bv);
...
static int
test_easysimd_svadd_s32_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  ...
  static const int32_t len = HEDLEY_STATIC_CAST(int32_t, sizeof(e) / sizeof(e[0]));
  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svint32_t
      rv,
      av = easysimd_svld1_s32(pg, &(a[i])),
      bv = easysimd_svld1_s32(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_s32_z(pv, av, bv);

    easysimd_svst1_s32(pg, &(r[i]), rv);

    i += easysimd_svcntw();
    pg = easysimd_svwhilelt_b32_s32(i, len);
  }

  easysimd_assert_equal_vi32(len, e, r);
  ...
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svint32_t av, bv, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(0));
    av = easysimd_svld1_s32(pg, &(a[i]));
    bv = easysimd_svld1_s32(pg, &(b[i]));

    ev = easysimd_svadd_s32_z(pv, av, bv);

    easysimd_svst1_s32(pg, &(e[i]), ev);
    easysimd_svst1_s32(pg, &(p[i]), easysimd_svdup_n_s32_z(pv, ~INT32_C(0)));

    i += easysimd_svcntw();
    pg = easysimd_svwhilelt_b32_s32(i, len);
  }

  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_s32_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_s32_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_s32_m(pv, av, bv);
...
static int
test_easysimd_svadd_s64_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  ...
  static const int32_t len = HEDLEY_STATIC_CAST(int32_t, sizeof(e) / sizeof(e[0]));
  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svint64_t
      rv,
      av = easysimd_svld1_s64(pg, &(a[i])),
      bv = easysimd_svld1_s64(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_s64_z(pv, av, bv);

    easysimd_svst1_s64(pg, &(r[i]), rv);

    i += easysimd_svcntd();
    pg = easysimd_svwhilelt_b64_s32(i, len);
  }

  easysimd_assert_equal_vi64(len, e, r);
  ...
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svint64_t av, bv, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(0));
    av = easysimd_svld1_s64(pg, &(a[i]));
    bv = easysimd_svld1_s64(pg, &(b[i]));

    ev = easysimd_svadd_s64_z(pv, av, bv);

    easysimd_svst1_s64(pg, &(e[i]), ev);
    easysimd_svst1_s64(pg, &(p[i]), easysimd_svdup_n_s64_z(pv, ~INT64_C(0)));

    i += easysimd_svcntd();
    pg = easysimd_svwhilelt_b64_s32(i, len);
  }

  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_s64_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_s64_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_s64_m(pv, av, bv);
...
static int
test_easysimd_svadd_u8_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    easysimd_svuint8_t
      rv,
      av = easysimd_svld1_u8(pg, &(a[i])),
      bv = easysimd_svld1_u8(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_u8_z(pv, av, bv);

    easysimd_svst1_u8(pg, &(r[i]), rv);

    i += easysimd_svcntb();
    pg = easysimd_svwhilelt_b8_s32(i, len);
  }

  easysimd_assert_equal_vu8(len, e, r);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    ev = easysimd_svadd_u8_z(pv, av, bv);

    easysimd_svst1_u8(pg, &(e[i]), ev);
    easysimd_svst1_s8(pg, &(p[i]), easysimd_svdup_n_s8_z(pv, ~INT8_C(0)));
  ...
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu8(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vu8(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vu8(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_u8_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_u8_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    ev = easysimd_svadd_u8_m(pv, av, bv);
...
static int
test_easysimd_svadd_u16_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
    {  INT16_C(     0), -INT16_C(     1),  INT16_C(     0),  INT16_C(     0), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    easysimd_svuint16_t
      rv,
      av = easysimd_svld1_u16(pg, &(a[i])),
      bv = easysimd_svld1_u16(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_u16_z(pv, av, bv);

    easysimd_svst1_u16(pg, &(r[i]), rv);

    i += easysimd_svcnth();
    pg = easysimd_svwhilelt_b16_s32(i, len);
  }

  easysimd_assert_equal_vu16(len, e, r);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    ev = easysimd_svadd_u16_z(pv, av, bv);

    easysimd_svst1_u16(pg, &(e[i]), ev);
    easysimd_svst1_s16(pg, &(p[i]), easysimd_svdup_n_s16_z(pv, ~INT16_C(0)));
  ...
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu16(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vu16(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vu16(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_u16_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
    {  INT16_C(     0),  INT16_C(     0),  INT16_C(     0),  INT16_C(     0), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_u16_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    ev = easysimd_svadd_u16_m(pv, av, bv);
...
static int
test_easysimd_svadd_u32_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svuint32_t
      rv,
      av = easysimd_svld1_u32(pg, &(a[i])),
      bv = easysimd_svld1_u32(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_u32_z(pv, av, bv);

    easysimd_svst1_u32(pg, &(r[i]), rv);

    i += easysimd_svcntw();
    pg = easysimd_svwhilelt_b32_s32(i, len);
  }

  easysimd_assert_equal_vu32(len, e, r);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_u32_z(pv, av, bv);

    easysimd_svst1_u32(pg, &(e[i]), ev);
    easysimd_svst1_s32(pg, &(p[i]), easysimd_svdup_n_s32_z(pv, ~INT32_C(0)));
  ...
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu32(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vu32(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vu32(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_u32_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_u32_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_u32_m(pv, av, bv);
...
static int
test_easysimd_svadd_u64_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    {  INT64_C(                   0),  INT64_C(                   0),  INT64_C(                   0), -INT64_C(                   1),
       INT64_C(                   0),  INT64_C(                   0),  INT64_C(                   0), -INT64_C(                   1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svuint64_t
      rv,
      av = easysimd_svld1_u64(pg, &(a[i])),
      bv = easysimd_svld1_u64(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_u64_z(pv, av, bv);

    easysimd_svst1_u64(pg, &(r[i]), rv);

    i += easysimd_svcntd();
    pg = easysimd_svwhilelt_b64_s32(i, len);
  }

  easysimd_assert_equal_vu64(len, e, r);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_u64_z(pv, av, bv);

    easysimd_svst1_u64(pg, &(e[i]), ev);
    easysimd_svst1_s64(pg, &(p[i]), easysimd_svdup_n_s64_z(pv, ~INT64_C(0)));
  ...
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu64(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vu64(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vu64(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_u64_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_u64_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_u64_m(pv, av, bv);
...
static int
test_easysimd_svadd_f32_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
    {  INT32_C(           0),  INT32_C(           0), -INT32_C(           1), -INT32_C(           1), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0),
      ...
      -INT32_C(           1),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1), -INT32_C(           1),
       ...
       INT32_C(           0) };
  static const easysimd_float32 a[] =
    { EASYSIMD_FLOAT32_C(   570.54), EASYSIMD_FLOAT32_C(   320.87), EASYSIMD_FLOAT32_C(   703.29), EASYSIMD_FLOAT32_C(  -583.55),
      EASYSIMD_FLOAT32_C(   -40.68), EASYSIMD_FLOAT32_C(  -771.90), EASYSIMD_FLOAT32_C(  -624.45), EASYSIMD_FLOAT32_C(  -418.62),
      EASYSIMD_FLOAT32_C(  -342.79), EASYSIMD_FLOAT32_C(  -902.29), EASYSIMD_FLOAT32_C(   -40.10), EASYSIMD_FLOAT32_C(  -214.24),
      EASYSIMD_FLOAT32_C(   -66.44), EASYSIMD_FLOAT32_C(   766.13), EASYSIMD_FLOAT32_C(   189.56), EASYSIMD_FLOAT32_C(  -670.36),
      EASYSIMD_FLOAT32_C(  -300.11), EASYSIMD_FLOAT32_C(   405.33), EASYSIMD_FLOAT32_C(   434.25), EASYSIMD_FLOAT32_C(   830.27),
      EASYSIMD_FLOAT32_C(   -19.06), EASYSIMD_FLOAT32_C(   300.49), EASYSIMD_FLOAT32_C(  -841.78), EASYSIMD_FLOAT32_C(  -360.76),
      EASYSIMD_FLOAT32_C(  -995.84), EASYSIMD_FLOAT32_C(  -766.33), EASYSIMD_FLOAT32_C(   581.45), EASYSIMD_FLOAT32_C(  -828.44),
      EASYSIMD_FLOAT32_C(  -842.94), EASYSIMD_FLOAT32_C(  -687.68), EASYSIMD_FLOAT32_C(  -735.83), EASYSIMD_FLOAT32_C(   727.60),
      EASYSIMD_FLOAT32_C(   633.19), EASYSIMD_FLOAT32_C(   967.46), EASYSIMD_FLOAT32_C(  -855.96), EASYSIMD_FLOAT32_C(  -407.49),
      EASYSIMD_FLOAT32_C(  -804.44), EASYSIMD_FLOAT32_C(  -480.41), EASYSIMD_FLOAT32_C(   173.89), EASYSIMD_FLOAT32_C(  -147.23),
      EASYSIMD_FLOAT32_C(  -382.69), EASYSIMD_FLOAT32_C(  -866.21), EASYSIMD_FLOAT32_C(   638.54), EASYSIMD_FLOAT32_C(   550.86),
      EASYSIMD_FLOAT32_C(   899.92), EASYSIMD_FLOAT32_C(  -171.91), EASYSIMD_FLOAT32_C(   880.50), EASYSIMD_FLOAT32_C(  -400.19),
      EASYSIMD_FLOAT32_C(  -766.58), EASYSIMD_FLOAT32_C(   314.75), EASYSIMD_FLOAT32_C(  -569.92), EASYSIMD_FLOAT32_C(   214.35),
      EASYSIMD_FLOAT32_C(  -384.76), EASYSIMD_FLOAT32_C(  -411.70), EASYSIMD_FLOAT32_C(   853.60), EASYSIMD_FLOAT32_C(  -380.60),
      EASYSIMD_FLOAT32_C(  -178.03), EASYSIMD_FLOAT32_C(   435.05), EASYSIMD_FLOAT32_C(  -209.04), EASYSIMD_FLOAT32_C(   -20.97),
      EASYSIMD_FLOAT32_C(   747.37), EASYSIMD_FLOAT32_C(    55.13), EASYSIMD_FLOAT32_C(  -293.37), EASYSIMD_FLOAT32_C(   380.56),
      EASYSIMD_FLOAT32_C(    22.59), EASYSIMD_FLOAT32_C(  -149.32), EASYSIMD_FLOAT32_C(   973.08), EASYSIMD_FLOAT32_C(   218.15),
      EASYSIMD_FLOAT32_C(   370.27), EASYSIMD_FLOAT32_C(   146.97), EASYSIMD_FLOAT32_C(  -929.08), EASYSIMD_FLOAT32_C(   987.58),
      EASYSIMD_FLOAT32_C(   280.75), EASYSIMD_FLOAT32_C(   709.46), EASYSIMD_FLOAT32_C(   538.44), EASYSIMD_FLOAT32_C(   180.67),
      EASYSIMD_FLOAT32_C(  -462.45), EASYSIMD_FLOAT32_C(   418.94), EASYSIMD_FLOAT32_C(   780.48), EASYSIMD_FLOAT32_C(  -229.03),
      EASYSIMD_FLOAT32_C(  -266.31), EASYSIMD_FLOAT32_C(  -789.44), EASYSIMD_FLOAT32_C(   985.32), EASYSIMD_FLOAT32_C(   348.93),
      EASYSIMD_FLOAT32_C(  -201.14), EASYSIMD_FLOAT32_C(   838.92), EASYSIMD_FLOAT32_C(   968.33), EASYSIMD_FLOAT32_C(   620.83),
      EASYSIMD_FLOAT32_C(   273.97), EASYSIMD_FLOAT32_C(  -240.71), EASYSIMD_FLOAT32_C(  -400.14), EASYSIMD_FLOAT32_C(    21.34),
      EASYSIMD_FLOAT32_C(   814.42), EASYSIMD_FLOAT32_C(   306.49), EASYSIMD_FLOAT32_C(  -598.10), EASYSIMD_FLOAT32_C(  -162.99),
      EASYSIMD_FLOAT32_C(  -842.83), EASYSIMD_FLOAT32_C(  -625.02), EASYSIMD_FLOAT32_C(  -944.84), EASYSIMD_FLOAT32_C(   527.44),
      EASYSIMD_FLOAT32_C(   521.95), EASYSIMD_FLOAT32_C(  -873.92), EASYSIMD_FLOAT32_C(   515.01), EASYSIMD_FLOAT32_C(  -197.30),
      EASYSIMD_FLOAT32_C(   835.54), EASYSIMD_FLOAT32_C(    53.45), EASYSIMD_FLOAT32_C(   983.37), EASYSIMD_FLOAT32_C(  -626.91),
      EASYSIMD_FLOAT32_C(  -527.61), EASYSIMD_FLOAT32_C(   763.85), EASYSIMD_FLOAT32_C(   144.06), EASYSIMD_FLOAT32_C(   206.08),
      EASYSIMD_FLOAT32_C(   974.40), EASYSIMD_FLOAT32_C(   129.38), EASYSIMD_FLOAT32_C(  -444.99), EASYSIMD_FLOAT32_C(  -226.74),
      EASYSIMD_FLOAT32_C(   -31.70), EASYSIMD_FLOAT32_C(  -476.66), EASYSIMD_FLOAT32_C(  -605.91), EASYSIMD_FLOAT32_C(  -757.73),
      EASYSIMD_FLOAT32_C(   282.63), EASYSIMD_FLOAT32_C(    -6.06), EASYSIMD_FLOAT32_C(   263.61), EASYSIMD_FLOAT32_C(    97.05),
      EASYSIMD_FLOAT32_C(  -699.57), EASYSIMD_FLOAT32_C(   665.51), EASYSIMD_FLOAT32_C(   934.06), EASYSIMD_FLOAT32_C(  -542.40),
      EASYSIMD_FLOAT32_C(  -959.50), EASYSIMD_FLOAT32_C(   989.21), EASYSIMD_FLOAT32_C(   985.04), EASYSIMD_FLOAT32_C(   562.44),
      EASYSIMD_FLOAT32_C(  -884.70), EASYSIMD_FLOAT32_C(   500.05), EASYSIMD_FLOAT32_C(  -634.86), EASYSIMD_FLOAT32_C(   950.83),
      EASYSIMD_FLOAT32_C(  -446.50), EASYSIMD_FLOAT32_C(  -651.49), EASYSIMD_FLOAT32_C(  -676.07), EASYSIMD_FLOAT32_C(    25.89),
      EASYSIMD_FLOAT32_C(  -887.64), EASYSIMD_FLOAT32_C(   467.98), EASYSIMD_FLOAT32_C(  -768.03), EASYSIMD_FLOAT32_C(  -913.24),
      EASYSIMD_FLOAT32_C(  -402.63), EASYSIMD_FLOAT32_C(  -213.01), EASYSIMD_FLOAT32_C(  -139.98), EASYSIMD_FLOAT32_C(   565.67),
      EASYSIMD_FLOAT32_C(   310.33), EASYSIMD_FLOAT32_C(   254.11), EASYSIMD_FLOAT32_C(   807.94), EASYSIMD_FLOAT32_C(  -407.04),
      EASYSIMD_FLOAT32_C(  -751.94), EASYSIMD_FLOAT32_C(    71.54), EASYSIMD_FLOAT32_C(   690.00), EASYSIMD_FLOAT32_C(  -451.51),
      EASYSIMD_FLOAT32_C(  -262.94), EASYSIMD_FLOAT32_C(   624.06), EASYSIMD_FLOAT32_C(     6.09), EASYSIMD_FLOAT32_C(  -222.45),
      EASYSIMD_FLOAT32_C(   613.27), EASYSIMD_FLOAT32_C(    -8.87), EASYSIMD_FLOAT32_C(  -660.00), EASYSIMD_FLOAT32_C(   728.57),
      EASYSIMD_FLOAT32_C(  -508.82), EASYSIMD_FLOAT32_C(  -294.86), EASYSIMD_FLOAT32_C(   679.40), EASYSIMD_FLOAT32_C(    44.68),
      EASYSIMD_FLOAT32_C(    53.65), EASYSIMD_FLOAT32_C(  -996.67), EASYSIMD_FLOAT32_C(  -929.43), EASYSIMD_FLOAT32_C(   166.01),
      EASYSIMD_FLOAT32_C(   471.31), EASYSIMD_FLOAT32_C(  -697.46), EASYSIMD_FLOAT32_C(   252.78), EASYSIMD_FLOAT32_C(  -931.32),
      EASYSIMD_FLOAT32_C(    89.53), EASYSIMD_FLOAT32_C(  -887.20), EASYSIMD_FLOAT32_C(   634.35), EASYSIMD_FLOAT32_C(  -600.15),
      EASYSIMD_FLOAT32_C(   366.91), EASYSIMD_FLOAT32_C(   442.28), EASYSIMD_FLOAT32_C(    -7.19), EASYSIMD_FLOAT32_C(   614.97),
      EASYSIMD_FLOAT32_C(  -486.17), EASYSIMD_FLOAT32_C(  -317.18), EASYSIMD_FLOAT32_C(  -836.54), EASYSIMD_FLOAT32_C(   250.88),
      EASYSIMD_FLOAT32_C(  -693.13), EASYSIMD_FLOAT32_C(   169.55), EASYSIMD_FLOAT32_C(  -971.57), EASYSIMD_FLOAT32_C(   920.15),
      EASYSIMD_FLOAT32_C(  -839.32), EASYSIMD_FLOAT32_C(  -631.57), EASYSIMD_FLOAT32_C(   648.72), EASYSIMD_FLOAT32_C(  -348.14),
      EASYSIMD_FLOAT32_C(    73.57), EASYSIMD_FLOAT32_C(   328.12), EASYSIMD_FLOAT32_C(   696.53), EASYSIMD_FLOAT32_C(  -872.78),
      EASYSIMD_FLOAT32_C(   331.45), EASYSIMD_FLOAT32_C(   767.10), EASYSIMD_FLOAT32_C(   293.24), EASYSIMD_FLOAT32_C(  -197.24),
      EASYSIMD_FLOAT32_C(  -930.36), EASYSIMD_FLOAT32_C(  -453.98), EASYSIMD_FLOAT32_C(  -128.56), EASYSIMD_FLOAT32_C(   159.17),
      EASYSIMD_FLOAT32_C(  -341.18), EASYSIMD_FLOAT32_C(  -494.21), EASYSIMD_FLOAT32_C(   559.02), EASYSIMD_FLOAT32_C(  -974.27),
      EASYSIMD_FLOAT32_C(   948.07), EASYSIMD_FLOAT32_C(  -448.17), EASYSIMD_FLOAT32_C(   640.70), EASYSIMD_FLOAT32_C(  -538.11),
      EASYSIMD_FLOAT32_C(   234.65) };
  static const easysimd_float32 b[] =
    { EASYSIMD_FLOAT32_C(   804.16), EASYSIMD_FLOAT32_C(   712.78), EASYSIMD_FLOAT32_C(   541.52), EASYSIMD_FLOAT32_C(   -26.29),
      EASYSIMD_FLOAT32_C(   741.21), EASYSIMD_FLOAT32_C(   461.67), EASYSIMD_FLOAT32_C(   134.39), EASYSIMD_FLOAT32_C(  -890.36),
      EASYSIMD_FLOAT32_C(   110.39), EASYSIMD_FLOAT32_C(   786.24), EASYSIMD_FLOAT32_C(   183.21), EASYSIMD_FLOAT32_C(  -561.50),
      EASYSIMD_FLOAT32_C(   482.77), EASYSIMD_FLOAT32_C(   310.44), EASYSIMD_FLOAT32_C(   769.95), EASYSIMD_FLOAT32_C(   249.87),
      EASYSIMD_FLOAT32_C(  -396.32), EASYSIMD_FLOAT32_C(  -427.29), EASYSIMD_FLOAT32_C(   319.51), EASYSIMD_FLOAT32_C(   149.69),
      EASYSIMD_FLOAT32_C(   444.15), EASYSIMD_FLOAT32_C(  -521.32), EASYSIMD_FLOAT32_C(   808.51), EASYSIMD_FLOAT32_C(   949.94),
      EASYSIMD_FLOAT32_C(  -962.30), EASYSIMD_FLOAT32_C(   834.24), EASYSIMD_FLOAT32_C(   898.01), EASYSIMD_FLOAT32_C(  -410.47),
      EASYSIMD_FLOAT32_C(   474.93), EASYSIMD_FLOAT32_C(  -640.10), EASYSIMD_FLOAT32_C(   824.18), EASYSIMD_FLOAT32_C(   279.09),
      EASYSIMD_FLOAT32_C(  -927.33), EASYSIMD_FLOAT32_C(   365.70), EASYSIMD_FLOAT32_C(  -747.20), EASYSIMD_FLOAT32_C(   813.88),
      EASYSIMD_FLOAT32_C(  -172.63), EASYSIMD_FLOAT32_C(   387.19), EASYSIMD_FLOAT32_C(   923.52), EASYSIMD_FLOAT32_C(   937.76),
      EASYSIMD_FLOAT32_C(   173.43), EASYSIMD_FLOAT32_C(   106.74), EASYSIMD_FLOAT32_C(  -623.74), EASYSIMD_FLOAT32_C(  -343.80),
      EASYSIMD_FLOAT32_C(  -582.83), EASYSIMD_FLOAT32_C(  -853.79), EASYSIMD_FLOAT32_C(   906.08), EASYSIMD_FLOAT32_C(    20.85),
      EASYSIMD_FLOAT32_C(  -281.07), EASYSIMD_FLOAT32_C(   225.59), EASYSIMD_FLOAT32_C(  -829.46), EASYSIMD_FLOAT32_C(  -836.92),
      EASYSIMD_FLOAT32_C(   704.27), EASYSIMD_FLOAT32_C(   979.05), EASYSIMD_FLOAT32_C(  -886.98), EASYSIMD_FLOAT32_C(   741.97),
      EASYSIMD_FLOAT32_C(   813.28), EASYSIMD_FLOAT32_C(  -988.98), EASYSIMD_FLOAT32_C(  -668.50), EASYSIMD_FLOAT32_C(   288.22),
      EASYSIMD_FLOAT32_C(  -629.08), EASYSIMD_FLOAT32_C(  -844.32), EASYSIMD_FLOAT32_C(  -432.69), EASYSIMD_FLOAT32_C(  -556.40),
      EASYSIMD_FLOAT32_C(   521.39), EASYSIMD_FLOAT32_C(  -179.89), EASYSIMD_FLOAT32_C(  -742.52), EASYSIMD_FLOAT32_C(  -651.24),
      EASYSIMD_FLOAT32_C(  -792.70), EASYSIMD_FLOAT32_C(  -819.00), EASYSIMD_FLOAT32_C(  -713.48), EASYSIMD_FLOAT32_C(   380.73),
      EASYSIMD_FLOAT32_C(   287.74), EASYSIMD_FLOAT32_C(  -337.22), EASYSIMD_FLOAT32_C(  -963.07), EASYSIMD_FLOAT32_C(   704.92),
      EASYSIMD_FLOAT32_C(  -191.00), EASYSIMD_FLOAT32_C(   943.01), EASYSIMD_FLOAT32_C(  -274.23), EASYSIMD_FLOAT32_C(   527.92),
      EASYSIMD_FLOAT32_C(   168.60), EASYSIMD_FLOAT32_C(  -103.69), EASYSIMD_FLOAT32_C(   691.00), EASYSIMD_FLOAT32_C(  -127.13),
      EASYSIMD_FLOAT32_C(  -124.65), EASYSIMD_FLOAT32_C(   804.02), EASYSIMD_FLOAT32_C(  -385.16), EASYSIMD_FLOAT32_C(  -311.36),
      EASYSIMD_FLOAT32_C(   815.05), EASYSIMD_FLOAT32_C(   -53.66), EASYSIMD_FLOAT32_C(   976.86), EASYSIMD_FLOAT32_C(  -814.03),
      EASYSIMD_FLOAT32_C(   102.02), EASYSIMD_FLOAT32_C(  -455.83), EASYSIMD_FLOAT32_C(  -370.44), EASYSIMD_FLOAT32_C(  -376.59),
      EASYSIMD_FLOAT32_C(   364.28), EASYSIMD_FLOAT32_C(  -112.96), EASYSIMD_FLOAT32_C(   -27.83), EASYSIMD_FLOAT32_C(   571.58),
      EASYSIMD_FLOAT32_C(    68.05), EASYSIMD_FLOAT32_C(   258.69), EASYSIMD_FLOAT32_C(   -47.69), EASYSIMD_FLOAT32_C(  -644.21),
      EASYSIMD_FLOAT32_C(   921.48), EASYSIMD_FLOAT32_C(   -10.76), EASYSIMD_FLOAT32_C(  -939.30), EASYSIMD_FLOAT32_C(  -269.53),
      EASYSIMD_FLOAT32_C(   -67.75), EASYSIMD_FLOAT32_C(  -213.53), EASYSIMD_FLOAT32_C(  -741.60), EASYSIMD_FLOAT32_C(  -899.16),
      EASYSIMD_FLOAT32_C(   682.77), EASYSIMD_FLOAT32_C(   949.40), EASYSIMD_FLOAT32_C(   -26.29), EASYSIMD_FLOAT32_C(  -441.87),
      EASYSIMD_FLOAT32_C(   753.42), EASYSIMD_FLOAT32_C(   588.54), EASYSIMD_FLOAT32_C(   246.76), EASYSIMD_FLOAT32_C(   568.47),
      EASYSIMD_FLOAT32_C(  -465.12), EASYSIMD_FLOAT32_C(   223.62), EASYSIMD_FLOAT32_C(   754.44), EASYSIMD_FLOAT32_C(   636.91),
      EASYSIMD_FLOAT32_C(   767.79), EASYSIMD_FLOAT32_C(  -616.00), EASYSIMD_FLOAT32_C(  -739.68), EASYSIMD_FLOAT32_C(   132.07),
      EASYSIMD_FLOAT32_C(   271.04), EASYSIMD_FLOAT32_C(   232.49), EASYSIMD_FLOAT32_C(  -296.35), EASYSIMD_FLOAT32_C(  -660.91),
      EASYSIMD_FLOAT32_C(  -508.82), EASYSIMD_FLOAT32_C(   655.95), EASYSIMD_FLOAT32_C(  -305.13), EASYSIMD_FLOAT32_C(  -587.34),
      EASYSIMD_FLOAT32_C(  -354.81), EASYSIMD_FLOAT32_C(  -244.42), EASYSIMD_FLOAT32_C(   143.13), EASYSIMD_FLOAT32_C(   577.44),
      EASYSIMD_FLOAT32_C(   542.04), EASYSIMD_FLOAT32_C(   401.53), EASYSIMD_FLOAT32_C(   678.28), EASYSIMD_FLOAT32_C(   224.82),
      EASYSIMD_FLOAT32_C(   350.93), EASYSIMD_FLOAT32_C(  -348.01), EASYSIMD_FLOAT32_C(   782.94), EASYSIMD_FLOAT32_C(   104.35),
      EASYSIMD_FLOAT32_C(  -759.47), EASYSIMD_FLOAT32_C(    29.71), EASYSIMD_FLOAT32_C(  -327.17), EASYSIMD_FLOAT32_C(  -224.58),
      EASYSIMD_FLOAT32_C(  -746.67), EASYSIMD_FLOAT32_C(  -572.74), EASYSIMD_FLOAT32_C(  -587.68), EASYSIMD_FLOAT32_C(  -978.88),
      EASYSIMD_FLOAT32_C(  -188.74), EASYSIMD_FLOAT32_C(  -327.36), EASYSIMD_FLOAT32_C(   153.19), EASYSIMD_FLOAT32_C(  -917.69),
      EASYSIMD_FLOAT32_C(   905.13), EASYSIMD_FLOAT32_C(   856.83), EASYSIMD_FLOAT32_C(  -578.61), EASYSIMD_FLOAT32_C(  -603.68),
      EASYSIMD_FLOAT32_C(   512.79), EASYSIMD_FLOAT32_C(   116.27), EASYSIMD_FLOAT32_C(  -191.03), EASYSIMD_FLOAT32_C(  -842.03),
      EASYSIMD_FLOAT32_C(   871.84), EASYSIMD_FLOAT32_C(   952.11), EASYSIMD_FLOAT32_C(   735.41), EASYSIMD_FLOAT32_C(   413.89),
      EASYSIMD_FLOAT32_C(   353.63), EASYSIMD_FLOAT32_C(   413.69), EASYSIMD_FLOAT32_C(  -361.30), EASYSIMD_FLOAT32_C(  -295.44),
      EASYSIMD_FLOAT32_C(  -934.32), EASYSIMD_FLOAT32_C(  -578.35), EASYSIMD_FLOAT32_C(   808.92), EASYSIMD_FLOAT32_C(  -693.79),
      EASYSIMD_FLOAT32_C(   451.35), EASYSIMD_FLOAT32_C(  -518.26), EASYSIMD_FLOAT32_C(    81.63), EASYSIMD_FLOAT32_C(   704.68),
      EASYSIMD_FLOAT32_C(   -90.99), EASYSIMD_FLOAT32_C(   493.95), EASYSIMD_FLOAT32_C(   725.80), EASYSIMD_FLOAT32_C(   720.27),
      EASYSIMD_FLOAT32_C(  -833.41), EASYSIMD_FLOAT32_C(  -121.01), EASYSIMD_FLOAT32_C(   802.58), EASYSIMD_FLOAT32_C(  -928.27),
      EASYSIMD_FLOAT32_C(  -264.17), EASYSIMD_FLOAT32_C(  -776.03), EASYSIMD_FLOAT32_C(  -531.96), EASYSIMD_FLOAT32_C(  -751.39),
      EASYSIMD_FLOAT32_C(   340.24), EASYSIMD_FLOAT32_C(   277.02), EASYSIMD_FLOAT32_C(  -593.41), EASYSIMD_FLOAT32_C(   212.08),
      EASYSIMD_FLOAT32_C(   229.12), EASYSIMD_FLOAT32_C(  -858.01), EASYSIMD_FLOAT32_C(  -374.03), EASYSIMD_FLOAT32_C(  -417.24),
      EASYSIMD_FLOAT32_C(   555.68), EASYSIMD_FLOAT32_C(   264.68), EASYSIMD_FLOAT32_C(   287.32), EASYSIMD_FLOAT32_C(   621.36),
      EASYSIMD_FLOAT32_C(   686.32), EASYSIMD_FLOAT32_C(    96.24), EASYSIMD_FLOAT32_C(   927.57), EASYSIMD_FLOAT32_C(   137.68),
      EASYSIMD_FLOAT32_C(   577.98), EASYSIMD_FLOAT32_C(     9.20), EASYSIMD_FLOAT32_C(  -157.64), EASYSIMD_FLOAT32_C(  -513.01),
      EASYSIMD_FLOAT32_C(  -496.84) };
  static const easysimd_float32 e[] =
    { EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  1244.81), EASYSIMD_FLOAT32_C(  -609.84),
      EASYSIMD_FLOAT32_C(   700.53), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -490.06), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   143.11), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   416.33), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   -21.96), EASYSIMD_FLOAT32_C(   753.76), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   425.09), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   589.18),
      EASYSIMD_FLOAT32_C( -1958.14), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  1479.46), EASYSIMD_FLOAT32_C( -1238.91),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C( -1327.78), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -294.14), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C( -1603.16), EASYSIMD_FLOAT32_C(   406.39),
      EASYSIMD_FLOAT32_C(  -977.07), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  1097.41), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -209.26), EASYSIMD_FLOAT32_C(  -759.47), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   207.06),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C( -1025.70), EASYSIMD_FLOAT32_C(  1786.58), EASYSIMD_FLOAT32_C(  -379.34),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   540.34), EASYSIMD_FLOAT32_C( -1399.38), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   361.37),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -789.19), EASYSIMD_FLOAT32_C(  -726.06), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   543.98), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -672.03), EASYSIMD_FLOAT32_C( -1642.56), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   568.49), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   885.59),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -893.13), EASYSIMD_FLOAT32_C(  1676.32), EASYSIMD_FLOAT32_C(   221.80),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  1642.94), EASYSIMD_FLOAT32_C(   583.17), EASYSIMD_FLOAT32_C(   309.47),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -294.37), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -149.34), EASYSIMD_FLOAT32_C(  -968.54), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -972.67), EASYSIMD_FLOAT32_C(  1099.02),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -615.23), EASYSIMD_FLOAT32_C(   467.32), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(    42.69), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -896.44),
      EASYSIMD_FLOAT32_C(  -595.36), EASYSIMD_FLOAT32_C(   550.32), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  1657.17), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -471.28), EASYSIMD_FLOAT32_C(  -668.61),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   111.88), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -182.49), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   733.96),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -410.33),
      EASYSIMD_FLOAT32_C(  -688.46), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   -98.47),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -939.99), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -801.31), EASYSIMD_FLOAT32_C(  -895.91), EASYSIMD_FLOAT32_C(  -532.94), EASYSIMD_FLOAT32_C(   603.33),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   -89.75), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   642.96), EASYSIMD_FLOAT32_C(   670.02),
      EASYSIMD_FLOAT32_C(  -449.14), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C( -1430.39),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   296.70), EASYSIMD_FLOAT32_C(   159.28), EASYSIMD_FLOAT32_C( -1140.14),
      EASYSIMD_FLOAT32_C(  1518.40), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C( -1238.61), EASYSIMD_FLOAT32_C(   124.89),
      EASYSIMD_FLOAT32_C(     3.97), EASYSIMD_FLOAT32_C(  -178.59), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -797.35),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   -44.56), EASYSIMD_FLOAT32_C(  -194.02), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   824.94), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -108.52), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -844.79), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(    74.44), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   176.77), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -168.99), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   116.76), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   413.81), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   103.12), EASYSIMD_FLOAT32_C(  -660.70),
      EASYSIMD_FLOAT32_C(   560.57), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   -80.79), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -374.68), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   158.76), EASYSIMD_FLOAT32_C(   780.53),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -397.97), EASYSIMD_FLOAT32_C(  1486.59), EASYSIMD_FLOAT32_C(  -836.59),
      EASYSIMD_FLOAT32_C(  1526.05), EASYSIMD_FLOAT32_C(  -438.97), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00) };

  static easysimd_float32 r[sizeof(e) / sizeof(e[0])];
  static const int32_t len = HEDLEY_STATIC_CAST(int32_t, sizeof(e) / sizeof(e[0]));
  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svfloat32_t
      rv,
      av = easysimd_svld1_f32(pg, &(a[i])),
      bv = easysimd_svld1_f32(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_f32_z(pv, av, bv);

    easysimd_svst1_f32(pg, &(r[i]), rv);

    i += easysimd_svcntw();
    pg = easysimd_svwhilelt_b32_s32(i, len);
  }

  easysimd_assert_equal_vf32(len, e, r, 1);

  return 0;
#else
  int32_t p[1024 / sizeof(int32_t)];
  easysimd_float32 a[1024 / sizeof(easysimd_float32)], b[1024 / sizeof(easysimd_float32)], e[1024 / sizeof(easysimd_float32)];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));

  fputc('\n', stdout);

  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_vf32(HEDLEY_STATIC_CAST(size_t, len), a, EASYSIMD_FLOAT32_C(-1000.0), EASYSIMD_FLOAT32_C(1000.0));
  easysimd_test_codegen_random_vf32(HEDLEY_STATIC_CAST(size_t, len), b, EASYSIMD_FLOAT32_C(-1000.0), EASYSIMD_FLOAT32_C(1000.0));

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svfloat32_t av, bv, ev;
    easysimd_svbool_t pv;
    ...
    ev = easysimd_svadd_f32_z(pv, av, bv);

    easysimd_svst1_f32(pg, &(e[i]), ev);
    easysimd_svst1_s32(pg, &(p[i]), easysimd_svdup_n_s32_z(pv, ~INT32_C(0)));
  ...
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vf32(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vf32(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vf32(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_f32_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
    {  INT32_C(           0), -INT32_C(           1), -INT32_C(           1), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),
      ...
      -INT32_C(           1) };
  static const easysimd_float32 a[] =
    { EASYSIMD_FLOAT32_C(  -663.70), EASYSIMD_FLOAT32_C(    35.12), EASYSIMD_FLOAT32_C(   599.74), EASYSIMD_FLOAT32_C(  -118.37),
      EASYSIMD_FLOAT32_C(  -227.34), EASYSIMD_FLOAT32_C(   663.04), EASYSIMD_FLOAT32_C(   217.80), EASYSIMD_FLOAT32_C(  -118.38),
      EASYSIMD_FLOAT32_C(  -638.99), EASYSIMD_FLOAT32_C(  -930.05), EASYSIMD_FLOAT32_C(  -483.76), EASYSIMD_FLOAT32_C(   720.35),
      EASYSIMD_FLOAT32_C(   466.58), EASYSIMD_FLOAT32_C(  -206.72), EASYSIMD_FLOAT32_C(   810.08), EASYSIMD_FLOAT32_C(  -277.58),
      EASYSIMD_FLOAT32_C(    94.69), EASYSIMD_FLOAT32_C(   741.76), EASYSIMD_FLOAT32_C(    30.15), EASYSIMD_FLOAT32_C(  -755.01),
      EASYSIMD_FLOAT32_C(   134.31), EASYSIMD_FLOAT32_C(  -574.60), EASYSIMD_FLOAT32_C(  -686.22), EASYSIMD_FLOAT32_C(  -217.20),
      EASYSIMD_FLOAT32_C(  -647.51), EASYSIMD_FLOAT32_C(  -584.72), EASYSIMD_FLOAT32_C(   974.25), EASYSIMD_FLOAT32_C(  -864.07),
      EASYSIMD_FLOAT32_C(   220.34), EASYSIMD_FLOAT32_C(   590.35), EASYSIMD_FLOAT32_C(   357.96), EASYSIMD_FLOAT32_C(   556.64),
      EASYSIMD_FLOAT32_C(  -374.53), EASYSIMD_FLOAT32_C(   -42.30), EASYSIMD_FLOAT32_C(  -561.72), EASYSIMD_FLOAT32_C(   398.13),
      EASYSIMD_FLOAT32_C(  -379.26), EASYSIMD_FLOAT32_C(   656.08), EASYSIMD_FLOAT32_C(  -720.24), EASYSIMD_FLOAT32_C(   -18.26),
      EASYSIMD_FLOAT32_C(   726.03), EASYSIMD_FLOAT32_C(  -204.00), EASYSIMD_FLOAT32_C(  -297.91), EASYSIMD_FLOAT32_C(   192.62),
      EASYSIMD_FLOAT32_C(   589.28), EASYSIMD_FLOAT32_C(  -487.83), EASYSIMD_FLOAT32_C(   915.04), EASYSIMD_FLOAT32_C(  -316.03),
      EASYSIMD_FLOAT32_C(  -746.07), EASYSIMD_FLOAT32_C(   -54.81), EASYSIMD_FLOAT32_C(   -71.04), EASYSIMD_FLOAT32_C(   388.25),
      EASYSIMD_FLOAT32_C(   370.59), EASYSIMD_FLOAT32_C(   242.74), EASYSIMD_FLOAT32_C(  -828.95), EASYSIMD_FLOAT32_C(   723.08),
      EASYSIMD_FLOAT32_C(   658.02), EASYSIMD_FLOAT32_C(  -854.70), EASYSIMD_FLOAT32_C(   859.01), EASYSIMD_FLOAT32_C(  -121.64),
      EASYSIMD_FLOAT32_C(   735.66), EASYSIMD_FLOAT32_C(   216.97), EASYSIMD_FLOAT32_C(  -564.99), EASYSIMD_FLOAT32_C(  -638.87),
      EASYSIMD_FLOAT32_C(  -825.33), EASYSIMD_FLOAT32_C(  -126.71), EASYSIMD_FLOAT32_C(   759.26), EASYSIMD_FLOAT32_C(  -204.59),
      EASYSIMD_FLOAT32_C(  -470.63), EASYSIMD_FLOAT32_C(  -960.98), EASYSIMD_FLOAT32_C(   777.16), EASYSIMD_FLOAT32_C(  -744.60),
      EASYSIMD_FLOAT32_C(  -164.98), EASYSIMD_FLOAT32_C(  -520.75), EASYSIMD_FLOAT32_C(   448.02), EASYSIMD_FLOAT32_C(  -575.70),
      EASYSIMD_FLOAT32_C(    -8.58), EASYSIMD_FLOAT32_C(   363.06), EASYSIMD_FLOAT32_C(   108.27), EASYSIMD_FLOAT32_C(   245.35),
      EASYSIMD_FLOAT32_C(  -691.75), EASYSIMD_FLOAT32_C(  -962.76), EASYSIMD_FLOAT32_C(  -366.40), EASYSIMD_FLOAT32_C(   678.85),
      EASYSIMD_FLOAT32_C(   279.98), EASYSIMD_FLOAT32_C(  -195.35), EASYSIMD_FLOAT32_C(   401.93), EASYSIMD_FLOAT32_C(   -62.00),
      EASYSIMD_FLOAT32_C(   -50.05), EASYSIMD_FLOAT32_C(   260.94), EASYSIMD_FLOAT32_C(   816.36), EASYSIMD_FLOAT32_C(  -314.39),
      EASYSIMD_FLOAT32_C(  -522.09), EASYSIMD_FLOAT32_C(  -748.63), EASYSIMD_FLOAT32_C(    46.74), EASYSIMD_FLOAT32_C(  -347.41),
      EASYSIMD_FLOAT32_C(   124.66), EASYSIMD_FLOAT32_C(  -194.00), EASYSIMD_FLOAT32_C(   448.00), EASYSIMD_FLOAT32_C(   654.03),
      EASYSIMD_FLOAT32_C(  -154.98), EASYSIMD_FLOAT32_C(   225.16), EASYSIMD_FLOAT32_C(   909.43), EASYSIMD_FLOAT32_C(   680.04),
      EASYSIMD_FLOAT32_C(   704.41), EASYSIMD_FLOAT32_C(   357.46), EASYSIMD_FLOAT32_C(  -895.66), EASYSIMD_FLOAT32_C(  -304.17),
      EASYSIMD_FLOAT32_C(  -279.48), EASYSIMD_FLOAT32_C(   212.61), EASYSIMD_FLOAT32_C(   941.18), EASYSIMD_FLOAT32_C(    28.78),
      EASYSIMD_FLOAT32_C(   249.85), EASYSIMD_FLOAT32_C(  -425.21), EASYSIMD_FLOAT32_C(  -292.38), EASYSIMD_FLOAT32_C(  -470.17),
      EASYSIMD_FLOAT32_C(   379.44), EASYSIMD_FLOAT32_C(  -890.45), EASYSIMD_FLOAT32_C(   467.83), EASYSIMD_FLOAT32_C(  -670.61),
      EASYSIMD_FLOAT32_C(   370.49), EASYSIMD_FLOAT32_C(   284.19), EASYSIMD_FLOAT32_C(    15.00), EASYSIMD_FLOAT32_C(   848.40),
      EASYSIMD_FLOAT32_C(   535.57), EASYSIMD_FLOAT32_C(  -938.26), EASYSIMD_FLOAT32_C(  -499.01), EASYSIMD_FLOAT32_C(  -339.77),
      EASYSIMD_FLOAT32_C(  -132.27), EASYSIMD_FLOAT32_C(   948.99), EASYSIMD_FLOAT32_C(  -685.74), EASYSIMD_FLOAT32_C(   712.75),
      EASYSIMD_FLOAT32_C(   174.15), EASYSIMD_FLOAT32_C(  -776.31), EASYSIMD_FLOAT32_C(   392.79), EASYSIMD_FLOAT32_C(  -121.44),
      EASYSIMD_FLOAT32_C(   581.15), EASYSIMD_FLOAT32_C(   497.13), EASYSIMD_FLOAT32_C(   574.39), EASYSIMD_FLOAT32_C(  -698.33),
      EASYSIMD_FLOAT32_C(  -290.26), EASYSIMD_FLOAT32_C(   515.57), EASYSIMD_FLOAT32_C(   330.45), EASYSIMD_FLOAT32_C(   959.59),
      EASYSIMD_FLOAT32_C(  -909.64), EASYSIMD_FLOAT32_C(  -961.93), EASYSIMD_FLOAT32_C(  -510.59), EASYSIMD_FLOAT32_C(   469.79),
      EASYSIMD_FLOAT32_C(  -852.38), EASYSIMD_FLOAT32_C(   957.24), EASYSIMD_FLOAT32_C(   799.18), EASYSIMD_FLOAT32_C(   518.11),
      EASYSIMD_FLOAT32_C(   241.43), EASYSIMD_FLOAT32_C(  -185.82), EASYSIMD_FLOAT32_C(   366.51), EASYSIMD_FLOAT32_C(  -223.00),
      EASYSIMD_FLOAT32_C(  -124.08), EASYSIMD_FLOAT32_C(   867.50), EASYSIMD_FLOAT32_C(   437.23), EASYSIMD_FLOAT32_C(   743.65),
      EASYSIMD_FLOAT32_C(   816.49), EASYSIMD_FLOAT32_C(   751.48), EASYSIMD_FLOAT32_C(   456.40), EASYSIMD_FLOAT32_C(    -9.36),
      EASYSIMD_FLOAT32_C(   975.18), EASYSIMD_FLOAT32_C(  -150.81), EASYSIMD_FLOAT32_C(   869.20), EASYSIMD_FLOAT32_C(   556.33),
      EASYSIMD_FLOAT32_C(  -653.69), EASYSIMD_FLOAT32_C(   443.59), EASYSIMD_FLOAT32_C(   858.00), EASYSIMD_FLOAT32_C(    56.05),
      EASYSIMD_FLOAT32_C(   -40.84), EASYSIMD_FLOAT32_C(   188.44), EASYSIMD_FLOAT32_C(    15.64), EASYSIMD_FLOAT32_C(    49.51),
      EASYSIMD_FLOAT32_C(   226.51) };
  static const easysimd_float32 b[] =
    { EASYSIMD_FLOAT32_C(   505.06), EASYSIMD_FLOAT32_C(  -480.69), EASYSIMD_FLOAT32_C(   374.13), EASYSIMD_FLOAT32_C(   462.30),
      EASYSIMD_FLOAT32_C(  -681.51), EASYSIMD_FLOAT32_C(  -107.77), EASYSIMD_FLOAT32_C(  -296.27), EASYSIMD_FLOAT32_C(   132.67),
      EASYSIMD_FLOAT32_C(  -741.26), EASYSIMD_FLOAT32_C(   480.73), EASYSIMD_FLOAT32_C(  -991.41), EASYSIMD_FLOAT32_C(  -873.76),
      EASYSIMD_FLOAT32_C(   -82.04), EASYSIMD_FLOAT32_C(   752.25), EASYSIMD_FLOAT32_C(   942.74), EASYSIMD_FLOAT32_C(  -330.56),
      EASYSIMD_FLOAT32_C(   208.65), EASYSIMD_FLOAT32_C(   -66.62), EASYSIMD_FLOAT32_C(  -355.39), EASYSIMD_FLOAT32_C(  -942.17),
      EASYSIMD_FLOAT32_C(  -197.42), EASYSIMD_FLOAT32_C(  -799.06), EASYSIMD_FLOAT32_C(  -595.85), EASYSIMD_FLOAT32_C(  -753.83),
      EASYSIMD_FLOAT32_C(  -941.06), EASYSIMD_FLOAT32_C(   460.20), EASYSIMD_FLOAT32_C(   205.32), EASYSIMD_FLOAT32_C(   247.38),
      EASYSIMD_FLOAT32_C(  -524.16), EASYSIMD_FLOAT32_C(  -745.16), EASYSIMD_FLOAT32_C(  -526.11), EASYSIMD_FLOAT32_C(   980.90),
      EASYSIMD_FLOAT32_C(  -225.86), EASYSIMD_FLOAT32_C(   848.01), EASYSIMD_FLOAT32_C(   443.19), EASYSIMD_FLOAT32_C(    92.63),
      EASYSIMD_FLOAT32_C(  -259.75), EASYSIMD_FLOAT32_C(  -853.08), EASYSIMD_FLOAT32_C(  -774.70), EASYSIMD_FLOAT32_C(    -1.01),
      EASYSIMD_FLOAT32_C(   627.65), EASYSIMD_FLOAT32_C(  -766.11), EASYSIMD_FLOAT32_C(   125.23), EASYSIMD_FLOAT32_C(  -454.39),
      EASYSIMD_FLOAT32_C(   986.14), EASYSIMD_FLOAT32_C(    67.97), EASYSIMD_FLOAT32_C(   215.05), EASYSIMD_FLOAT32_C(   194.79),
      EASYSIMD_FLOAT32_C(  -998.65), EASYSIMD_FLOAT32_C(   859.66), EASYSIMD_FLOAT32_C(   252.62), EASYSIMD_FLOAT32_C(  -196.07),
      EASYSIMD_FLOAT32_C(  -939.40), EASYSIMD_FLOAT32_C(   656.77), EASYSIMD_FLOAT32_C(    50.09), EASYSIMD_FLOAT32_C(  -880.46),
      EASYSIMD_FLOAT32_C(   116.97), EASYSIMD_FLOAT32_C(  -744.59), EASYSIMD_FLOAT32_C(   366.92), EASYSIMD_FLOAT32_C(   592.81),
      EASYSIMD_FLOAT32_C(  -489.75), EASYSIMD_FLOAT32_C(   840.81), EASYSIMD_FLOAT32_C(   573.70), EASYSIMD_FLOAT32_C(   284.39),
      EASYSIMD_FLOAT32_C(   688.82), EASYSIMD_FLOAT32_C(    16.90), EASYSIMD_FLOAT32_C(  -622.98), EASYSIMD_FLOAT32_C(  -570.93),
      EASYSIMD_FLOAT32_C(   163.82), EASYSIMD_FLOAT32_C(  -397.68), EASYSIMD_FLOAT32_C(   428.05), EASYSIMD_FLOAT32_C(  -208.52),
      EASYSIMD_FLOAT32_C(  -163.78), EASYSIMD_FLOAT32_C(  -446.72), EASYSIMD_FLOAT32_C(   337.08), EASYSIMD_FLOAT32_C(  -177.64),
      EASYSIMD_FLOAT32_C(   621.25), EASYSIMD_FLOAT32_C(  -447.87), EASYSIMD_FLOAT32_C(  -982.86), EASYSIMD_FLOAT32_C(   622.60),
      EASYSIMD_FLOAT32_C(  -588.21), EASYSIMD_FLOAT32_C(   269.77), EASYSIMD_FLOAT32_C(  -573.48), EASYSIMD_FLOAT32_C(  -527.60),
      EASYSIMD_FLOAT32_C(   -73.47), EASYSIMD_FLOAT32_C(   476.61), EASYSIMD_FLOAT32_C(  -408.07), EASYSIMD_FLOAT32_C(  -956.50),
      EASYSIMD_FLOAT32_C(   732.03), EASYSIMD_FLOAT32_C(   958.85), EASYSIMD_FLOAT32_C(   636.31), EASYSIMD_FLOAT32_C(  -757.72),
      EASYSIMD_FLOAT32_C(   799.66), EASYSIMD_FLOAT32_C(   210.01), EASYSIMD_FLOAT32_C(   526.67), EASYSIMD_FLOAT32_C(   488.48),
      EASYSIMD_FLOAT32_C(  -773.09), EASYSIMD_FLOAT32_C(   903.69), EASYSIMD_FLOAT32_C(   917.54), EASYSIMD_FLOAT32_C(   390.73),
      EASYSIMD_FLOAT32_C(  -493.98), EASYSIMD_FLOAT32_C(   345.60), EASYSIMD_FLOAT32_C(  -817.79), EASYSIMD_FLOAT32_C(   342.23),
      EASYSIMD_FLOAT32_C(   898.88), EASYSIMD_FLOAT32_C(   519.29), EASYSIMD_FLOAT32_C(  -835.41), EASYSIMD_FLOAT32_C(   520.13),
      EASYSIMD_FLOAT32_C(  -928.58), EASYSIMD_FLOAT32_C(  -818.26), EASYSIMD_FLOAT32_C(   142.73), EASYSIMD_FLOAT32_C(  -516.78),
      EASYSIMD_FLOAT32_C(   451.50), EASYSIMD_FLOAT32_C(   569.25), EASYSIMD_FLOAT32_C(   -44.39), EASYSIMD_FLOAT32_C(  -621.97),
      EASYSIMD_FLOAT32_C(    45.87), EASYSIMD_FLOAT32_C(   547.55), EASYSIMD_FLOAT32_C(  -578.46), EASYSIMD_FLOAT32_C(  -222.10),
      EASYSIMD_FLOAT32_C(   506.40), EASYSIMD_FLOAT32_C(  -942.16), EASYSIMD_FLOAT32_C(    20.17), EASYSIMD_FLOAT32_C(   306.06),
      EASYSIMD_FLOAT32_C(   267.86), EASYSIMD_FLOAT32_C(  -453.16), EASYSIMD_FLOAT32_C(  -205.47), EASYSIMD_FLOAT32_C(   494.77),
      EASYSIMD_FLOAT32_C(  -549.46), EASYSIMD_FLOAT32_C(  -287.92), EASYSIMD_FLOAT32_C(  -114.50), EASYSIMD_FLOAT32_C(   -43.45),
      EASYSIMD_FLOAT32_C(  -942.33), EASYSIMD_FLOAT32_C(    67.71), EASYSIMD_FLOAT32_C(  -701.21), EASYSIMD_FLOAT32_C(   956.56),
      EASYSIMD_FLOAT32_C(  -413.00), EASYSIMD_FLOAT32_C(  -536.62), EASYSIMD_FLOAT32_C(   476.69), EASYSIMD_FLOAT32_C(  -341.57),
      EASYSIMD_FLOAT32_C(  -354.89), EASYSIMD_FLOAT32_C(  -380.58), EASYSIMD_FLOAT32_C(   141.64), EASYSIMD_FLOAT32_C(  -903.38),
      EASYSIMD_FLOAT32_C(  -811.33), EASYSIMD_FLOAT32_C(  -902.74), EASYSIMD_FLOAT32_C(  -525.35), EASYSIMD_FLOAT32_C(   234.54),
      EASYSIMD_FLOAT32_C(   644.81), EASYSIMD_FLOAT32_C(  -103.81), EASYSIMD_FLOAT32_C(  -987.56), EASYSIMD_FLOAT32_C(   151.21),
      EASYSIMD_FLOAT32_C(   -45.97), EASYSIMD_FLOAT32_C(    32.61), EASYSIMD_FLOAT32_C(  -542.73), EASYSIMD_FLOAT32_C(  -778.11),
      EASYSIMD_FLOAT32_C(   579.45), EASYSIMD_FLOAT32_C(   251.80), EASYSIMD_FLOAT32_C(   716.66), EASYSIMD_FLOAT32_C(  -970.01),
      EASYSIMD_FLOAT32_C(   963.87), EASYSIMD_FLOAT32_C(  -397.84), EASYSIMD_FLOAT32_C(   -13.46), EASYSIMD_FLOAT32_C(  -978.45),
      EASYSIMD_FLOAT32_C(   669.87), EASYSIMD_FLOAT32_C(   285.33), EASYSIMD_FLOAT32_C(   978.10), EASYSIMD_FLOAT32_C(  -743.13),
      EASYSIMD_FLOAT32_C(   748.70), EASYSIMD_FLOAT32_C(   454.79), EASYSIMD_FLOAT32_C(   -84.71), EASYSIMD_FLOAT32_C(  -606.18),
      EASYSIMD_FLOAT32_C(  -925.79), EASYSIMD_FLOAT32_C(  -943.06), EASYSIMD_FLOAT32_C(  -509.57), EASYSIMD_FLOAT32_C(  -737.11),
      EASYSIMD_FLOAT32_C(  -845.80) };
  static const easysimd_float32 e[] =
    { EASYSIMD_FLOAT32_C(  -663.70), EASYSIMD_FLOAT32_C(  -445.57), EASYSIMD_FLOAT32_C(   973.87), EASYSIMD_FLOAT32_C(   343.93),
      EASYSIMD_FLOAT32_C(  -227.34), EASYSIMD_FLOAT32_C(   663.04), EASYSIMD_FLOAT32_C(   217.80), EASYSIMD_FLOAT32_C(    14.29),
      EASYSIMD_FLOAT32_C( -1380.25), EASYSIMD_FLOAT32_C(  -449.32), EASYSIMD_FLOAT32_C(  -483.76), EASYSIMD_FLOAT32_C(   720.35),
      EASYSIMD_FLOAT32_C(   466.58), EASYSIMD_FLOAT32_C(  -206.72), EASYSIMD_FLOAT32_C(   810.08), EASYSIMD_FLOAT32_C(  -277.58),
      EASYSIMD_FLOAT32_C(   303.34), EASYSIMD_FLOAT32_C(   675.14), EASYSIMD_FLOAT32_C(    30.15), EASYSIMD_FLOAT32_C(  -755.01),
      EASYSIMD_FLOAT32_C(   -63.11), EASYSIMD_FLOAT32_C(  -574.60), EASYSIMD_FLOAT32_C(  -686.22), EASYSIMD_FLOAT32_C(  -217.20),
      EASYSIMD_FLOAT32_C(  -647.51), EASYSIMD_FLOAT32_C(  -584.72), EASYSIMD_FLOAT32_C(  1179.57), EASYSIMD_FLOAT32_C(  -616.69),
      EASYSIMD_FLOAT32_C(   220.34), EASYSIMD_FLOAT32_C(  -154.81), EASYSIMD_FLOAT32_C(  -168.15), EASYSIMD_FLOAT32_C(  1537.54),
      EASYSIMD_FLOAT32_C(  -374.53), EASYSIMD_FLOAT32_C(   -42.30), EASYSIMD_FLOAT32_C(  -561.72), EASYSIMD_FLOAT32_C(   490.76),
      EASYSIMD_FLOAT32_C(  -379.26), EASYSIMD_FLOAT32_C(   656.08), EASYSIMD_FLOAT32_C(  -720.24), EASYSIMD_FLOAT32_C(   -18.26),
      EASYSIMD_FLOAT32_C(   726.03), EASYSIMD_FLOAT32_C(  -970.11), EASYSIMD_FLOAT32_C(  -172.68), EASYSIMD_FLOAT32_C(   192.62),
      EASYSIMD_FLOAT32_C(  1575.42), EASYSIMD_FLOAT32_C(  -419.86), EASYSIMD_FLOAT32_C(   915.04), EASYSIMD_FLOAT32_C(  -316.03),
      EASYSIMD_FLOAT32_C(  -746.07), EASYSIMD_FLOAT32_C(   -54.81), EASYSIMD_FLOAT32_C(   181.58), EASYSIMD_FLOAT32_C(   192.18),
      EASYSIMD_FLOAT32_C(   370.59), EASYSIMD_FLOAT32_C(   242.74), EASYSIMD_FLOAT32_C(  -828.95), EASYSIMD_FLOAT32_C(   723.08),
      EASYSIMD_FLOAT32_C(   774.99), EASYSIMD_FLOAT32_C( -1599.29), EASYSIMD_FLOAT32_C(   859.01), EASYSIMD_FLOAT32_C(  -121.64),
      EASYSIMD_FLOAT32_C(   245.91), EASYSIMD_FLOAT32_C(   216.97), EASYSIMD_FLOAT32_C(     8.71), EASYSIMD_FLOAT32_C(  -354.48),
      EASYSIMD_FLOAT32_C(  -825.33), EASYSIMD_FLOAT32_C(  -126.71), EASYSIMD_FLOAT32_C(   759.26), EASYSIMD_FLOAT32_C(  -204.59),
      EASYSIMD_FLOAT32_C(  -470.63), EASYSIMD_FLOAT32_C( -1358.66), EASYSIMD_FLOAT32_C(  1205.21), EASYSIMD_FLOAT32_C(  -744.60),
      EASYSIMD_FLOAT32_C(  -164.98), EASYSIMD_FLOAT32_C(  -967.47), EASYSIMD_FLOAT32_C(   785.10), EASYSIMD_FLOAT32_C(  -753.34),
      EASYSIMD_FLOAT32_C(    -8.58), EASYSIMD_FLOAT32_C(   363.06), EASYSIMD_FLOAT32_C(  -874.59), EASYSIMD_FLOAT32_C(   245.35),
      EASYSIMD_FLOAT32_C( -1279.96), EASYSIMD_FLOAT32_C(  -692.99), EASYSIMD_FLOAT32_C(  -366.40), EASYSIMD_FLOAT32_C(   678.85),
      EASYSIMD_FLOAT32_C(   206.51), EASYSIMD_FLOAT32_C(  -195.35), EASYSIMD_FLOAT32_C(   401.93), EASYSIMD_FLOAT32_C(   -62.00),
      EASYSIMD_FLOAT32_C(   681.98), EASYSIMD_FLOAT32_C(  1219.79), EASYSIMD_FLOAT32_C(  1452.67), EASYSIMD_FLOAT32_C(  -314.39),
      EASYSIMD_FLOAT32_C(  -522.09), EASYSIMD_FLOAT32_C(  -538.62), EASYSIMD_FLOAT32_C(    46.74), EASYSIMD_FLOAT32_C(  -347.41),
      EASYSIMD_FLOAT32_C(  -648.43), EASYSIMD_FLOAT32_C(   709.69), EASYSIMD_FLOAT32_C(   448.00), EASYSIMD_FLOAT32_C(   654.03),
      EASYSIMD_FLOAT32_C(  -648.96), EASYSIMD_FLOAT32_C(   570.76), EASYSIMD_FLOAT32_C(   909.43), EASYSIMD_FLOAT32_C(   680.04),
      EASYSIMD_FLOAT32_C(   704.41), EASYSIMD_FLOAT32_C(   876.75), EASYSIMD_FLOAT32_C( -1731.07), EASYSIMD_FLOAT32_C(   215.96),
      EASYSIMD_FLOAT32_C( -1208.06), EASYSIMD_FLOAT32_C(  -605.65), EASYSIMD_FLOAT32_C(  1083.91), EASYSIMD_FLOAT32_C(  -488.00),
      EASYSIMD_FLOAT32_C(   701.35), EASYSIMD_FLOAT32_C(   144.04), EASYSIMD_FLOAT32_C(  -292.38), EASYSIMD_FLOAT32_C( -1092.14),
      EASYSIMD_FLOAT32_C(   425.31), EASYSIMD_FLOAT32_C(  -342.90), EASYSIMD_FLOAT32_C(   467.83), EASYSIMD_FLOAT32_C(  -892.71),
      EASYSIMD_FLOAT32_C(   370.49), EASYSIMD_FLOAT32_C(   284.19), EASYSIMD_FLOAT32_C(    35.17), EASYSIMD_FLOAT32_C(   848.40),
      EASYSIMD_FLOAT32_C(   803.43), EASYSIMD_FLOAT32_C(  -938.26), EASYSIMD_FLOAT32_C(  -499.01), EASYSIMD_FLOAT32_C(   155.00),
      EASYSIMD_FLOAT32_C(  -681.73), EASYSIMD_FLOAT32_C(   661.07), EASYSIMD_FLOAT32_C(  -800.24), EASYSIMD_FLOAT32_C(   669.30),
      EASYSIMD_FLOAT32_C(   174.15), EASYSIMD_FLOAT32_C(  -776.31), EASYSIMD_FLOAT32_C(  -308.42), EASYSIMD_FLOAT32_C(   835.12),
      EASYSIMD_FLOAT32_C(   168.15), EASYSIMD_FLOAT32_C(   497.13), EASYSIMD_FLOAT32_C(  1051.08), EASYSIMD_FLOAT32_C( -1039.90),
      EASYSIMD_FLOAT32_C(  -645.15), EASYSIMD_FLOAT32_C(   515.57), EASYSIMD_FLOAT32_C(   330.45), EASYSIMD_FLOAT32_C(    56.21),
      EASYSIMD_FLOAT32_C( -1720.97), EASYSIMD_FLOAT32_C( -1864.67), EASYSIMD_FLOAT32_C( -1035.94), EASYSIMD_FLOAT32_C(   704.33),
      EASYSIMD_FLOAT32_C(  -852.38), EASYSIMD_FLOAT32_C(   853.43), EASYSIMD_FLOAT32_C(   799.18), EASYSIMD_FLOAT32_C(   518.11),
      EASYSIMD_FLOAT32_C(   195.46), EASYSIMD_FLOAT32_C(  -153.21), EASYSIMD_FLOAT32_C(  -176.22), EASYSIMD_FLOAT32_C(  -223.00),
      EASYSIMD_FLOAT32_C(  -124.08), EASYSIMD_FLOAT32_C(   867.50), EASYSIMD_FLOAT32_C(  1153.89), EASYSIMD_FLOAT32_C(  -226.36),
      EASYSIMD_FLOAT32_C(   816.49), EASYSIMD_FLOAT32_C(   751.48), EASYSIMD_FLOAT32_C(   456.40), EASYSIMD_FLOAT32_C(    -9.36),
      EASYSIMD_FLOAT32_C(   975.18), EASYSIMD_FLOAT32_C(   134.52), EASYSIMD_FLOAT32_C(  1847.30), EASYSIMD_FLOAT32_C(  -186.80),
      EASYSIMD_FLOAT32_C(    95.01), EASYSIMD_FLOAT32_C(   443.59), EASYSIMD_FLOAT32_C(   858.00), EASYSIMD_FLOAT32_C(  -550.13),
      EASYSIMD_FLOAT32_C(   -40.84), EASYSIMD_FLOAT32_C(  -754.62), EASYSIMD_FLOAT32_C(  -493.93), EASYSIMD_FLOAT32_C(    49.51),
      EASYSIMD_FLOAT32_C(  -619.29) };
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_f32_m(pv, av, bv);
...
#else
  int32_t p[1024 / sizeof(int32_t)];
  easysimd_float32 a[1024 / sizeof(int32_t)], b[1024 / sizeof(int32_t)], e[1024 / sizeof(int32_t)];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_f32_m(pv, av, bv);
...
static int
test_easysimd_svadd_f64_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    { -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),
       INT64_C(                   0), -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),
      ...
      -INT64_C(                   1),  INT64_C(                   0), -INT64_C(                   1), -INT64_C(                   1),
       INT64_C(                   0) };
  static const easysimd_float64 a[] =
    { EASYSIMD_FLOAT64_C(   -64.06), EASYSIMD_FLOAT64_C(    31.58), EASYSIMD_FLOAT64_C(  -226.04), EASYSIMD_FLOAT64_C(   405.84),
      EASYSIMD_FLOAT64_C(   687.79), EASYSIMD_FLOAT64_C(  -264.95), EASYSIMD_FLOAT64_C(   939.27), EASYSIMD_FLOAT64_C(  -991.38),
      EASYSIMD_FLOAT64_C(  -986.92), EASYSIMD_FLOAT64_C(  -822.73), EASYSIMD_FLOAT64_C(    60.68), EASYSIMD_FLOAT64_C(  -437.61),
      EASYSIMD_FLOAT64_C(   592.86), EASYSIMD_FLOAT64_C(   925.29), EASYSIMD_FLOAT64_C(   123.93), EASYSIMD_FLOAT64_C(  -941.71),
      EASYSIMD_FLOAT64_C(  -988.36), EASYSIMD_FLOAT64_C(   763.53), EASYSIMD_FLOAT64_C(  -643.53), EASYSIMD_FLOAT64_C(   754.01),
      EASYSIMD_FLOAT64_C(   -34.98), EASYSIMD_FLOAT64_C(   178.76), EASYSIMD_FLOAT64_C(  -981.44), EASYSIMD_FLOAT64_C(   654.32),
      EASYSIMD_FLOAT64_C(  -406.43), EASYSIMD_FLOAT64_C(  -905.77), EASYSIMD_FLOAT64_C(  -773.50), EASYSIMD_FLOAT64_C(   418.62),
      EASYSIMD_FLOAT64_C(   125.75), EASYSIMD_FLOAT64_C(  -939.63), EASYSIMD_FLOAT64_C(   255.62), EASYSIMD_FLOAT64_C(  -938.31),
      EASYSIMD_FLOAT64_C(    91.95), EASYSIMD_FLOAT64_C(  -970.42), EASYSIMD_FLOAT64_C(   467.52), EASYSIMD_FLOAT64_C(  -220.26),
      EASYSIMD_FLOAT64_C(  -235.37), EASYSIMD_FLOAT64_C(   406.79), EASYSIMD_FLOAT64_C(  -211.64), EASYSIMD_FLOAT64_C(  -222.29),
      EASYSIMD_FLOAT64_C(   584.06), EASYSIMD_FLOAT64_C(   849.04), EASYSIMD_FLOAT64_C(   340.10), EASYSIMD_FLOAT64_C(   176.92),
      EASYSIMD_FLOAT64_C(   774.32), EASYSIMD_FLOAT64_C(  -535.97), EASYSIMD_FLOAT64_C(   235.21), EASYSIMD_FLOAT64_C(   785.97),
      EASYSIMD_FLOAT64_C(  -772.44), EASYSIMD_FLOAT64_C(   591.69), EASYSIMD_FLOAT64_C(   539.97), EASYSIMD_FLOAT64_C(   192.58),
      EASYSIMD_FLOAT64_C(  -229.56), EASYSIMD_FLOAT64_C(   558.54), EASYSIMD_FLOAT64_C(  -153.10), EASYSIMD_FLOAT64_C(   364.02),
      EASYSIMD_FLOAT64_C(   652.77) };
  static const easysimd_float64 b[] =
    { EASYSIMD_FLOAT64_C(    73.40), EASYSIMD_FLOAT64_C(  -217.36), EASYSIMD_FLOAT64_C(  -221.48), EASYSIMD_FLOAT64_C(   133.77),
      EASYSIMD_FLOAT64_C(  -961.75), EASYSIMD_FLOAT64_C(  -159.79), EASYSIMD_FLOAT64_C(  -774.28), EASYSIMD_FLOAT64_C(  -932.17),
      EASYSIMD_FLOAT64_C(  -692.27), EASYSIMD_FLOAT64_C(     5.46), EASYSIMD_FLOAT64_C(  -167.53), EASYSIMD_FLOAT64_C(   714.52),
      EASYSIMD_FLOAT64_C(   793.81), EASYSIMD_FLOAT64_C(   610.18), EASYSIMD_FLOAT64_C(   298.58), EASYSIMD_FLOAT64_C(   642.85),
      EASYSIMD_FLOAT64_C(   -49.72), EASYSIMD_FLOAT64_C(  -524.50), EASYSIMD_FLOAT64_C(   417.18), EASYSIMD_FLOAT64_C(   414.31),
      EASYSIMD_FLOAT64_C(   710.72), EASYSIMD_FLOAT64_C(   203.15), EASYSIMD_FLOAT64_C(   641.87), EASYSIMD_FLOAT64_C(   302.40),
      EASYSIMD_FLOAT64_C(  -256.88), EASYSIMD_FLOAT64_C(  -165.55), EASYSIMD_FLOAT64_C(  -927.15), EASYSIMD_FLOAT64_C(  -698.34),
      EASYSIMD_FLOAT64_C(   681.35), EASYSIMD_FLOAT64_C(   436.87), EASYSIMD_FLOAT64_C(   954.42), EASYSIMD_FLOAT64_C(  -245.25),
      EASYSIMD_FLOAT64_C(  -780.50), EASYSIMD_FLOAT64_C(  -267.06), EASYSIMD_FLOAT64_C(   888.53), EASYSIMD_FLOAT64_C(  -742.24),
      EASYSIMD_FLOAT64_C(   573.15), EASYSIMD_FLOAT64_C(  -885.75), EASYSIMD_FLOAT64_C(  -674.41), EASYSIMD_FLOAT64_C(   880.88),
      EASYSIMD_FLOAT64_C(   119.70), EASYSIMD_FLOAT64_C(   158.06), EASYSIMD_FLOAT64_C(   595.40), EASYSIMD_FLOAT64_C(   -86.48),
      EASYSIMD_FLOAT64_C(  -231.76), EASYSIMD_FLOAT64_C(  -106.02), EASYSIMD_FLOAT64_C(  -443.63), EASYSIMD_FLOAT64_C(   718.51),
      EASYSIMD_FLOAT64_C(   369.48), EASYSIMD_FLOAT64_C(   973.55), EASYSIMD_FLOAT64_C(   132.82), EASYSIMD_FLOAT64_C(    80.20),
      EASYSIMD_FLOAT64_C(   176.69), EASYSIMD_FLOAT64_C(  -225.31), EASYSIMD_FLOAT64_C(  -617.40), EASYSIMD_FLOAT64_C(   919.81),
      EASYSIMD_FLOAT64_C(   609.14) };
  static const easysimd_float64 e[] =
    { EASYSIMD_FLOAT64_C(     9.34), EASYSIMD_FLOAT64_C(  -185.78), EASYSIMD_FLOAT64_C(  -447.52), EASYSIMD_FLOAT64_C(   539.61),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -424.74), EASYSIMD_FLOAT64_C(   164.99), EASYSIMD_FLOAT64_C( -1923.55),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -817.27), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   276.91),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1535.47), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -298.86),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1168.32),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   381.91), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   956.72),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C( -1700.65), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -502.76), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C( -1183.56),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -478.96), EASYSIMD_FLOAT64_C(  -886.05), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1007.10), EASYSIMD_FLOAT64_C(   935.50), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -208.42), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(  -402.96), EASYSIMD_FLOAT64_C(  1565.24), EASYSIMD_FLOAT64_C(   672.79), EASYSIMD_FLOAT64_C(   272.78),
      EASYSIMD_FLOAT64_C(   -52.87), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  -770.50), EASYSIMD_FLOAT64_C(  1283.83),
      EASYSIMD_FLOAT64_C(     0.00) };

  static easysimd_float64 r[sizeof(e) / sizeof(e[0])];
  static const int32_t len = HEDLEY_STATIC_CAST(int32_t, sizeof(e) / sizeof(e[0]));
  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svfloat64_t
      rv,
      av = easysimd_svld1_f64(pg, &(a[i])),
      bv = easysimd_svld1_f64(pg, &(b[i]));
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_f64_z(pv, av, bv);

    easysimd_svst1_f64(pg, &(r[i]), rv);

    i += easysimd_svcntd();
    pg = easysimd_svwhilelt_b64_s32(i, len);
  }

  easysimd_assert_equal_vf64(len, e, r, 1);

  return 0;
#else
  int64_t p[1024 / sizeof(int64_t)];
  easysimd_float64 a[1024 / sizeof(int64_t)], b[1024 / sizeof(int64_t)], e[1024 / sizeof(int64_t)];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));

  fputc('\n', stdout);

  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_vf64(HEDLEY_STATIC_CAST(size_t, len), a, EASYSIMD_FLOAT64_C(-1000.0), EASYSIMD_FLOAT64_C(1000.0));
  easysimd_test_codegen_random_vf64(HEDLEY_STATIC_CAST(size_t, len), b, EASYSIMD_FLOAT64_C(-1000.0), EASYSIMD_FLOAT64_C(1000.0));

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svfloat64_t av, bv, ev;
    easysimd_svbool_t pv;
    ...
    ev = easysimd_svadd_f64_z(pv, av, bv);

    easysimd_svst1_f64(pg, &(e[i]), ev);
    easysimd_svst1_s64(pg, &(p[i]), easysimd_svdup_n_s64_z(pv, ~INT64_C(0)));
  ...
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vf64(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1vf64(1, HEDLEY_STATIC_CAST(size_t, len), b);
  easysimd_test_codegen_write_1vf64(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_f64_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    {  INT64_C(                   0),  INT64_C(                   0), -INT64_C(                   1), -INT64_C(                   1),
       INT64_C(                   0),  INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0),
      ...
      -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),
       ...
       INT64_C(                   0),  INT64_C(                   0) };
  static const easysimd_float64 a[] =
    { EASYSIMD_FLOAT64_C(  -120.40), EASYSIMD_FLOAT64_C(  -627.34), EASYSIMD_FLOAT64_C(  -149.90), EASYSIMD_FLOAT64_C(  -486.60),
      EASYSIMD_FLOAT64_C(   566.16), EASYSIMD_FLOAT64_C(   990.74), EASYSIMD_FLOAT64_C(   246.46), EASYSIMD_FLOAT64_C(   -79.15),
      EASYSIMD_FLOAT64_C(  -175.22), EASYSIMD_FLOAT64_C(   771.10), EASYSIMD_FLOAT64_C(  -582.35), EASYSIMD_FLOAT64_C(   579.32),
      EASYSIMD_FLOAT64_C(  -720.44), EASYSIMD_FLOAT64_C(  -715.64), EASYSIMD_FLOAT64_C(   796.54), EASYSIMD_FLOAT64_C(  -318.07),
      EASYSIMD_FLOAT64_C(  -622.24), EASYSIMD_FLOAT64_C(   162.14), EASYSIMD_FLOAT64_C(   104.64), EASYSIMD_FLOAT64_C(   234.38),
      EASYSIMD_FLOAT64_C(   484.37), EASYSIMD_FLOAT64_C(  -987.92), EASYSIMD_FLOAT64_C(  -394.43), EASYSIMD_FLOAT64_C(   682.07),
      EASYSIMD_FLOAT64_C(   972.48), EASYSIMD_FLOAT64_C(  -244.92), EASYSIMD_FLOAT64_C(   177.74), EASYSIMD_FLOAT64_C(  -306.17),
      EASYSIMD_FLOAT64_C(  -101.56), EASYSIMD_FLOAT64_C(   -76.70), EASYSIMD_FLOAT64_C(   -71.73), EASYSIMD_FLOAT64_C(   778.05),
      EASYSIMD_FLOAT64_C(   295.96), EASYSIMD_FLOAT64_C(   778.37), EASYSIMD_FLOAT64_C(  -708.56), EASYSIMD_FLOAT64_C(  -137.87),
      EASYSIMD_FLOAT64_C(   769.12), EASYSIMD_FLOAT64_C(   537.90), EASYSIMD_FLOAT64_C(   782.98), EASYSIMD_FLOAT64_C(  -406.10),
      EASYSIMD_FLOAT64_C(   309.01), EASYSIMD_FLOAT64_C(  -799.38), EASYSIMD_FLOAT64_C(  -826.78), EASYSIMD_FLOAT64_C(   588.57),
      EASYSIMD_FLOAT64_C(  -515.02), EASYSIMD_FLOAT64_C(   969.76), EASYSIMD_FLOAT64_C(  -729.50), EASYSIMD_FLOAT64_C(  -137.26),
      EASYSIMD_FLOAT64_C(   131.90), EASYSIMD_FLOAT64_C(   375.14), EASYSIMD_FLOAT64_C(  -902.88), EASYSIMD_FLOAT64_C(  -383.73),
      EASYSIMD_FLOAT64_C(   387.23), EASYSIMD_FLOAT64_C(  -297.31), EASYSIMD_FLOAT64_C(  -701.66), EASYSIMD_FLOAT64_C(   359.71),
      EASYSIMD_FLOAT64_C(   457.77), EASYSIMD_FLOAT64_C(   476.08), EASYSIMD_FLOAT64_C(  -946.47), EASYSIMD_FLOAT64_C(  -643.78),
      EASYSIMD_FLOAT64_C(  -600.62), EASYSIMD_FLOAT64_C(   -18.19) };
  static const easysimd_float64 b[] =
    { EASYSIMD_FLOAT64_C(  -865.74), EASYSIMD_FLOAT64_C(   695.35), EASYSIMD_FLOAT64_C(  -239.82), EASYSIMD_FLOAT64_C(  -574.29),
      EASYSIMD_FLOAT64_C(  -442.52), EASYSIMD_FLOAT64_C(  -470.70), EASYSIMD_FLOAT64_C(   963.61), EASYSIMD_FLOAT64_C(  -659.55),
      EASYSIMD_FLOAT64_C(   123.20), EASYSIMD_FLOAT64_C(   272.62), EASYSIMD_FLOAT64_C(  -458.93), EASYSIMD_FLOAT64_C(   296.42),
      EASYSIMD_FLOAT64_C(  -138.82), EASYSIMD_FLOAT64_C(    26.05), EASYSIMD_FLOAT64_C(   266.18), EASYSIMD_FLOAT64_C(   131.68),
      EASYSIMD_FLOAT64_C(   888.79), EASYSIMD_FLOAT64_C(  -601.92), EASYSIMD_FLOAT64_C(  -493.18), EASYSIMD_FLOAT64_C(   985.91),
      EASYSIMD_FLOAT64_C(    14.35), EASYSIMD_FLOAT64_C(   894.05), EASYSIMD_FLOAT64_C(  -311.39), EASYSIMD_FLOAT64_C(   312.69),
      EASYSIMD_FLOAT64_C(   253.76), EASYSIMD_FLOAT64_C(  -853.62), EASYSIMD_FLOAT64_C(  -211.23), EASYSIMD_FLOAT64_C(   307.29),
      EASYSIMD_FLOAT64_C(  -497.41), EASYSIMD_FLOAT64_C(   188.15), EASYSIMD_FLOAT64_C(  -710.91), EASYSIMD_FLOAT64_C(  -363.14),
      EASYSIMD_FLOAT64_C(  -116.50), EASYSIMD_FLOAT64_C(    49.27), EASYSIMD_FLOAT64_C(    62.56), EASYSIMD_FLOAT64_C(   440.98),
      EASYSIMD_FLOAT64_C(   578.57), EASYSIMD_FLOAT64_C(    26.17), EASYSIMD_FLOAT64_C(   781.43), EASYSIMD_FLOAT64_C(  -298.23),
      EASYSIMD_FLOAT64_C(  -701.21), EASYSIMD_FLOAT64_C(  -677.50), EASYSIMD_FLOAT64_C(   998.19), EASYSIMD_FLOAT64_C(   159.97),
      EASYSIMD_FLOAT64_C(   348.56), EASYSIMD_FLOAT64_C(   264.37), EASYSIMD_FLOAT64_C(  -708.35), EASYSIMD_FLOAT64_C(   237.35),
      EASYSIMD_FLOAT64_C(   662.45), EASYSIMD_FLOAT64_C(  -201.52), EASYSIMD_FLOAT64_C(   223.26), EASYSIMD_FLOAT64_C(  -323.20),
      EASYSIMD_FLOAT64_C(  -307.47), EASYSIMD_FLOAT64_C(   911.87), EASYSIMD_FLOAT64_C(   989.49), EASYSIMD_FLOAT64_C(   946.29),
      EASYSIMD_FLOAT64_C(  -941.76), EASYSIMD_FLOAT64_C(  -221.74), EASYSIMD_FLOAT64_C(   253.57), EASYSIMD_FLOAT64_C(  -439.16),
      EASYSIMD_FLOAT64_C(   966.41), EASYSIMD_FLOAT64_C(   542.67) };
  static const easysimd_float64 e[] =
    { EASYSIMD_FLOAT64_C(  -120.40), EASYSIMD_FLOAT64_C(  -627.34), EASYSIMD_FLOAT64_C(  -389.72), EASYSIMD_FLOAT64_C( -1060.89),
      EASYSIMD_FLOAT64_C(   566.16), EASYSIMD_FLOAT64_C(   990.74), EASYSIMD_FLOAT64_C(  1210.07), EASYSIMD_FLOAT64_C(   -79.15),
      EASYSIMD_FLOAT64_C(  -175.22), EASYSIMD_FLOAT64_C(  1043.72), EASYSIMD_FLOAT64_C( -1041.28), EASYSIMD_FLOAT64_C(   875.74),
      EASYSIMD_FLOAT64_C(  -720.44), EASYSIMD_FLOAT64_C(  -715.64), EASYSIMD_FLOAT64_C(   796.54), EASYSIMD_FLOAT64_C(  -186.39),
      EASYSIMD_FLOAT64_C(  -622.24), EASYSIMD_FLOAT64_C(  -439.78), EASYSIMD_FLOAT64_C(   104.64), EASYSIMD_FLOAT64_C(   234.38),
      EASYSIMD_FLOAT64_C(   484.37), EASYSIMD_FLOAT64_C(  -987.92), EASYSIMD_FLOAT64_C(  -394.43), EASYSIMD_FLOAT64_C(   682.07),
      EASYSIMD_FLOAT64_C(   972.48), EASYSIMD_FLOAT64_C(  -244.92), EASYSIMD_FLOAT64_C(   -33.49), EASYSIMD_FLOAT64_C(     1.12),
      EASYSIMD_FLOAT64_C(  -598.97), EASYSIMD_FLOAT64_C(   -76.70), EASYSIMD_FLOAT64_C(   -71.73), EASYSIMD_FLOAT64_C(   414.91),
      EASYSIMD_FLOAT64_C(   179.46), EASYSIMD_FLOAT64_C(   778.37), EASYSIMD_FLOAT64_C(  -646.00), EASYSIMD_FLOAT64_C(  -137.87),
      EASYSIMD_FLOAT64_C(  1347.69), EASYSIMD_FLOAT64_C(   537.90), EASYSIMD_FLOAT64_C(   782.98), EASYSIMD_FLOAT64_C(  -704.33),
      EASYSIMD_FLOAT64_C(   309.01), EASYSIMD_FLOAT64_C( -1476.88), EASYSIMD_FLOAT64_C(   171.41), EASYSIMD_FLOAT64_C(   588.57),
      EASYSIMD_FLOAT64_C(  -166.46), EASYSIMD_FLOAT64_C(   969.76), EASYSIMD_FLOAT64_C( -1437.85), EASYSIMD_FLOAT64_C(  -137.26),
      EASYSIMD_FLOAT64_C(   794.35), EASYSIMD_FLOAT64_C(   173.62), EASYSIMD_FLOAT64_C(  -679.62), EASYSIMD_FLOAT64_C(  -706.93),
      EASYSIMD_FLOAT64_C(   387.23), EASYSIMD_FLOAT64_C(   614.56), EASYSIMD_FLOAT64_C(   287.83), EASYSIMD_FLOAT64_C(   359.71),
      EASYSIMD_FLOAT64_C(   457.77), EASYSIMD_FLOAT64_C(   254.34), EASYSIMD_FLOAT64_C(  -946.47), EASYSIMD_FLOAT64_C(  -643.78),
      EASYSIMD_FLOAT64_C(  -600.62), EASYSIMD_FLOAT64_C(   -18.19) };
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_f64_m(pv, av, bv);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_f64_m(pv, av, bv);
...
static int
test_easysimd_svadd_n_s8_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_n_s8_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(b), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_i8();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b8_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    easysimd_svint8_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(0));
    av = easysimd_svld1_s8(pg, &(a[i]));

    ev = easysimd_svadd_n_s8_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1i8(1, b);
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_s8_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
    { -INT8_C(   1),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1),  INT8_C(   0),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_n_s8_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    ev = easysimd_svadd_n_s8_m(pv, av, b);
...
static int
test_easysimd_svadd_n_s16_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
    {  INT16_C(     0),  INT16_C(     0),  INT16_C(     0),  INT16_C(     0), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1),
      -INT16_C(     1),  INT16_C(     0),  INT16_C(     0), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_n_s16_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_i16();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b16_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    easysimd_svint16_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(0));
    av = easysimd_svld1_s16(pg, &(a[i]));

    ev = easysimd_svadd_n_s16_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1i16(1, b);
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_s16_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_n_s16_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    ev = easysimd_svadd_n_s16_m(pv, av, b);
...
static int
test_easysimd_svadd_n_s32_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
    {  INT32_C(           0), -INT32_C(           1), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_n_s32_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_i32();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svint32_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(0));
    av = easysimd_svld1_s32(pg, &(a[i]));

    ev = easysimd_svadd_n_s32_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1i32(1, b);
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_s32_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
    { -INT32_C(           1), -INT32_C(           1), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),
       INT32_C(           0),  INT32_C(           0),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_n_s32_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_n_s32_m(pv, av, b);
...
static int
test_easysimd_svadd_n_s64_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    { -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),  INT64_C(                   0),
      -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_n_s64_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_i64();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svint64_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(0));
    av = easysimd_svld1_s64(pg, &(a[i]));

    ev = easysimd_svadd_n_s64_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1i64(1, b);
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_s64_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    {  INT64_C(                   0),  INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0),
      -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),  INT64_C(                   0),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_n_s64_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_n_s64_m(pv, av, b);
...
static int
test_easysimd_svadd_n_u8_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
    {  INT8_C(   0),  INT8_C(   0), -INT8_C(   1),  INT8_C(   0),  INT8_C(   0),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1),
       INT8_C(   0), -INT8_C(   1),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1), -INT8_C(   1), -INT8_C(   1),  INT8_C(   0),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_n_u8_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_u8();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b8_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    easysimd_svuint8_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(0));
    av = easysimd_svld1_u8(pg, &(a[i]));

    ev = easysimd_svadd_n_u8_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi8(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu8(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1u8(1, b);
  easysimd_test_codegen_write_1vu8(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_u8_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int8_t p[] =
    { -INT8_C(   1), -INT8_C(   1), -INT8_C(   1),  INT8_C(   0),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1),  INT8_C(   0),
      -INT8_C(   1), -INT8_C(   1),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1),  INT8_C(   0), -INT8_C(   1), -INT8_C(   1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    pv = easysimd_svcmplt_s8(pg, easysimd_svld1_s8(pg, &(p[i])), easysimd_svdup_n_s8(INT8_C(0)));
    rv = easysimd_svadd_n_u8_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b8(), pg)) {
    ...
    ev = easysimd_svadd_n_u8_m(pv, av, b);
...
static int
test_easysimd_svadd_n_u16_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_n_u16_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_u16();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b16_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    easysimd_svuint16_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(0));
    av = easysimd_svld1_u16(pg, &(a[i]));

    ev = easysimd_svadd_n_u16_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi16(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu16(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1u16(1, b);
  easysimd_test_codegen_write_1vu16(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_u16_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int16_t p[] =
    {  INT16_C(     0), -INT16_C(     1),  INT16_C(     0),  INT16_C(     0),  INT16_C(     0), -INT16_C(     1),  INT16_C(     0),  INT16_C(     0),
       INT16_C(     0), -INT16_C(     1), -INT16_C(     1), -INT16_C(     1),  INT16_C(     0),  INT16_C(     0), -INT16_C(     1), -INT16_C(     1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    pv = easysimd_svcmplt_s16(pg, easysimd_svld1_s16(pg, &(p[i])), easysimd_svdup_n_s16(INT16_C(0)));
    rv = easysimd_svadd_n_u16_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b16(), pg)) {
    ...
    ev = easysimd_svadd_n_u16_m(pv, av, b);
...
static int
test_easysimd_svadd_n_u32_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
    {  INT32_C(           0),  INT32_C(           0),  INT32_C(           0),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0),
       INT32_C(           0), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0), -INT32_C(           1), -INT32_C(           1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_n_u32_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_u32();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svuint32_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(0));
    av = easysimd_svld1_u32(pg, &(a[i]));

    ev = easysimd_svadd_n_u32_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu32(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1u32(1, b);
  easysimd_test_codegen_write_1vu32(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_u32_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_n_u32_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_n_u32_m(pv, av, b);
...
static int
test_easysimd_svadd_n_u64_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    {  INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0),  INT64_C(                   0),
      -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1), -INT64_C(                   1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_n_u64_z(pv, av, b);
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_memory(sizeof(a), HEDLEY_REINTERPRET_CAST(uint8_t*, a));
  b = easysimd_test_codegen_random_u64();

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svuint64_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(0));
    av = easysimd_svld1_u64(pg, &(a[i]));

    ev = easysimd_svadd_n_u64_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vu64(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1u64(1, b);
  easysimd_test_codegen_write_1vu64(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_u64_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    {  INT64_C(                   0), -INT64_C(                   1), -INT64_C(                   1),  INT64_C(                   0),
       INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0), -INT64_C(                   1),
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_n_u64_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_n_u64_m(pv, av, b);
...
static int
test_easysimd_svadd_n_f32_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
    {  INT32_C(           0),  INT32_C(           0), -INT32_C(           1), -INT32_C(           1), -INT32_C(           1), -INT32_C(           1), -INT32_C(           1),  INT32_C(           0),
      -INT32_C(           1), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),  INT32_C(           0),  INT32_C(           0), -INT32_C(           1),
  ...
  static const easysimd_float32 a[] =
    { EASYSIMD_FLOAT32_C(    22.40), EASYSIMD_FLOAT32_C(  -650.28), EASYSIMD_FLOAT32_C(  -629.80), EASYSIMD_FLOAT32_C(  -779.73),
      EASYSIMD_FLOAT32_C(  -816.53), EASYSIMD_FLOAT32_C(   479.41), EASYSIMD_FLOAT32_C(   530.90), EASYSIMD_FLOAT32_C(  -489.84),
      EASYSIMD_FLOAT32_C(  -596.91), EASYSIMD_FLOAT32_C(  -567.85), EASYSIMD_FLOAT32_C(    -2.49), EASYSIMD_FLOAT32_C(   623.58),
      EASYSIMD_FLOAT32_C(     5.37), EASYSIMD_FLOAT32_C(  -196.00), EASYSIMD_FLOAT32_C(   111.39), EASYSIMD_FLOAT32_C(    49.57),
      EASYSIMD_FLOAT32_C(  -871.65), EASYSIMD_FLOAT32_C(   396.44), EASYSIMD_FLOAT32_C(   129.00), EASYSIMD_FLOAT32_C(    54.68),
      EASYSIMD_FLOAT32_C(  -702.68), EASYSIMD_FLOAT32_C(  -764.69), EASYSIMD_FLOAT32_C(   938.92), EASYSIMD_FLOAT32_C(   736.37),
      EASYSIMD_FLOAT32_C(   -23.42), EASYSIMD_FLOAT32_C(  -845.78), EASYSIMD_FLOAT32_C(  -324.63), EASYSIMD_FLOAT32_C(   131.23),
      EASYSIMD_FLOAT32_C(   771.45), EASYSIMD_FLOAT32_C(   220.71), EASYSIMD_FLOAT32_C(  -575.48), EASYSIMD_FLOAT32_C(  -206.15),
      EASYSIMD_FLOAT32_C(   570.43), EASYSIMD_FLOAT32_C(  -205.28), EASYSIMD_FLOAT32_C(    14.12), EASYSIMD_FLOAT32_C(   753.90),
      EASYSIMD_FLOAT32_C(  -725.88), EASYSIMD_FLOAT32_C(  -454.97), EASYSIMD_FLOAT32_C(  -735.94), EASYSIMD_FLOAT32_C(  -322.78),
      EASYSIMD_FLOAT32_C(   -22.83), EASYSIMD_FLOAT32_C(   261.58), EASYSIMD_FLOAT32_C(  -699.21), EASYSIMD_FLOAT32_C(   982.55),
      EASYSIMD_FLOAT32_C(  -934.43), EASYSIMD_FLOAT32_C(   412.19), EASYSIMD_FLOAT32_C(    32.11), EASYSIMD_FLOAT32_C(  -806.08),
      EASYSIMD_FLOAT32_C(  -191.37), EASYSIMD_FLOAT32_C(  -838.89), EASYSIMD_FLOAT32_C(   248.60), EASYSIMD_FLOAT32_C(   105.95),
      EASYSIMD_FLOAT32_C(  -603.57), EASYSIMD_FLOAT32_C(   187.52), EASYSIMD_FLOAT32_C(  -157.68), EASYSIMD_FLOAT32_C(   373.01),
      EASYSIMD_FLOAT32_C(   341.75), EASYSIMD_FLOAT32_C(   517.69), EASYSIMD_FLOAT32_C(  -495.76), EASYSIMD_FLOAT32_C(   113.20),
      EASYSIMD_FLOAT32_C(  -261.60), EASYSIMD_FLOAT32_C(   -71.25), EASYSIMD_FLOAT32_C(   907.05), EASYSIMD_FLOAT32_C(  -691.17),
      EASYSIMD_FLOAT32_C(   723.47), EASYSIMD_FLOAT32_C(   -78.83), EASYSIMD_FLOAT32_C(  -937.26), EASYSIMD_FLOAT32_C(   997.59),
      EASYSIMD_FLOAT32_C(   466.20), EASYSIMD_FLOAT32_C(  -673.20), EASYSIMD_FLOAT32_C(  -325.19), EASYSIMD_FLOAT32_C(  -556.63),
      EASYSIMD_FLOAT32_C(   588.38), EASYSIMD_FLOAT32_C(   -24.40), EASYSIMD_FLOAT32_C(  -574.08), EASYSIMD_FLOAT32_C(   653.95),
      EASYSIMD_FLOAT32_C(  -612.22), EASYSIMD_FLOAT32_C(   458.03), EASYSIMD_FLOAT32_C(   847.87), EASYSIMD_FLOAT32_C(   196.41),
      EASYSIMD_FLOAT32_C(   619.14), EASYSIMD_FLOAT32_C(    96.47), EASYSIMD_FLOAT32_C(  -697.64), EASYSIMD_FLOAT32_C(  -984.43),
      EASYSIMD_FLOAT32_C(  -716.00), EASYSIMD_FLOAT32_C(   144.68), EASYSIMD_FLOAT32_C(   388.58), EASYSIMD_FLOAT32_C(   625.74),
      EASYSIMD_FLOAT32_C(  -337.63), EASYSIMD_FLOAT32_C(   892.81), EASYSIMD_FLOAT32_C(  -261.06), EASYSIMD_FLOAT32_C(   400.77),
      EASYSIMD_FLOAT32_C(  -178.44), EASYSIMD_FLOAT32_C(  -354.01), EASYSIMD_FLOAT32_C(   709.60), EASYSIMD_FLOAT32_C(  -454.97),
      EASYSIMD_FLOAT32_C(   567.16), EASYSIMD_FLOAT32_C(   772.34), EASYSIMD_FLOAT32_C(  -457.38), EASYSIMD_FLOAT32_C(    33.36),
      EASYSIMD_FLOAT32_C(  -900.86), EASYSIMD_FLOAT32_C(   217.43), EASYSIMD_FLOAT32_C(   476.73), EASYSIMD_FLOAT32_C(   687.51),
      EASYSIMD_FLOAT32_C(  -806.97), EASYSIMD_FLOAT32_C(   902.65), EASYSIMD_FLOAT32_C(   341.46), EASYSIMD_FLOAT32_C(  -419.19),
      EASYSIMD_FLOAT32_C(   360.68), EASYSIMD_FLOAT32_C(   189.33), EASYSIMD_FLOAT32_C(   777.22), EASYSIMD_FLOAT32_C(   -20.18),
      EASYSIMD_FLOAT32_C(  -714.19), EASYSIMD_FLOAT32_C(  -920.42), EASYSIMD_FLOAT32_C(    -4.61), EASYSIMD_FLOAT32_C(  -430.20),
      EASYSIMD_FLOAT32_C(   224.26), EASYSIMD_FLOAT32_C(  -616.03), EASYSIMD_FLOAT32_C(  -804.46), EASYSIMD_FLOAT32_C(   886.63),
      EASYSIMD_FLOAT32_C(  -723.22), EASYSIMD_FLOAT32_C(   -65.52), EASYSIMD_FLOAT32_C(   287.39), EASYSIMD_FLOAT32_C(    98.34),
      EASYSIMD_FLOAT32_C(   580.47), EASYSIMD_FLOAT32_C(    -3.01), EASYSIMD_FLOAT32_C(   643.37), EASYSIMD_FLOAT32_C(   147.63),
      EASYSIMD_FLOAT32_C(  -230.67), EASYSIMD_FLOAT32_C(  -814.00), EASYSIMD_FLOAT32_C(  -819.01), EASYSIMD_FLOAT32_C(  -131.54),
      EASYSIMD_FLOAT32_C(   403.43), EASYSIMD_FLOAT32_C(   657.72), EASYSIMD_FLOAT32_C(  -444.02), EASYSIMD_FLOAT32_C(   596.46),
      EASYSIMD_FLOAT32_C(   560.36), EASYSIMD_FLOAT32_C(   897.44), EASYSIMD_FLOAT32_C(  -822.73), EASYSIMD_FLOAT32_C(   -78.96),
      EASYSIMD_FLOAT32_C(    86.77), EASYSIMD_FLOAT32_C(   954.49), EASYSIMD_FLOAT32_C(   900.86), EASYSIMD_FLOAT32_C(   372.58),
      EASYSIMD_FLOAT32_C(  -965.92), EASYSIMD_FLOAT32_C(  -103.75), EASYSIMD_FLOAT32_C(   942.38), EASYSIMD_FLOAT32_C(   258.34),
      EASYSIMD_FLOAT32_C(   280.22), EASYSIMD_FLOAT32_C(  -862.08) };
  static const easysimd_float32 b = EASYSIMD_FLOAT32_C(   144.96);
  static const easysimd_float32 e[] =
    { EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -484.84), EASYSIMD_FLOAT32_C(  -634.77),
      EASYSIMD_FLOAT32_C(  -671.57), EASYSIMD_FLOAT32_C(   624.37), EASYSIMD_FLOAT32_C(   675.86), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -451.95), EASYSIMD_FLOAT32_C(  -422.89), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   150.33), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   194.53),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   273.96), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   881.33),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -179.67), EASYSIMD_FLOAT32_C(   276.19),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   159.08), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -580.92), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -590.98), EASYSIMD_FLOAT32_C(  -177.82),
      EASYSIMD_FLOAT32_C(   122.13), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -554.25), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -789.47), EASYSIMD_FLOAT32_C(   557.15), EASYSIMD_FLOAT32_C(   177.07), EASYSIMD_FLOAT32_C(  -661.12),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   393.56), EASYSIMD_FLOAT32_C(   250.91),
      EASYSIMD_FLOAT32_C(  -458.61), EASYSIMD_FLOAT32_C(   332.48), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   517.97),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -350.80), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(    73.71), EASYSIMD_FLOAT32_C(  1052.01), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(   611.16), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -411.67),
      EASYSIMD_FLOAT32_C(   733.34), EASYSIMD_FLOAT32_C(   120.56), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   992.83), EASYSIMD_FLOAT32_C(   341.37),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   241.43), EASYSIMD_FLOAT32_C(  -552.68), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -571.04), EASYSIMD_FLOAT32_C(   289.64), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   770.70),
      EASYSIMD_FLOAT32_C(  -192.67), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   545.73),
      EASYSIMD_FLOAT32_C(   -33.48), EASYSIMD_FLOAT32_C(  -209.05), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -310.01),
      EASYSIMD_FLOAT32_C(   712.12), EASYSIMD_FLOAT32_C(   917.30), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   178.32),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   362.39), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  1047.61), EASYSIMD_FLOAT32_C(   486.42), EASYSIMD_FLOAT32_C(  -274.23),
      EASYSIMD_FLOAT32_C(   505.64), EASYSIMD_FLOAT32_C(   334.29), EASYSIMD_FLOAT32_C(   922.18), EASYSIMD_FLOAT32_C(   124.78),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -285.24),
      EASYSIMD_FLOAT32_C(   369.22), EASYSIMD_FLOAT32_C(  -471.07), EASYSIMD_FLOAT32_C(  -659.50), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   432.35), EASYSIMD_FLOAT32_C(   243.30),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   788.33), EASYSIMD_FLOAT32_C(   292.59),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -669.04), EASYSIMD_FLOAT32_C(  -674.05), EASYSIMD_FLOAT32_C(    13.42),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  1099.45), EASYSIMD_FLOAT32_C(  1045.82), EASYSIMD_FLOAT32_C(     0.00),
      EASYSIMD_FLOAT32_C(  -820.96), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(   403.30),
      EASYSIMD_FLOAT32_C(     0.00), EASYSIMD_FLOAT32_C(  -717.12) };
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_n_f32_z(pv, av, b);
...
#else
  int32_t p[1024 / sizeof(int32_t)];
  easysimd_float32 a[1024 / sizeof(easysimd_float32)], b, e[1024 / sizeof(easysimd_float32)];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_vf32(HEDLEY_STATIC_CAST(size_t, len), a, EASYSIMD_FLOAT32_C(-1000.0), EASYSIMD_FLOAT32_C(1000.0));
  b = easysimd_test_codegen_random_f32(EASYSIMD_FLOAT32_C(-1000.0), EASYSIMD_FLOAT32_C(1000.0));

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b32_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    easysimd_svfloat32_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(0));
    av = easysimd_svld1_f32(pg, &(a[i]));

    ev = easysimd_svadd_n_f32_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi32(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vf32(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1f32(1, b);
  easysimd_test_codegen_write_1vf32(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_f32_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int32_t p[] =
  ...
  static const easysimd_float32 a[] =
    { EASYSIMD_FLOAT32_C(   794.89), EASYSIMD_FLOAT32_C(  -962.67), EASYSIMD_FLOAT32_C(  -950.01), EASYSIMD_FLOAT32_C(   497.52),
      EASYSIMD_FLOAT32_C(  -992.94), EASYSIMD_FLOAT32_C(   125.39), EASYSIMD_FLOAT32_C(  -116.49), EASYSIMD_FLOAT32_C(   718.28),
      EASYSIMD_FLOAT32_C(  -383.64), EASYSIMD_FLOAT32_C(  -362.50), EASYSIMD_FLOAT32_C(  -817.24), EASYSIMD_FLOAT32_C(  -615.61),
      EASYSIMD_FLOAT32_C(  -983.13), EASYSIMD_FLOAT32_C(   446.88), EASYSIMD_FLOAT32_C(  -562.02), EASYSIMD_FLOAT32_C(    78.78),
      EASYSIMD_FLOAT32_C(  -357.68), EASYSIMD_FLOAT32_C(   708.96), EASYSIMD_FLOAT32_C(  -635.94), EASYSIMD_FLOAT32_C(   354.73),
      EASYSIMD_FLOAT32_C(   -36.72), EASYSIMD_FLOAT32_C(   514.18), EASYSIMD_FLOAT32_C(  -777.98), EASYSIMD_FLOAT32_C(    93.03),
      EASYSIMD_FLOAT32_C(  -873.72), EASYSIMD_FLOAT32_C(   115.54), EASYSIMD_FLOAT32_C(  -132.06), EASYSIMD_FLOAT32_C(   149.47),
      EASYSIMD_FLOAT32_C(   774.90), EASYSIMD_FLOAT32_C(   568.38), EASYSIMD_FLOAT32_C(   -50.85), EASYSIMD_FLOAT32_C(   569.78),
      EASYSIMD_FLOAT32_C(   605.71), EASYSIMD_FLOAT32_C(    -0.86), EASYSIMD_FLOAT32_C(    67.30), EASYSIMD_FLOAT32_C(   612.77),
      EASYSIMD_FLOAT32_C(  -875.47), EASYSIMD_FLOAT32_C(   950.81), EASYSIMD_FLOAT32_C(   331.05), EASYSIMD_FLOAT32_C(  -259.11),
      EASYSIMD_FLOAT32_C(  -411.69), EASYSIMD_FLOAT32_C(   513.80), EASYSIMD_FLOAT32_C(   125.28), EASYSIMD_FLOAT32_C(  -394.82),
      EASYSIMD_FLOAT32_C(   -39.32), EASYSIMD_FLOAT32_C(   563.25), EASYSIMD_FLOAT32_C(   683.96), EASYSIMD_FLOAT32_C(   603.00),
      EASYSIMD_FLOAT32_C(   272.21), EASYSIMD_FLOAT32_C(  -951.98), EASYSIMD_FLOAT32_C(   -42.27), EASYSIMD_FLOAT32_C(  -764.51),
      EASYSIMD_FLOAT32_C(   562.21), EASYSIMD_FLOAT32_C(   179.75), EASYSIMD_FLOAT32_C(   328.52), EASYSIMD_FLOAT32_C(   688.48),
      EASYSIMD_FLOAT32_C(  -704.70), EASYSIMD_FLOAT32_C(  -803.54), EASYSIMD_FLOAT32_C(  -162.04), EASYSIMD_FLOAT32_C(  -929.80),
      EASYSIMD_FLOAT32_C(   764.84), EASYSIMD_FLOAT32_C(   787.11), EASYSIMD_FLOAT32_C(   639.98), EASYSIMD_FLOAT32_C(   370.55),
      EASYSIMD_FLOAT32_C(  -213.76), EASYSIMD_FLOAT32_C(  -292.72), EASYSIMD_FLOAT32_C(   -16.68), EASYSIMD_FLOAT32_C(   -89.23),
      EASYSIMD_FLOAT32_C(  -341.91), EASYSIMD_FLOAT32_C(  -685.63), EASYSIMD_FLOAT32_C(   651.66), EASYSIMD_FLOAT32_C(   246.40),
      EASYSIMD_FLOAT32_C(   828.17), EASYSIMD_FLOAT32_C(  -223.06), EASYSIMD_FLOAT32_C(   851.57), EASYSIMD_FLOAT32_C(  -211.15),
      EASYSIMD_FLOAT32_C(  -659.81), EASYSIMD_FLOAT32_C(   535.53), EASYSIMD_FLOAT32_C(  -608.15), EASYSIMD_FLOAT32_C(   612.40),
      EASYSIMD_FLOAT32_C(   583.55), EASYSIMD_FLOAT32_C(   349.59), EASYSIMD_FLOAT32_C(   847.88), EASYSIMD_FLOAT32_C(   145.76),
      EASYSIMD_FLOAT32_C(  -470.66), EASYSIMD_FLOAT32_C(   176.40), EASYSIMD_FLOAT32_C(  -165.76), EASYSIMD_FLOAT32_C(  -175.36),
      EASYSIMD_FLOAT32_C(   372.86), EASYSIMD_FLOAT32_C(   672.20), EASYSIMD_FLOAT32_C(  -105.16), EASYSIMD_FLOAT32_C(   137.70),
      EASYSIMD_FLOAT32_C(   459.31), EASYSIMD_FLOAT32_C(  -465.19), EASYSIMD_FLOAT32_C(  -491.75), EASYSIMD_FLOAT32_C(  -754.45),
      EASYSIMD_FLOAT32_C(   242.09), EASYSIMD_FLOAT32_C(   491.57), EASYSIMD_FLOAT32_C(   156.33), EASYSIMD_FLOAT32_C(   900.18),
      EASYSIMD_FLOAT32_C(   805.94), EASYSIMD_FLOAT32_C(  -192.01), EASYSIMD_FLOAT32_C(   146.57), EASYSIMD_FLOAT32_C(   634.11),
      EASYSIMD_FLOAT32_C(   584.92), EASYSIMD_FLOAT32_C(    -1.86), EASYSIMD_FLOAT32_C(  -577.03), EASYSIMD_FLOAT32_C(   925.11),
      EASYSIMD_FLOAT32_C(  -466.33), EASYSIMD_FLOAT32_C(  -185.18), EASYSIMD_FLOAT32_C(   537.51), EASYSIMD_FLOAT32_C(  -882.78),
      EASYSIMD_FLOAT32_C(  -835.59), EASYSIMD_FLOAT32_C(   385.39), EASYSIMD_FLOAT32_C(   262.98), EASYSIMD_FLOAT32_C(  -306.25),
      EASYSIMD_FLOAT32_C(  -438.21), EASYSIMD_FLOAT32_C(  -902.78), EASYSIMD_FLOAT32_C(   518.39), EASYSIMD_FLOAT32_C(   934.65),
      EASYSIMD_FLOAT32_C(   769.42), EASYSIMD_FLOAT32_C(  -586.78), EASYSIMD_FLOAT32_C(    72.35), EASYSIMD_FLOAT32_C(   228.73),
      EASYSIMD_FLOAT32_C(   -51.96), EASYSIMD_FLOAT32_C(   580.59), EASYSIMD_FLOAT32_C(   474.28), EASYSIMD_FLOAT32_C(  -809.87),
      EASYSIMD_FLOAT32_C(    72.16), EASYSIMD_FLOAT32_C(  -369.39), EASYSIMD_FLOAT32_C(  -909.70), EASYSIMD_FLOAT32_C(  -121.89),
      EASYSIMD_FLOAT32_C(   438.59), EASYSIMD_FLOAT32_C(   236.88), EASYSIMD_FLOAT32_C(  -487.78), EASYSIMD_FLOAT32_C(    23.51),
      EASYSIMD_FLOAT32_C(  -764.98), EASYSIMD_FLOAT32_C(   -64.81), EASYSIMD_FLOAT32_C(   -51.37), EASYSIMD_FLOAT32_C(  -231.31),
      EASYSIMD_FLOAT32_C(   750.01), EASYSIMD_FLOAT32_C(  -513.87), EASYSIMD_FLOAT32_C(  -114.08), EASYSIMD_FLOAT32_C(   914.41),
      EASYSIMD_FLOAT32_C(   871.53), EASYSIMD_FLOAT32_C(  -851.10), EASYSIMD_FLOAT32_C(  -391.84), EASYSIMD_FLOAT32_C(  -566.68),
      EASYSIMD_FLOAT32_C(  -753.88), EASYSIMD_FLOAT32_C(  -873.45), EASYSIMD_FLOAT32_C(  -632.03), EASYSIMD_FLOAT32_C(  -984.46),
      EASYSIMD_FLOAT32_C(  -460.23), EASYSIMD_FLOAT32_C(   440.32), EASYSIMD_FLOAT32_C(   244.27), EASYSIMD_FLOAT32_C(   487.81),
      EASYSIMD_FLOAT32_C(    20.91), EASYSIMD_FLOAT32_C(  -281.45), EASYSIMD_FLOAT32_C(   677.93), EASYSIMD_FLOAT32_C(  -906.93),
      EASYSIMD_FLOAT32_C(   349.15), EASYSIMD_FLOAT32_C(   768.24), EASYSIMD_FLOAT32_C(   -28.82), EASYSIMD_FLOAT32_C(  -212.26),
      EASYSIMD_FLOAT32_C(     5.11), EASYSIMD_FLOAT32_C(   483.40), EASYSIMD_FLOAT32_C(   811.26), EASYSIMD_FLOAT32_C(   240.13),
      EASYSIMD_FLOAT32_C(  -581.41), EASYSIMD_FLOAT32_C(  -240.12), EASYSIMD_FLOAT32_C(  -991.17), EASYSIMD_FLOAT32_C(  -831.40),
      EASYSIMD_FLOAT32_C(   246.02), EASYSIMD_FLOAT32_C(  -105.25), EASYSIMD_FLOAT32_C(  -916.99), EASYSIMD_FLOAT32_C(   117.55),
      EASYSIMD_FLOAT32_C(    43.64), EASYSIMD_FLOAT32_C(  -308.83), EASYSIMD_FLOAT32_C(   550.87), EASYSIMD_FLOAT32_C(   289.76),
      EASYSIMD_FLOAT32_C(  -182.29), EASYSIMD_FLOAT32_C(   918.84), EASYSIMD_FLOAT32_C(   305.30), EASYSIMD_FLOAT32_C(   357.48),
      EASYSIMD_FLOAT32_C(   359.15), EASYSIMD_FLOAT32_C(  -450.43), EASYSIMD_FLOAT32_C(  -154.71), EASYSIMD_FLOAT32_C(  -619.94) };
  static const easysimd_float32 b = EASYSIMD_FLOAT32_C(   268.11);
  static const easysimd_float32 e[] =
    { EASYSIMD_FLOAT32_C(  1063.00), EASYSIMD_FLOAT32_C(  -694.56), EASYSIMD_FLOAT32_C(  -681.90), EASYSIMD_FLOAT32_C(   497.52),
      EASYSIMD_FLOAT32_C(  -724.83), EASYSIMD_FLOAT32_C(   393.50), EASYSIMD_FLOAT32_C(   151.62), EASYSIMD_FLOAT32_C(   986.39),
      EASYSIMD_FLOAT32_C(  -115.53), EASYSIMD_FLOAT32_C(   -94.39), EASYSIMD_FLOAT32_C(  -549.13), EASYSIMD_FLOAT32_C(  -615.61),
      EASYSIMD_FLOAT32_C(  -715.02), EASYSIMD_FLOAT32_C(   446.88), EASYSIMD_FLOAT32_C(  -562.02), EASYSIMD_FLOAT32_C(    78.78),
      EASYSIMD_FLOAT32_C(  -357.68), EASYSIMD_FLOAT32_C(   977.07), EASYSIMD_FLOAT32_C(  -635.94), EASYSIMD_FLOAT32_C(   354.73),
      EASYSIMD_FLOAT32_C(   231.39), EASYSIMD_FLOAT32_C(   514.18), EASYSIMD_FLOAT32_C(  -777.98), EASYSIMD_FLOAT32_C(    93.03),
      EASYSIMD_FLOAT32_C(  -605.61), EASYSIMD_FLOAT32_C(   383.65), EASYSIMD_FLOAT32_C(   136.05), EASYSIMD_FLOAT32_C(   149.47),
      EASYSIMD_FLOAT32_C(   774.90), EASYSIMD_FLOAT32_C(   836.49), EASYSIMD_FLOAT32_C(   -50.85), EASYSIMD_FLOAT32_C(   837.89),
      EASYSIMD_FLOAT32_C(   605.71), EASYSIMD_FLOAT32_C(    -0.86), EASYSIMD_FLOAT32_C(   335.41), EASYSIMD_FLOAT32_C(   612.77),
      EASYSIMD_FLOAT32_C(  -607.36), EASYSIMD_FLOAT32_C(  1218.92), EASYSIMD_FLOAT32_C(   599.16), EASYSIMD_FLOAT32_C(  -259.11),
      EASYSIMD_FLOAT32_C(  -411.69), EASYSIMD_FLOAT32_C(   781.91), EASYSIMD_FLOAT32_C(   393.39), EASYSIMD_FLOAT32_C(  -126.71),
      EASYSIMD_FLOAT32_C(   -39.32), EASYSIMD_FLOAT32_C(   563.25), EASYSIMD_FLOAT32_C(   683.96), EASYSIMD_FLOAT32_C(   871.11),
      EASYSIMD_FLOAT32_C(   540.32), EASYSIMD_FLOAT32_C(  -683.87), EASYSIMD_FLOAT32_C(   -42.27), EASYSIMD_FLOAT32_C(  -496.40),
      EASYSIMD_FLOAT32_C(   830.32), EASYSIMD_FLOAT32_C(   447.86), EASYSIMD_FLOAT32_C(   596.63), EASYSIMD_FLOAT32_C(   688.48),
      EASYSIMD_FLOAT32_C(  -704.70), EASYSIMD_FLOAT32_C(  -803.54), EASYSIMD_FLOAT32_C(  -162.04), EASYSIMD_FLOAT32_C(  -929.80),
      EASYSIMD_FLOAT32_C(  1032.95), EASYSIMD_FLOAT32_C(  1055.22), EASYSIMD_FLOAT32_C(   908.09), EASYSIMD_FLOAT32_C(   638.66),
      EASYSIMD_FLOAT32_C(  -213.76), EASYSIMD_FLOAT32_C(  -292.72), EASYSIMD_FLOAT32_C(   251.43), EASYSIMD_FLOAT32_C(   -89.23),
      EASYSIMD_FLOAT32_C(   -73.80), EASYSIMD_FLOAT32_C(  -685.63), EASYSIMD_FLOAT32_C(   651.66), EASYSIMD_FLOAT32_C(   246.40),
      EASYSIMD_FLOAT32_C(  1096.28), EASYSIMD_FLOAT32_C(    45.05), EASYSIMD_FLOAT32_C(   851.57), EASYSIMD_FLOAT32_C(    56.96),
      EASYSIMD_FLOAT32_C(  -659.81), EASYSIMD_FLOAT32_C(   803.64), EASYSIMD_FLOAT32_C(  -608.15), EASYSIMD_FLOAT32_C(   612.40),
      EASYSIMD_FLOAT32_C(   851.66), EASYSIMD_FLOAT32_C(   349.59), EASYSIMD_FLOAT32_C(  1115.99), EASYSIMD_FLOAT32_C(   145.76),
      EASYSIMD_FLOAT32_C(  -470.66), EASYSIMD_FLOAT32_C(   176.40), EASYSIMD_FLOAT32_C(   102.35), EASYSIMD_FLOAT32_C(  -175.36),
      EASYSIMD_FLOAT32_C(   372.86), EASYSIMD_FLOAT32_C(   940.31), EASYSIMD_FLOAT32_C(  -105.16), EASYSIMD_FLOAT32_C(   405.81),
      EASYSIMD_FLOAT32_C(   727.42), EASYSIMD_FLOAT32_C(  -465.19), EASYSIMD_FLOAT32_C(  -223.64), EASYSIMD_FLOAT32_C(  -754.45),
      EASYSIMD_FLOAT32_C(   510.20), EASYSIMD_FLOAT32_C(   491.57), EASYSIMD_FLOAT32_C(   424.44), EASYSIMD_FLOAT32_C(   900.18),
      EASYSIMD_FLOAT32_C(  1074.05), EASYSIMD_FLOAT32_C(    76.10), EASYSIMD_FLOAT32_C(   146.57), EASYSIMD_FLOAT32_C(   634.11),
      EASYSIMD_FLOAT32_C(   853.03), EASYSIMD_FLOAT32_C(   266.25), EASYSIMD_FLOAT32_C(  -308.92), EASYSIMD_FLOAT32_C(  1193.22),
      EASYSIMD_FLOAT32_C(  -466.33), EASYSIMD_FLOAT32_C(  -185.18), EASYSIMD_FLOAT32_C(   805.62), EASYSIMD_FLOAT32_C(  -882.78),
      EASYSIMD_FLOAT32_C(  -567.48), EASYSIMD_FLOAT32_C(   653.50), EASYSIMD_FLOAT32_C(   262.98), EASYSIMD_FLOAT32_C(   -38.14),
      EASYSIMD_FLOAT32_C(  -438.21), EASYSIMD_FLOAT32_C(  -902.78), EASYSIMD_FLOAT32_C(   518.39), EASYSIMD_FLOAT32_C(   934.65),
      EASYSIMD_FLOAT32_C(  1037.53), EASYSIMD_FLOAT32_C(  -586.78), EASYSIMD_FLOAT32_C(    72.35), EASYSIMD_FLOAT32_C(   496.84),
      EASYSIMD_FLOAT32_C(   -51.96), EASYSIMD_FLOAT32_C(   848.70), EASYSIMD_FLOAT32_C(   742.39), EASYSIMD_FLOAT32_C(  -809.87),
      EASYSIMD_FLOAT32_C(   340.27), EASYSIMD_FLOAT32_C(  -369.39), EASYSIMD_FLOAT32_C(  -909.70), EASYSIMD_FLOAT32_C(   146.22),
      EASYSIMD_FLOAT32_C(   438.59), EASYSIMD_FLOAT32_C(   504.99), EASYSIMD_FLOAT32_C(  -219.67), EASYSIMD_FLOAT32_C(    23.51),
      EASYSIMD_FLOAT32_C(  -764.98), EASYSIMD_FLOAT32_C(   -64.81), EASYSIMD_FLOAT32_C(   216.74), EASYSIMD_FLOAT32_C(    36.80),
      EASYSIMD_FLOAT32_C(  1018.12), EASYSIMD_FLOAT32_C(  -513.87), EASYSIMD_FLOAT32_C(   154.03), EASYSIMD_FLOAT32_C(   914.41),
      EASYSIMD_FLOAT32_C(   871.53), EASYSIMD_FLOAT32_C(  -582.99), EASYSIMD_FLOAT32_C(  -123.73), EASYSIMD_FLOAT32_C(  -566.68),
      EASYSIMD_FLOAT32_C(  -753.88), EASYSIMD_FLOAT32_C(  -605.34), EASYSIMD_FLOAT32_C(  -632.03), EASYSIMD_FLOAT32_C(  -716.35),
      EASYSIMD_FLOAT32_C(  -460.23), EASYSIMD_FLOAT32_C(   708.43), EASYSIMD_FLOAT32_C(   244.27), EASYSIMD_FLOAT32_C(   487.81),
      EASYSIMD_FLOAT32_C(   289.02), EASYSIMD_FLOAT32_C(   -13.34), EASYSIMD_FLOAT32_C(   677.93), EASYSIMD_FLOAT32_C(  -906.93),
      EASYSIMD_FLOAT32_C(   349.15), EASYSIMD_FLOAT32_C(  1036.35), EASYSIMD_FLOAT32_C(   -28.82), EASYSIMD_FLOAT32_C(    55.85),
      EASYSIMD_FLOAT32_C(     5.11), EASYSIMD_FLOAT32_C(   483.40), EASYSIMD_FLOAT32_C(  1079.37), EASYSIMD_FLOAT32_C(   240.13),
      EASYSIMD_FLOAT32_C(  -313.30), EASYSIMD_FLOAT32_C(  -240.12), EASYSIMD_FLOAT32_C(  -991.17), EASYSIMD_FLOAT32_C(  -563.29),
      EASYSIMD_FLOAT32_C(   514.13), EASYSIMD_FLOAT32_C(   162.86), EASYSIMD_FLOAT32_C(  -916.99), EASYSIMD_FLOAT32_C(   385.66),
      EASYSIMD_FLOAT32_C(   311.75), EASYSIMD_FLOAT32_C(   -40.72), EASYSIMD_FLOAT32_C(   818.98), EASYSIMD_FLOAT32_C(   289.76),
      EASYSIMD_FLOAT32_C(  -182.29), EASYSIMD_FLOAT32_C(   918.84), EASYSIMD_FLOAT32_C(   305.30), EASYSIMD_FLOAT32_C(   357.48),
      EASYSIMD_FLOAT32_C(   359.15), EASYSIMD_FLOAT32_C(  -450.43), EASYSIMD_FLOAT32_C(   113.40), EASYSIMD_FLOAT32_C(  -619.94) };
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    pv = easysimd_svcmplt_s32(pg, easysimd_svld1_s32(pg, &(p[i])), easysimd_svdup_n_s32(INT32_C(0)));
    rv = easysimd_svadd_n_f32_m(pv, av, b);
...
#else
  int32_t p[1024 / sizeof(int32_t)];
  easysimd_float32 a[1024 / sizeof(int32_t)], b, e[1024 / sizeof(int32_t)];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b32(), pg)) {
    ...
    ev = easysimd_svadd_n_f32_m(pv, av, b);
...
static int
test_easysimd_svadd_n_f64_z (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    { -INT64_C(                   1),  INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0),
      -INT64_C(                   1), -INT64_C(                   1),  INT64_C(                   0), -INT64_C(                   1),
      ...
      -INT64_C(                   1),  INT64_C(                   0), -INT64_C(                   1), -INT64_C(                   1) };
  static const easysimd_float64 a[] =
    { EASYSIMD_FLOAT64_C(   530.20), EASYSIMD_FLOAT64_C(   909.18), EASYSIMD_FLOAT64_C(   110.70), EASYSIMD_FLOAT64_C(  -353.67),
      EASYSIMD_FLOAT64_C(  -963.42), EASYSIMD_FLOAT64_C(   465.68), EASYSIMD_FLOAT64_C(   790.66), EASYSIMD_FLOAT64_C(   969.70),
      EASYSIMD_FLOAT64_C(   981.06), EASYSIMD_FLOAT64_C(   264.73), EASYSIMD_FLOAT64_C(  -932.21), EASYSIMD_FLOAT64_C(    50.62),
      EASYSIMD_FLOAT64_C(   257.25), EASYSIMD_FLOAT64_C(  -884.42), EASYSIMD_FLOAT64_C(   447.89), EASYSIMD_FLOAT64_C(  -850.53),
      EASYSIMD_FLOAT64_C(  -132.51), EASYSIMD_FLOAT64_C(   909.82), EASYSIMD_FLOAT64_C(   281.05), EASYSIMD_FLOAT64_C(  -553.94),
      EASYSIMD_FLOAT64_C(  -806.12), EASYSIMD_FLOAT64_C(   969.99), EASYSIMD_FLOAT64_C(  -879.80), EASYSIMD_FLOAT64_C(   920.90),
      EASYSIMD_FLOAT64_C(  -550.94), EASYSIMD_FLOAT64_C(   875.87), EASYSIMD_FLOAT64_C(  -227.48), EASYSIMD_FLOAT64_C(    90.68),
      EASYSIMD_FLOAT64_C(   757.31), EASYSIMD_FLOAT64_C(  -358.48), EASYSIMD_FLOAT64_C(   706.99), EASYSIMD_FLOAT64_C(   287.51),
      EASYSIMD_FLOAT64_C(  -449.30), EASYSIMD_FLOAT64_C(  -182.31), EASYSIMD_FLOAT64_C(   933.84), EASYSIMD_FLOAT64_C(  -412.71),
      EASYSIMD_FLOAT64_C(  -716.63), EASYSIMD_FLOAT64_C(   724.50), EASYSIMD_FLOAT64_C(  -443.02), EASYSIMD_FLOAT64_C(  -735.57),
      EASYSIMD_FLOAT64_C(   -10.77), EASYSIMD_FLOAT64_C(  -375.23), EASYSIMD_FLOAT64_C(   315.05), EASYSIMD_FLOAT64_C(  -753.52),
      EASYSIMD_FLOAT64_C(  -259.64), EASYSIMD_FLOAT64_C(  -237.06), EASYSIMD_FLOAT64_C(  -604.06), EASYSIMD_FLOAT64_C(   607.84),
      EASYSIMD_FLOAT64_C(  -327.24), EASYSIMD_FLOAT64_C(   676.99), EASYSIMD_FLOAT64_C(  -946.10), EASYSIMD_FLOAT64_C(  -133.37),
      EASYSIMD_FLOAT64_C(   646.98), EASYSIMD_FLOAT64_C(  -825.90), EASYSIMD_FLOAT64_C(  -212.47), EASYSIMD_FLOAT64_C(  -903.96),
      EASYSIMD_FLOAT64_C(  -950.04), EASYSIMD_FLOAT64_C(   560.06), EASYSIMD_FLOAT64_C(   186.72), EASYSIMD_FLOAT64_C(   807.27),
      EASYSIMD_FLOAT64_C(  -798.43), EASYSIMD_FLOAT64_C(  -106.29), EASYSIMD_FLOAT64_C(    94.78), EASYSIMD_FLOAT64_C(  -247.72),
      EASYSIMD_FLOAT64_C(   711.40), EASYSIMD_FLOAT64_C(    28.62), EASYSIMD_FLOAT64_C(   339.56), EASYSIMD_FLOAT64_C(   994.77),
      EASYSIMD_FLOAT64_C(  -246.88), EASYSIMD_FLOAT64_C(   896.55), EASYSIMD_FLOAT64_C(  -740.80), EASYSIMD_FLOAT64_C(   742.35),
      EASYSIMD_FLOAT64_C(  -478.68), EASYSIMD_FLOAT64_C(   574.24), EASYSIMD_FLOAT64_C(   988.82), EASYSIMD_FLOAT64_C(   261.68),
      EASYSIMD_FLOAT64_C(  -662.82), EASYSIMD_FLOAT64_C(  -615.23), EASYSIMD_FLOAT64_C(  -130.48), EASYSIMD_FLOAT64_C(     9.94),
      EASYSIMD_FLOAT64_C(  -938.25), EASYSIMD_FLOAT64_C(   -76.58), EASYSIMD_FLOAT64_C(   876.58), EASYSIMD_FLOAT64_C(   708.73),
      EASYSIMD_FLOAT64_C(    97.51), EASYSIMD_FLOAT64_C(  -335.89), EASYSIMD_FLOAT64_C(   804.77), EASYSIMD_FLOAT64_C(   147.48),
      EASYSIMD_FLOAT64_C(  -775.83), EASYSIMD_FLOAT64_C(    -8.51), EASYSIMD_FLOAT64_C(   -45.25), EASYSIMD_FLOAT64_C(  -574.26),
      EASYSIMD_FLOAT64_C(   885.20), EASYSIMD_FLOAT64_C(  -950.47), EASYSIMD_FLOAT64_C(   178.02), EASYSIMD_FLOAT64_C(   596.60),
      EASYSIMD_FLOAT64_C(    78.15), EASYSIMD_FLOAT64_C(  -482.42), EASYSIMD_FLOAT64_C(   591.37), EASYSIMD_FLOAT64_C(   831.27),
      EASYSIMD_FLOAT64_C(  -585.87), EASYSIMD_FLOAT64_C(   850.56), EASYSIMD_FLOAT64_C(   573.61), EASYSIMD_FLOAT64_C(   -64.55),
      EASYSIMD_FLOAT64_C(   424.81), EASYSIMD_FLOAT64_C(   562.44), EASYSIMD_FLOAT64_C(  -802.87), EASYSIMD_FLOAT64_C(   761.99) };
  static const easysimd_float64 b = EASYSIMD_FLOAT64_C(   947.20);
  static const easysimd_float64 e[] =
    { EASYSIMD_FLOAT64_C(  1477.40), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1057.90), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   -16.22), EASYSIMD_FLOAT64_C(  1412.88), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1916.90),
      EASYSIMD_FLOAT64_C(  1928.26), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(  1204.45), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1857.02), EASYSIMD_FLOAT64_C(  1228.25), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   141.08), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   396.26), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   719.72), EASYSIMD_FLOAT64_C(  1037.88),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1654.19), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   497.90), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1881.04), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   230.57), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   571.97), EASYSIMD_FLOAT64_C(  1262.25), EASYSIMD_FLOAT64_C(   193.68),
      EASYSIMD_FLOAT64_C(   687.56), EASYSIMD_FLOAT64_C(   710.14), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   619.96), EASYSIMD_FLOAT64_C(  1624.19), EASYSIMD_FLOAT64_C(     1.10), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(  1594.18), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   734.73), EASYSIMD_FLOAT64_C(    43.24),
      EASYSIMD_FLOAT64_C(    -2.84), EASYSIMD_FLOAT64_C(  1507.26), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1754.47),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   840.91), EASYSIMD_FLOAT64_C(  1041.98), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(  1658.60), EASYSIMD_FLOAT64_C(   975.82), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(   700.32), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1689.55),
      EASYSIMD_FLOAT64_C(   468.52), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   816.72), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   870.62), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(  1044.71), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1094.68),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   372.94),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(    -3.27), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1543.80),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1538.57), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(  1520.81), EASYSIMD_FLOAT64_C(     0.00),
      EASYSIMD_FLOAT64_C(  1372.01), EASYSIMD_FLOAT64_C(     0.00), EASYSIMD_FLOAT64_C(   144.33), EASYSIMD_FLOAT64_C(  1709.19) };
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_n_f64_z(pv, av, b);
...
#else
  int64_t p[1024 / sizeof(int64_t)];
  easysimd_float64 a[1024 / sizeof(int64_t)], b, e[1024 / sizeof(int64_t)];
  int32_t len = easysimd_test_arm_sve_random_length(sizeof(e), sizeof(e[0]));
  ...
  easysimd_test_codegen_random_memory(sizeof(p), HEDLEY_REINTERPRET_CAST(uint8_t*, p));
  easysimd_test_codegen_random_vf64(HEDLEY_STATIC_CAST(size_t, len), a, EASYSIMD_FLOAT64_C(-1000.0), EASYSIMD_FLOAT64_C(1000.0));
  b = easysimd_test_codegen_random_f64(EASYSIMD_FLOAT64_C(-1000.0), EASYSIMD_FLOAT64_C(1000.0));

  int32_t i = 0;
  easysimd_svbool_t pg = easysimd_svwhilelt_b64_s32(i, len);
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    easysimd_svfloat64_t av, ev;
    easysimd_svbool_t pv;

    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(0));
    av = easysimd_svld1_f64(pg, &(a[i]));

    ev = easysimd_svadd_n_f64_z(pv, av, b);
  ...
  easysimd_test_codegen_write_1vi64(1, HEDLEY_STATIC_CAST(size_t, len), p);
  easysimd_test_codegen_write_1vf64(1, HEDLEY_STATIC_CAST(size_t, len), a);
  easysimd_test_codegen_write_1f64(1, b);
  easysimd_test_codegen_write_1vf64(1, HEDLEY_STATIC_CAST(size_t, len), e);
...
static int
test_easysimd_svadd_n_f64_m (EASYSIMD_MUNIT_TEST_ARGS) {
#if 1
  static const int64_t p[] =
    {  INT64_C(                   0), -INT64_C(                   1), -INT64_C(                   1),  INT64_C(                   0),
       INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0),  INT64_C(                   0),
      ...
      -INT64_C(                   1),  INT64_C(                   0), -INT64_C(                   1),  INT64_C(                   0),
       ...
       INT64_C(                   0), -INT64_C(                   1) };
  static const easysimd_float64 a[] =
    { EASYSIMD_FLOAT64_C(  -472.33), EASYSIMD_FLOAT64_C(  -730.51), EASYSIMD_FLOAT64_C(   672.00), EASYSIMD_FLOAT64_C(   364.85),
      EASYSIMD_FLOAT64_C(  -246.69), EASYSIMD_FLOAT64_C(  -256.46), EASYSIMD_FLOAT64_C(   778.81), EASYSIMD_FLOAT64_C(  -675.92),
      EASYSIMD_FLOAT64_C(   541.43), EASYSIMD_FLOAT64_C(   724.84), EASYSIMD_FLOAT64_C(   301.02), EASYSIMD_FLOAT64_C(   620.68),
      EASYSIMD_FLOAT64_C(   116.77), EASYSIMD_FLOAT64_C(  -947.06), EASYSIMD_FLOAT64_C(  -585.06), EASYSIMD_FLOAT64_C(   653.72),
      EASYSIMD_FLOAT64_C(    38.87), EASYSIMD_FLOAT64_C(   856.61), EASYSIMD_FLOAT64_C(   172.82), EASYSIMD_FLOAT64_C(   815.04),
      EASYSIMD_FLOAT64_C(  -546.82), EASYSIMD_FLOAT64_C(  -495.48), EASYSIMD_FLOAT64_C(   118.21), EASYSIMD_FLOAT64_C(   990.18),
      EASYSIMD_FLOAT64_C(   656.25), EASYSIMD_FLOAT64_C(  -811.95), EASYSIMD_FLOAT64_C(  -327.54), EASYSIMD_FLOAT64_C(   706.62),
      EASYSIMD_FLOAT64_C(   657.61), EASYSIMD_FLOAT64_C(  -415.78), EASYSIMD_FLOAT64_C(   259.81), EASYSIMD_FLOAT64_C(  -814.73),
      EASYSIMD_FLOAT64_C(  -146.29), EASYSIMD_FLOAT64_C(   -68.19), EASYSIMD_FLOAT64_C(   550.13), EASYSIMD_FLOAT64_C(   607.02),
      EASYSIMD_FLOAT64_C(   675.35), EASYSIMD_FLOAT64_C(   328.93), EASYSIMD_FLOAT64_C(   931.10), EASYSIMD_FLOAT64_C(   216.78),
      EASYSIMD_FLOAT64_C(    53.77), EASYSIMD_FLOAT64_C(   232.12), EASYSIMD_FLOAT64_C(  -162.54), EASYSIMD_FLOAT64_C(  -829.46),
      EASYSIMD_FLOAT64_C(   285.06), EASYSIMD_FLOAT64_C(   252.39), EASYSIMD_FLOAT64_C(   824.26), EASYSIMD_FLOAT64_C(  -676.07),
      EASYSIMD_FLOAT64_C(   109.00), EASYSIMD_FLOAT64_C(    -2.92), EASYSIMD_FLOAT64_C(  -861.03), EASYSIMD_FLOAT64_C(   562.19),
      EASYSIMD_FLOAT64_C(   501.60), EASYSIMD_FLOAT64_C(   257.18), EASYSIMD_FLOAT64_C(   552.37), EASYSIMD_FLOAT64_C(   157.85),
      EASYSIMD_FLOAT64_C(   445.23), EASYSIMD_FLOAT64_C(  -775.17), EASYSIMD_FLOAT64_C(  -135.54), EASYSIMD_FLOAT64_C(   102.84),
      EASYSIMD_FLOAT64_C(  -190.95), EASYSIMD_FLOAT64_C(  -875.73), EASYSIMD_FLOAT64_C(   288.11), EASYSIMD_FLOAT64_C(   662.76),
      EASYSIMD_FLOAT64_C(    56.08), EASYSIMD_FLOAT64_C(  -161.77), EASYSIMD_FLOAT64_C(   269.79), EASYSIMD_FLOAT64_C(  -268.57),
      EASYSIMD_FLOAT64_C(  -832.84), EASYSIMD_FLOAT64_C(   200.88), EASYSIMD_FLOAT64_C(   948.20), EASYSIMD_FLOAT64_C(   220.93),
      EASYSIMD_FLOAT64_C(  -567.00), EASYSIMD_FLOAT64_C(  -214.34), EASYSIMD_FLOAT64_C(   391.47), EASYSIMD_FLOAT64_C(   718.06),
      EASYSIMD_FLOAT64_C(  -961.95), EASYSIMD_FLOAT64_C(   215.73), EASYSIMD_FLOAT64_C(  -958.01), EASYSIMD_FLOAT64_C(   147.06),
      EASYSIMD_FLOAT64_C(  -787.19), EASYSIMD_FLOAT64_C(  -819.03), EASYSIMD_FLOAT64_C(  -290.76), EASYSIMD_FLOAT64_C(   714.41),
      EASYSIMD_FLOAT64_C(   438.15), EASYSIMD_FLOAT64_C(  -738.39), EASYSIMD_FLOAT64_C(  -127.74), EASYSIMD_FLOAT64_C(  -116.62),
      EASYSIMD_FLOAT64_C(  -513.57), EASYSIMD_FLOAT64_C(   736.72), EASYSIMD_FLOAT64_C(   986.22), EASYSIMD_FLOAT64_C(   295.48),
      EASYSIMD_FLOAT64_C(   860.99), EASYSIMD_FLOAT64_C(   274.33), EASYSIMD_FLOAT64_C(   -41.75), EASYSIMD_FLOAT64_C(   -82.93),
      EASYSIMD_FLOAT64_C(  -887.44), EASYSIMD_FLOAT64_C(  -771.97) };
  static const easysimd_float64 b = EASYSIMD_FLOAT64_C(   648.49);
  static const easysimd_float64 e[] =
    { EASYSIMD_FLOAT64_C(  -472.33), EASYSIMD_FLOAT64_C(   -82.02), EASYSIMD_FLOAT64_C(  1320.49), EASYSIMD_FLOAT64_C(   364.85),
      EASYSIMD_FLOAT64_C(  -246.69), EASYSIMD_FLOAT64_C(   392.03), EASYSIMD_FLOAT64_C(   778.81), EASYSIMD_FLOAT64_C(  -675.92),
      EASYSIMD_FLOAT64_C(  1189.92), EASYSIMD_FLOAT64_C(   724.84), EASYSIMD_FLOAT64_C(   949.51), EASYSIMD_FLOAT64_C(  1269.17),
      EASYSIMD_FLOAT64_C(   116.77), EASYSIMD_FLOAT64_C(  -947.06), EASYSIMD_FLOAT64_C(  -585.06), EASYSIMD_FLOAT64_C(  1302.21),
      EASYSIMD_FLOAT64_C(    38.87), EASYSIMD_FLOAT64_C(   856.61), EASYSIMD_FLOAT64_C(   821.31), EASYSIMD_FLOAT64_C(  1463.53),
      EASYSIMD_FLOAT64_C(  -546.82), EASYSIMD_FLOAT64_C(   153.01), EASYSIMD_FLOAT64_C(   766.70), EASYSIMD_FLOAT64_C(  1638.67),
      EASYSIMD_FLOAT64_C(   656.25), EASYSIMD_FLOAT64_C(  -811.95), EASYSIMD_FLOAT64_C(  -327.54), EASYSIMD_FLOAT64_C(  1355.11),
      EASYSIMD_FLOAT64_C(  1306.10), EASYSIMD_FLOAT64_C(   232.71), EASYSIMD_FLOAT64_C(   259.81), EASYSIMD_FLOAT64_C(  -166.24),
      EASYSIMD_FLOAT64_C(  -146.29), EASYSIMD_FLOAT64_C(   580.30), EASYSIMD_FLOAT64_C(   550.13), EASYSIMD_FLOAT64_C(   607.02),
      EASYSIMD_FLOAT64_C(  1323.84), EASYSIMD_FLOAT64_C(   977.42), EASYSIMD_FLOAT64_C(  1579.59), EASYSIMD_FLOAT64_C(   865.27),
      EASYSIMD_FLOAT64_C(   702.26), EASYSIMD_FLOAT64_C(   880.61), EASYSIMD_FLOAT64_C(   485.95), EASYSIMD_FLOAT64_C(  -829.46),
      EASYSIMD_FLOAT64_C(   285.06), EASYSIMD_FLOAT64_C(   900.88), EASYSIMD_FLOAT64_C(  1472.75), EASYSIMD_FLOAT64_C(  -676.07),
      EASYSIMD_FLOAT64_C(   109.00), EASYSIMD_FLOAT64_C(    -2.92), EASYSIMD_FLOAT64_C(  -212.54), EASYSIMD_FLOAT64_C(  1210.68),
      EASYSIMD_FLOAT64_C(  1150.09), EASYSIMD_FLOAT64_C(   905.67), EASYSIMD_FLOAT64_C(   552.37), EASYSIMD_FLOAT64_C(   806.34),
      EASYSIMD_FLOAT64_C(  1093.72), EASYSIMD_FLOAT64_C(  -775.17), EASYSIMD_FLOAT64_C(   512.95), EASYSIMD_FLOAT64_C(   102.84),
      EASYSIMD_FLOAT64_C(   457.54), EASYSIMD_FLOAT64_C(  -227.24), EASYSIMD_FLOAT64_C(   288.11), EASYSIMD_FLOAT64_C(   662.76),
      EASYSIMD_FLOAT64_C(    56.08), EASYSIMD_FLOAT64_C(  -161.77), EASYSIMD_FLOAT64_C(   918.28), EASYSIMD_FLOAT64_C(   379.92),
      EASYSIMD_FLOAT64_C(  -184.35), EASYSIMD_FLOAT64_C(   849.37), EASYSIMD_FLOAT64_C(  1596.69), EASYSIMD_FLOAT64_C(   869.42),
      EASYSIMD_FLOAT64_C(    81.49), EASYSIMD_FLOAT64_C(   434.15), EASYSIMD_FLOAT64_C(  1039.96), EASYSIMD_FLOAT64_C(   718.06),
      EASYSIMD_FLOAT64_C(  -313.46), EASYSIMD_FLOAT64_C(   215.73), EASYSIMD_FLOAT64_C(  -309.52), EASYSIMD_FLOAT64_C(   147.06),
      EASYSIMD_FLOAT64_C(  -787.19), EASYSIMD_FLOAT64_C(  -170.54), EASYSIMD_FLOAT64_C(  -290.76), EASYSIMD_FLOAT64_C(  1362.90),
      EASYSIMD_FLOAT64_C(   438.15), EASYSIMD_FLOAT64_C(  -738.39), EASYSIMD_FLOAT64_C(   520.75), EASYSIMD_FLOAT64_C(  -116.62),
      EASYSIMD_FLOAT64_C(  -513.57), EASYSIMD_FLOAT64_C(  1385.21), EASYSIMD_FLOAT64_C(  1634.71), EASYSIMD_FLOAT64_C(   943.97),
      EASYSIMD_FLOAT64_C(   860.99), EASYSIMD_FLOAT64_C(   922.82), EASYSIMD_FLOAT64_C(   -41.75), EASYSIMD_FLOAT64_C(   -82.93),
      EASYSIMD_FLOAT64_C(  -887.44), EASYSIMD_FLOAT64_C(  -123.48) };
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    pv = easysimd_svcmplt_s64(pg, easysimd_svld1_s64(pg, &(p[i])), easysimd_svdup_n_s64(INT64_C(0)));
    rv = easysimd_svadd_n_f64_m(pv, av, b);
  ...
  while (easysimd_svptest_first(easysimd_svptrue_b64(), pg)) {
    ...
    ev = easysimd_svadd_n_f64_m(pv, av, b);
...
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s8_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s8_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s16_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s16_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s32_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s32_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s64_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_s64_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u8_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u8_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u16_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u16_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u32_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u32_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u64_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_u64_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_f32_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_f32_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_f64_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_f64_m)

EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s8_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s8_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s16_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s16_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s32_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s32_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s64_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_s64_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u8_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u8_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u16_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u16_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u32_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u32_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u64_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_u64_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_f32_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_f32_m)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_f64_z)
EASYSIMD_TEST_FUNC_LIST_ENTRY(svadd_n_f64_m)
